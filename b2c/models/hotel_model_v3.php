...
class Hotel_Model_V3 extends CI_Model
{
	...
	function save_search_history_data($search_request)
	{
		...
		/* if ($search_request['hotel_version'] == 'new') {
		...
		if (isset($search_request['search_type'])) {
			...
			if ($search_request['search_type'] == 'location_search') {
			...
			} elseif ($search_request['search_type'] == 'hotel_name_search') {
				$request['hotel_name'] = $search_request['CityId'];
				$request['CountryCode'] = $search_request['CountryCode'];
			} elseif ($search_request['search_type'] == 'hotel_search') {
				$request['hotel_destination'] = $search_request['CityId'];
				...
				$request['hotelspro_city_id'] = @$hotel_city_details['hotelspro_city_id'];
		...
		$request['nationality'] = $search_request['nationality'];
		$request['ministarrating'] = $search_request['ministarrating'];
		$request['maxistarrating'] = $search_request['maxistarrating'];
		...
		if ($search_request['IslandHopper']) {
			$request['IslandHopper'] = @$search_request['IslandHopper'];
		}
	...
	function get_safe_search_data($search_id)
	{
		...
		if ($search_data != false) {
			...
			if (isset($temp_search_data['city']) == true) {
				...
				if ($temp_search_data['search_type'] == 'hotel_name_search') {
					if (!(isset($temp_search_data['hotel_name'])) && !(isset($temp_search_data['CountryCode']))) {
						$success = false;
					}
				}
			}
			if (isset($temp_search_data['search_type'])) {
				if ($temp_search_data['search_type'] == 'location_search') {
				...
				} else if ($temp_search_data['search_type'] == 'hotel_name_search') {
					...
					$clean_search['hotelspro_city_id'] = @$temp_search_data['hotelspro_city_id'];
					$clean_search['hotel_origin'] = @$temp_search_data['hotel_origin'];
					$clean_search['ratehawk_id'] = @$temp_search_data['ratehawk_id'];
				}
			} else {
				$clean_search['CountryCode'] = @$temp_search_data['CountryCode'];
				$clean_search['city_code'] = @$temp_search_data['city_code'];
			...
			if (isset($temp_search_data['nationality']) == true) {
				$clean_search['nationality'] = $temp_search_data['nationality'];
			} else {
				$success = true;
			}

			if (isset($temp_search_data['ministarrating']) == true) {
				$clean_search['ministarrating'] = $temp_search_data['ministarrating'];
			}

			if (isset($temp_search_data['maxistarrating']) == true) {
				$clean_search['maxistarrating'] = $temp_search_data['maxistarrating'];
			}
			...
			if (isset($temp_search_data['IslandHopper']) == true) {
				$clean_search['IslandHopper'] = $temp_search_data['IslandHopper'];
			}
	...
	function get_search_data_grn($search_id)
	{
			$search_data = $this->custom_db->single_table_records('search_history', '*', array('search_type' => META_ACCOMODATION_COURSE, 'origin' => $search_id));
			if ($search_data['status'] == true) {
	...
	{
		$result_data = $this->db->query('SELECT hotel_code from hb_master_data where city_code="' . $destination_code . '"')->result_array();
		return $result_data;
	}


	public  function store_api_request($request_type, $request, $remarks)
	{
	...
	{
		$search_data = $this->get_search_data_grn($search_id);
	...
	function save_booking_details(
		...
		$hotel_version = HOTEL_VERSION_1,
		$gst = 0,
		$hotel_markup_price = 0,
		$admin_markup_gst = 0
	) {
	 ...
	 */
	function get_booking_details($app_reference, $booking_source, $booking_status = NULL)
	{
	...
	public function update_cancellation_details($AppReference, $cancellation_details)
	{
		...
		$this->custom_db->update_record('hotel_booking_details', array('status' => $booking_status,'booking_status'=>$booking_status), array('app_reference' => $AppReference)); //later
	 ...
	 * @param string $h_cond
	 */
	function get_static_hotel_info($hotel_code, $cols = NULL)
	{
		...
		if ($cols == NULL) {
			$cols = 'hi.image_path AS img, h.hotel_code AS hc, h.category_code AS cc, h.address AS address, h.postal_code AS postal,
				h.chain_code as chain,  h.accomodation_type_code as accomodation_type_code, h.country_code as country_code, h.hotel_email as email, h.website as website, h.description as description,
				hcl.description hotel_chain, cl.accommodationType as acc_type, cl.description as cat_desc';
		}
		$query = ' SELECT ' . $cols . '  FROM hb_hotel_details AS h LEFT JOIN hb_hotel_images AS hi ON h.hotel_code=hi.hotel_code
		...
		if (valid_array($data) == true) {
			foreach ($data as $k => $v) {
				...
				$image_empty = array();
				if (empty($v['img'])) {
					//$image_empty = $this->get_static_hotel_images($v['hc']);
					/*foreach ($image_empty as $key => $value) {
																					
																					if(!empty($value['imagePath']) && (valid_array(getimagesize('http://photos.hotelbeds.com/giata/'.$value['imagePath'])) == true)){
																						$v['img'] = $value['imagePath'];
																						break;
																					}
																				}*/
				}
				//if(empty($country_name)){
				//$country_name = $GLOBALS ['CI']->hotel_model->get_hotel_country_name($v['country_code']);
				//}

				//debug($country_name); exit;

				$resp['info'][$v['hc']] = array(
					'chain' => $v['chain'],
					'accomodation_type_code' => $v['accomodation_type_code'],
					'hotel_chain' => $v['hotel_chain'],
					'category_code' => $v['cc'],
					...
					'accomodation_type' => $v['acc_type'],
					'category_description' => $v['cat_desc']
				);
	 ...
	 * @param $cols
	 */
	function get_static_hotel_facility_info($hotel_code, $cols = NULL, $filter = NULL)
	{
		...
		if ($cols == NULL) {
			$cols = 'hf.indFee AS additional_cost, hf.indLogic AS not_at_hotel, hf.hotel_code AS hc, hf.facility_code as fc, hfd.display_class AS dis_class, hf.facility_group_code as fgc,
	...
	//get hotel bed static facilities
	public function get_hotel_bed_facilities($hotel_code)
	{

		$result_data = $this->db->query("SELECT DISTINCT(d.description) FROM `hb_hotel_facilities` as f join hb_hotel_facilities_description as d on f.facility_code = d.facility_code where f.hotel_code = '" . $hotel_code . "'")->result_array();
	...
	{
		$result_data = $this->db->query('SELECT hotel_id from agoda_hotel_master where city_id="' . $destination_code . '"')->result_array();
		return $result_data;
	}
	...
	public function validate_city_id($CityId, $CountryCode)
	{
		$response['status'] = true;
		$response['data'] = array();
		$location_details = $this->custom_db->single_table_records('all_api_city_master', 'country_code, origin', array('origin like' => $CityId, 'country_code like' => $CountryCode));
