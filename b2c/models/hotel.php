		...
		if (isset($params['booking_source']) == true) {
			...
			$params['op'] == 'get_details' and $safe_search_data['status'] == true) {
				...
				if ($raw_hotel_details['status']) {
					...
					redirect(base_url().'hotel/exception?op=Remote IO error @ Session Expiry&notification=session');
				}
			} else {
				redirect(base_url());
			}
...
//			$verification_amount = roundoff_number($amount+$convenience_fees-$promocode_discount);
	...
	*/
		...
		if ($domain_balance_status) {
			...
			if ($temp_booking != false) {
				...
				if ($booking['status'] == SUCCESS_STATUS) {
					...
					redirect(base_url().'hotel/exception?op=booking_exception&notification='.$booking['msg']);
				}
			}
			//release table lock
		} else {
	...
	function exception()
	{
		...
		redirect(base_url().'hotel/event_logger/'.$eid.'/'.$is_session);
	}
