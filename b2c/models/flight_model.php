...

{
	 ...
	 */
	 ...
	 function get_airports_lat_long()
	{

		$query = 'SELECT * FROM flight_airport_list WHERE lat IS NULL';

		return $this->db->query($query)->result();
	}

	function get_airports_list_lat_long()
	{
	...
	function update_airport_lat_long($airportCode = '', $lat = '', $long='',$timezonename='')
	{

		$query = 'UPDATE flight_airport_list SET lat="' . $lat . '", lon = "' . $long . '", timezonename="'.$timezonename.'" WHERE airport_code="' . $airportCode . '"';


		//$query = 'UPDATE from flight_airport_list set lat="'.$lat.'" ,long="'.$long.'" where airport_code="'.$airportCode.'"';

		return $this->db->query($query);
	}

	function get_nearBy_airport_list($airport_code,$lat, $long, $distance)
	{
		// $query ='SELECT *, (6371 * acos(cos(radians('.$lat.')) * cos(radians(lat)) * cos(radians(lon) - radians(' . $long . ')) + sin(radians(' . $lat . ')) * sin(radians(lat)))) AS distance 
		// FROM flight_airport_list WHERE isTravellerAirport=1 and status=1  HAVING distance < '.$distance.' AND airport_code!="'.$airport_code.'" ORDER BY distance LIMIT 0, 20';

		$query ='SELECT *, 
       (6371 * acos(cos(radians(25.248665)) * cos(radians(lat)) * cos(radians(lon) - radians(' . $long . ')) 
       + sin(radians(' . $lat . ')) * sin(radians(lat)))) AS distance 
FROM flight_airport_list 
WHERE isTravellerAirport = 1 
      AND status = 1 
...
LIMIT 20';
...

	{
		...
		$query = 'Select * from flight_airport_list where isTravellerAirport=1 and status=1 and airport_city like ' . $search_chars . '

		OR airport_code like ' . $search_chars . ' OR country like ' . $search_chars . '

		ORDER BY IsInternational DESC, 
...
//            echo($query);die;   
  ...
  function booking_count($condition=array(), $count=false, $offset=0, $limit=100000000000)

	{
		...
		if ($count) {
			 ...
			 $query = 'select count(distinct(BD.app_reference)) AS total_records from flight_booking_details BD
					...
					where domain_origin='.get_domain_auth_id().' AND BD.created_by_id ='.$GLOBALS['CI']->entity_user_id.' and '.'BD.status !="'.$status.'"';
...

	{
		...
		$bd_query1 = 'select * from flight_booking_transaction_details AS FTD WHERE (FTD.app_reference like '.$this->db->escape($app_reference).' || FTD.pnr like '.$this->db->escape($app_reference).')';
...

	{
		...
		if(valid_array($from_loc) == true || valid_array($to_loc)) { //Multicity
			...
			$query = 'SELECT COUNT(DISTINCT CountryCode) total FROM flight_airport_list WHERE airport_code IN ('.$airport_city_codes.') ';

		} else { //Oneway/RoundWay

			$query = 'SELECT COUNT(DISTINCT CountryCode) total FROM flight_airport_list WHERE airport_code IN ('.$this->db->escape($from_loc).','.$this->db->escape($to_loc).') ';
	...
	function is_domestic_flight_old($from_loc, $to_loc)
		...
		if(isset($temp_search_data['trip_type']) == true){
			...
			if($temp_search_data['trip_type'] != 'multicity'){
				...
				if(isset($temp_search_data['from']) == true){
					...
					$clean_search['fromairportloc'] = @$temp_search_data['fromairportloc'];
				...
				if(isset($temp_search_data['to']) == true){
					...
					$clean_search['toairportloc'] = @$temp_search_data['toairportloc'];
				...
				for($i=0; $i<count($temp_search_data['depature']); $i++){
					...
					if($success == true) {
						...
						&& (strtotime($temp_search_data['depature'][$i]) >= strtotime(date('Y-m-d', @$temp_search_data['depature'][$i-1])))) {
							...
							$clean_search['fromairportloc'][$i] = @$temp_search_data['fromairportloc'][$i];
						...
						if(isset($temp_search_data['from'][$i]) == true){
							...
							$clean_search['toairportloc'][$i] = @$temp_search_data['toairportloc'][$i];
			...
			if(isset($temp_search_data['alt_days']) == true){

				$clean_search['alt_days'] = $temp_search_data['alt_days'];

			}
			
			$clean_search['direct_flight'] = !empty($temp_search_data['direct_flight']) && $temp_search_data['direct_flight'] == 1 ? 1:0;
	...
	$transaction_currency, $currency_conversion_rate, $gst_details, $phone_country_code, $total_fare_tax = 0 , $total_fare_markup = 0, $agentServicefee = 0)
...

	{
		...
		$data['is_lead'] = ($is_lead=='')?0:$is_lead;
...

	{
		...
		$baggage_total_price = round($this->get_baggage_total_price($app_reference));

		

		//get meal price

		$meal_total_price = round($this->get_meal_total_price($app_reference));
		...
		$extra_service_total_price = $baggage_total_price+$meal_total_price+$seat_total_price;

		//debug($baggage_total_price); exit;
	...
	function flight_top_destinations($domain=1)
	{
		/*$query = 'Select * from bus_stations where top_destination = '.ACTIVE;
		$data = $this->db->query($query)->result_array();
		return $data;*/
		$query = 'Select * from top_flight_destinations WHERE status = '.ACTIVE.' and domain='.$domain.' ORDER BY from_airport_name LIMIT 12';
		$data = $this->db->query($query)->result_array();
		return $data;
	}
	/*function flight_top_cities_routes()
	{
		$query = 'Select * from top_flight_cities_routes WHERE `user_type` = 1 AND status = '.ACTIVE.' order by origin DESC LIMIT 3';
		$data = $this->db->query($query)->result_array();
		return $data;
	}*/
    ...
    function save_insurance_details($response)
	...
	$data['app_reference'] = $response['app_reference'];

	$data['status'] = $response['status'];

	$data['response'] = json_encode($response['response']);

	$data['travel_date'] =  $response['travel_date'];
    ...
    function get_county_code($country){



    	$country_data = $this->db->query('select iso_country_code from country_list where en_name ="'.$country.'"')->row_array();

   		if(valid_array($country_data)){

   			

   			return $country_data['iso_country_code'];

   		}

    }

    function update_pax_details($data, $id) {

		if ($this->custom_db->update_record("flight_booking_passenger_details", $data, array("origin" => $id))) {
	...
	function update_booking_details($data, $id) {
		...
		if ($this->custom_db->update_record("flight_booking_details", $data, array("origin" => $id))) {
	...
	public function checkUSDestination($search_chars){
		//   debug($search_chars); exit;
		//$search_chars = $this->db->escape('%'.$search_chars.'%');
		$search_chars = $search_chars;
		$query = 'SELECT * FROM `flight_airport_list` WHERE `origin` = '.$search_chars.' ORDER BY `origin` ASC';
		$result=$this->db->query($query)->row_array();
		$flag_name=trim($result['flag']);
		if($flag_name=='us'){
				return 1;
		}else{
			return 0;
	...
	public function get_cities_names($airport_codes){
		$cities = array();
		$recs = $this->db->select('airport_code,airport_city')->where_in('airport_code',$airport_codes)->get('flight_airport_list')->result_array();
		if($recs){
			foreach($recs as $rec){
				$cities[$rec['airport_code']] = $rec['airport_city'];
			}
		}
		return $cities;
	}
