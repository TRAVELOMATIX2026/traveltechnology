...

{
...

	{
		...
		if (empty($table) == false and is_string($table) == true) {
			...
			*/
			...
			$tmp_data = $this->db->get($table, $limit, $offset);
	...
	public function escape_like_str($str) {
        // Use the database escape function to escape the string
        $escaped_str = $this->db->escape_str($str);
        
        // Escape the percent and underscore characters used in LIKE queries
        $escaped_str = str_replace(['%', '_'], ['\%', '\_'], $escaped_str);
	...
	function get_phone_code_list(){
		...
		$query = 'SELECT origin,country_code,name FROM api_country_list WHERE country_code!=0  ORDER BY country_code';
