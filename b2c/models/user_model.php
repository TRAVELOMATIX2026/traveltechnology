...
{
	function create_user($email, $password, $country_code, $first_name = 'Customer', $phone = '', $creation_source = 'portal', $user_type = '')
	{
		...
		$new_user_promo = $this->db->query('SELECT origin, promo_code, value, expiry_date FROM promo_code_list WHERE module="new_register" AND expiry_date >= CURDATE()')->result_array();
		$data['discount_coupon_origin'] = $new_user_promo[0]['origin'];
		$data['promo_code'] = $new_user_promo[0]['promo_code'];
		$data['promo_code_total_value'] = $new_user_promo[0]['value'];
		$data['promo_code_remaining_value'] = $new_user_promo[0]['value'];
		$dateTime = new DateTime($data['created_datetime']);
		$dateTime->modify('+1 year');
		$data['promo_code_expiry_date'] = $dateTime->format('Y-m-d H:i:s');
	...
	public function get_user_details($condition = array(), $count = false, $offset = 0, $limit = 10000000000, $order_by = array())
	{
		...
		// debug($count);exit;
		if (!$count) {
			/*echo 'SELECT U.*, UT.user_type as user_profile_name, ACL.country_code as country_code_value
			FROM user AS U, user_type AS UT, api_country_list AS ACL
		 	WHERE U.user_type=UT.origin 
		 	AND U.country_code=ACL.country_code'.$filter_condition.' limit '.$limit.' offset '.$offset.' '.$filter_order_by;exit;*/
			/*echo 'SELECT U.*, UT.user_type as user_profile_name, ACL.country_code as country_code_value
			FROM user AS U INNER JOIN user_type AS UT ON U.user_type=UT.origin LEFT JOIN `api_country_list` AS ACL ON U.country_code=ACL.country_code
		 	WHERE 1=1 '.$filter_condition.' limit '.$limit.' offset '.$offset.' '.$filter_order_by;*/
			$result = $this->db->query('SELECT U.*, UT.user_type as user_profile_name, ACL.country_code as country_code_value
			FROM user AS U INNER JOIN user_type AS UT ON U.user_type=UT.origin LEFT JOIN `api_country_list` AS ACL ON U.country_code=ACL.country_code
		 	WHERE 1=1 ' . $filter_condition . ' limit ' . $limit . ' offset ' . $offset . ' ' . $filter_order_by)->result_array();
			// debug($this->db->last_query());exit;
			return $result;
		} else {
			/*echo 'SELECT count(*) as total FROM user AS U, user_type AS UT, api_country_list AS ACL
		 WHERE U.user_type=UT.origin 
		 AND U.country_code=ACL.country_code'.$filter_condition.' limit '.$limit.' offset '.$offset;exit;*/
			/*echo 'SELECT count(*) as total FROM user AS U INNER JOIN user_type AS UT ON U.user_type=UT.origin LEFT JOIN api_country_list AS ACL ON U.country_code=ACL.country_code
		 WHERE 1=1 '.$filter_condition.' limit '.$limit.' offset '.$offset;*/
			return $this->db->query('SELECT count(*) as total FROM user AS U INNER JOIN user_type AS UT ON U.user_type=UT.origin LEFT JOIN api_country_list AS ACL ON U.country_code=ACL.country_code
		 WHERE 1=1 ' . $filter_condition . ' limit ' . $limit . ' offset ' . $offset)->row();
		}
		/*echo 'SELECT count(*) as total FROM user AS U, user_type AS UT, api_country_list AS ACL
		 WHERE U.user_type=UT.origin 
		 AND U.country_code=ACL.country_code'.$filter_condition.' limit '.$limit.' offset '.$offset;exit;*/
