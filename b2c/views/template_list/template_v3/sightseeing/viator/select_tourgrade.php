<?php 
 ...
 Js_Loader::$js[] = array('src' => $GLOBALS['CI']->template->template_js_dir('page_resource/pax_count.js'), 'defer' => 'defer'); 
	...
	foreach ($product_date_arr as $key => $value) {
	  ...
	  $end_year_date = $last_year_arr;  
	  $start_date_arr = reset($product_dates_arr);
	  $start_date_picker = $start_date_arr;
		            ...
		            <div class="line light-border-b option-row">
		             <?php
		               ...
		               <div class="unit size1of4">
		                <p class="mas">
							<?php if($grade_title):?>
								<span class="strong"><?=$grade_title?></span>
							<?php endif;?>
							<?php if($grade_code):?>
								<span class="xsmall hint">Code: <?=$grade_code?></span>
							<?php endif;?>
							</p>
		               </div>
		               ...
		               <?php if($t_value['TotalPax']):?>
		               	...
		               	<?php if(isset($t_value['ageBandsRequired'])):?>
		               		<?php 
		               			// Traveler requirements info removed - now shown on main details page
