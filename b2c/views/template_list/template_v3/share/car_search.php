...
// debug($car_search_params);exit;
	...
	if ((isset($car_search_params) == false) || (isset($car_search_params) == true && valid_array($car_search_params) == false)) {
		...
		if ($sid > 0) {
			...
			$car_search_params = $this->CI->car_model->get_safe_search_data($sid);
	...
	Js_Loader::$css[] = array('href' => $GLOBALS['CI']->template->template_css_dir('car-search-form.css'), 'media' => 'screen');
...
<script>
    ...
    // Toggle Dropdown for Extra Options
    $(function () {
        $("#extra_options_toggle").click(function () {
            $(this).toggleClass("active");
            $("#extra_options_content").toggleClass("active");
        });
    });
