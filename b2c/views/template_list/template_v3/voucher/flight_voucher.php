...
$base_fare_amount = round($booking_details['net_fare'], 2);
$base_convinence_amount = round($booking_details['convinence_amount'] * $booking_details['currency_conversion_rate'], 2);
$base_tax_amount = ($booking_details['admin_markup'] + $base_convinence_amount + $booking_details['agent_markup']) - $booking_details['admin_commission'] + $booking_details['admin_tds'];
$base_tax_amount = round($base_tax_amount, 2);
$base_discount_amount = $booking_details['discount'];
$base_grand_total = ($base_fare_amount + $base_convinence_amount + $base_tax_amount) - $base_discount_amount;
$adult_count = 0;
$infant_count = 0;
...
$BaseFare = round($BaseFare * $booking_details['currency_conversion_rate'], 2);
$Tax = $fare_details['Fare']['Tax'] + @$retur_fare_details['Fare']['Tax'];
$Tax = round($Tax * $booking_details['currency_conversion_rate'], 2);
$Tax = round($Tax + $base_tax_amount, 2);
...
$base_grand_total = round(($base_grand_total + $seat_price + $meal_price + $baggage_price), 2);
...
if (isset($airline_contact_no['data'][0])) {
    $airline_number = '<span><img style="vertical-align:middle" src=' . BASE_PATH . SYSTEM_IMAGE_DIR . 'phone.png /><span style="font-size:16px;color:#808080;vertical-align:middle;font-weight: 600;"> &nbsp;' . $airline_contact_no['data'][0]['phone_number'] . '</span></span> ';
} else {
    $airline_number = '';
}
...
//echo roundoff_number(($booking_details['total_fare_markup']+$booking_details['total_fare_tax']+$seat_price+$meal_price+$baggage_price)-$booking_details['discount']); exit;
?>
<style type="text/css">
    td,
    th {
...
<div style="background:#ccc; width:100%; position:relative; padding:20px 0;" id="tickect_flightonword">
    <table cellpadding="0" border-collapse cellspacing="0" width="100%" style="font-size:12px; font-family: 'Open Sans', sans-serif; width:850px; margin:0px auto;background-color:#fff; padding:45px;border-collapse:separate; color: #000;">
        <tbody>
            <tr>
                <td colspan="4" style="font-size: 18px;font-weight: 600;text-align: center;padding: 10px 0 0;">E-Ticket </td>
            </tr>
            <tr style="background: transparent;color:black;">
                <td colspan="2" style="padding-bottom:10px"><img style="width:170px;" src="<?= BASE_PATH . $GLOBALS['CI']->template->domain_images($data['logo']) ?>" /></td>
            ...
            <tr>
                <td align="right" colspan="4" style="line-height:26px; border-top:1px solid #808080; border-bottom:1px solid #808080;"><span style="font-size:12px;">Status: </span><strong class="<?php echo booking_status_label($booking_transaction_details[0]['status']) ?>" style=" font-size:14px;">
                        <?php
            ...
            <?php } ?>
            <tr>
                <td style="padding:10px 0;width:2%;"><img style="width:60px;" src="<?= BASE_PATH . SYSTEM_IMAGE_DIR . 'airline_logo/' . $itinerary_details[0]['airline_code'] . '.svg' ?>" /></td>
                <td style="padding:10px 20px; line-height:25px;width:35%"><span style="display: block;border-right: 1px solid #999;"><span style="font-size:14px;"><?= @$itinerary_details[0]['airline_name'] ?></span><br><?php echo $airline_number ?></span></td>
                <td style="padding:10px 0; padding-left: 8%; line-height:25px;"><span style="font-size:14px;">Agency</span><br><span><img style="vertical-align:middle" src="<?= BASE_PATH . SYSTEM_IMAGE_DIR . 'phone.png' ?>" /> <span style="font-size:16px;color:#808080;vertical-align:middle;font-weight: 600;"> &nbsp;<?php echo $data['phone'] ?></span></span></td>
                <?php if ((!empty($gds_pnr)) || (!empty($airline_pnr))) { ?>
                    <td style="padding:10px 0;float:right;">
                        <span style="font-size:14px; display:block">
                            <?php if ($booking_transaction_details[0]['status'] == 'BOOKING_CONFIRMED') { ?>
                                <span style="padding: 5px 0 0;">AIRLINE PNR: </span>
                                <span style="font-size:18px;line-height:28px;padding-bottom: 5px;font-weight: 600;"><?= @$airline_pnr ?></span><br>
                            <?php } ?>
                            <?php if (($booking_transaction_details[0]['status'] == 'BOOKING_CONFIRMED' || $booking_transaction_details[1]['status'] == 'BOOKING_HOLD') && !empty($gds_pnr)) { ?>
                                <span style="padding: 0 5px 5px 0;">GDS PNR: </span>
                                <span style="font-weight: 600;font-size: 18px;"><?= @$gds_pnr ?></span>
                            <?php } ?>
                        </span>
                    </td>
                <?php } ?>
            ...
            <tr>
            <tr>
                <td colspan="4" style="border:1px solid #808080; padding:0;">
                    <table cellspacing="0" cellpadding="5" width="100%" style="font-size:12px; padding:0;">
                        <tbody>
                            <tr>
                                <td colspan="2" style="background-color:#808080; color:#fff"><img style="vertical-align:middle" src="<?= BASE_PATH . SYSTEM_IMAGE_DIR . 'flight.png' ?>"> &nbsp;<span style="vertical-align:middle;font-size:13px">Onward Flight Details</span></td>
                                <td align="right" colspan="2" style="background-color:#808080; color:#fff"><span style="font-size:10px">*Please verify flight times with the airlines prior to departure</span></td>
                            </tr>
                            <?php
                            ...
                            ?>
                                        <tr>
                                            <td colspan="2" style="background-color:#808080; color:#fff"><img style="vertical-align:middle" src="<?= BASE_PATH . SYSTEM_IMAGE_DIR . 'flight.png' ?>"> &nbsp;<span style="vertical-align:middle;font-size:13px">Return Flight Details</span></td>
                                            <td align="right" colspan="2" style="background-color:#808080; color:#fff"><span style="font-size:10px">*Please verify flight times with the airlines prior to departure</span></td>
                                        </tr>
                                        <?php
                                    }
                                    ...
                                    if (valid_array($segment_details_v) == true) {
                                        if ($fight_count == 1): ?>
                                            <tr>
                                                <td style="background-color:#d9d9d9; color:#555555"><span>Flight <?php echo $fight_count; ?></span></td>
                                                <td style="background-color:#d9d9d9; color:#555555"><img style="vertical-align:middle" src="<?= BASE_PATH . SYSTEM_IMAGE_DIR . 'flight_up.png' ?>">&nbsp;<span style="vertical-align:middle">Departing</span></td>
                                                <td style="background-color:#d9d9d9; color:#555555"><img style="vertical-align:middle" src="<?= BASE_PATH . SYSTEM_IMAGE_DIR . 'flight_down.png' ?>">&nbsp;<span style="vertical-align:middle">Arriving</span></td>
                                        ...
                                        <tr>
                                            ...
                                            <td><span style="display:block;border-left: 1px solid #999; padding-left:10%">
                                                    <!-- <?php echo $non_stop; ?><br> -->
                                                    <?php echo $segment_details_v['total_duration']; ?><br><?php echo $segment_details_v['is_refundable'] ?></span></td>
                                        </tr>
                                        <?php if ($layour): ?>
                                            <tr>
                                                <td colspan="4" style="background-color:#d9d9d9; color:#808080;width:100%;text-align:center;font-style:italic;font-weight:bold;"><span style="vertical-align:middle;"><span style="color:#000;font-weight:normal;">--------</span> Layover <?php echo $layour ?> <span style="color:#000;font-weight:normal;">--------</span></span></td>
                                            </tr>
                                        <?php endif; ?>
            ...
            <tr>
                <td colspan="4" style="border:1px solid #808080; padding:0;">
                    <table cellspacing="0" cellpadding="5" width="100%" style="font-size:12px; padding:0;">
                        <tbody>
                            <tr>
                                <td colspan="4" style="background-color:#808080; color:#fff"><img style="vertical-align:middle" src="<?= BASE_PATH . SYSTEM_IMAGE_DIR . 'people_group.png' ?>"> &nbsp;<span style="vertical-align:middle;font-size:13px">Passenger(s) Details</span></td>
                            </tr>
                            <tr>
                            ...
                            ?>
                                    <tr>
                                        ...
                                        <td><span><strong><?php echo $pass_name; ?></strong< /span>
                                        </td>
                        ...
                        <tbody>
                            <tr>
                                <td width="50%" style="padding:0;padding-right:14px;vertical-align:top;">
                                    <table cellspacing="0" cellpadding="5" width="100%" style="font-size:12px; padding:0;border:1px solid #808080;">
                                        <tbody>
                                            <tr>
                                                <td style="border-bottom:1px solid #808080;background:#808080;color:#fff;"><span style="font-size:13px">Payment Details</span></td>
                                                <td style="border-bottom:1px solid #808080;background:#808080;color:#fff;"><span style="font-size:13px">Amount ( <?php echo $booking_details['currency'] ?>
                                                        )</span></td>
                                            </tr>
                                            <tr>
                                                <td><span>Air Fare</span></td>
                                                <td><span> <?php echo number_format($BaseFare, 2); //echo number_format(roundoff_number($booking_details['total_fare_markup']),2); 
                                                            ?></span></td>
                                            </tr>
                                            <?php if ($this->uri->segment(2) == 'b2c_flight_voucher'): ?>
                                                <tr>
                                                    <td><span>Taxes &amp; Fees</span></td>
                                                    <td><span> <?php echo number_format($Tax, 2); //echo number_format($booking_details['total_fare_tax'],2); echo number_format(($Tax + $booking_details['admin_markup'] + $booking_details['agent_markup'] + ($booking_details['convinence_amount'])) - $booking_details['admin_commission'] + $booking_details['admin_tds'], 2) 
                                                                ?></span></td>
                                                </tr>
                                            <?php else: ?>
                                                <tr>
                                                    <td><span>Taxes &amp; Fees</span></td>
                                                    <td><span> <?php echo number_format($Tax, 2); //echo number_format($booking_details['total_fare_tax'],2);  //echo number_format(($Tax + $booking_details['admin_markup'] + $booking_details['agent_markup'] + ($booking_details['convinence_amount'])), 2) 
                                                                ?></span></td>
                                                </tr>
                                            <?php endif; ?>
                                            <?php if ($GST > 0) { ?>
                                            ...
                                            if ($booking_details['discount'] != 0) { ?>
                                                <tr>
                                                    ...
                                                    <td><span> <?php echo number_format($base_discount_amount, 2); ?></span></td>
                                                </tr>
                                            <?php } ?>
                                            <tr>
                                                <td style="border-top:1px solid #808080"><span style="font-size:13px">Grand Total</span></td>
                                                <td style="border-top:1px solid #808080">
                                                    <span style="font-size:13px">
                                                        <?php //echo number_format(roundoff_number((ceil($booking_details['total_fare_markup'])+$booking_details['total_fare_tax']+$seat_price+$meal_price+$baggage_price)-$booking_details['discount']),2); 
                                                        ?>
                                                        <?php echo number_format((($booking_details['total_fare_markup'] + $booking_details['total_fare_tax'] + $seat_price + $meal_price + $baggage_price) - $booking_details['discount']), 2); ?>
                                                    </span>
                                                </td>
                                            </tr>
                                        </tbody>
                                ...
                                <td width="50%" style="padding:0;padding-left:14px; vertical-align:top">
                                    <table cellspacing="0" cellpadding="5" width="100%" style="border:1px solid #808080;font-size:12px; padding:0;">
                                        <tbody>
                                            <tr>
                                                <td colspan="4" style="border-bottom:1px solid #808080;background:#808080;color:#fff;"><span style="font-size:13px">Flight Inclusions</span></td>
                                            </tr>

                                            <tr>
                                                <?php
                                                if (!empty($TotalBaggageCheckIN)) { ?>
                                            ...
                                            <tr>
                                                <td colspan="4" style="border-top:1px solid #808080"><span style="font-size:10px; color:#666; line-height:20px;">* Flight inclusions are subject to change with Airlines.</span></td>
                                            </tr>
                                        </tbody>
                                    </table>
                                </td>
                                ...
                                <tr>
                                    <td colspan="2" style="padding-top:10px; vertical-align:top; padding:0;">
                                        <table cellspacing="0" cellpadding="5" width="100%" style="border:1px solid #808080;font-size:12px; padding:0;">
                                            <tbody>
                                                <tr>
                                                    <td colspan="4" style="background-color:#808080; color:#fff;"><span style="font-size:13px">Flight Extra Information</span></td>
                                                </tr>
                                                <?php
                                                if (isset($booking_transaction_details_value['extra_service_details']['baggage_details']) == true && valid_array($booking_transaction_details_value['extra_service_details']['baggage_details']) == true) {
                                                    $baggage_details = $booking_transaction_details_value['extra_service_details']['baggage_details'];
            ...
            <tr>
                <td colspan="4" style="border-bottom:1px solid #808080;padding-bottom:15px"><span style="font-size:13px; color:#555;">Customer Contact Details | E-mail : <?= $booking_details['email'] ?> | Contact No : <?= $booking_details['phone_code'] . " " . $booking_details['phone'] ?></span></td>
            </tr>
            <tr>
                <td style="line-height:15px;padding:0;">&nbsp;</td>
            </tr>
            ...
            <tr>
                <td colspan="4" style="border-bottom:1px solid #808080; line-height:20px; font-size:12px; color:#555">
            ...
            <tr>
                <td colspan="4" align="" style="padding-top:10px;font-size: 14px;">
                    <span style="font-weight:600;"><?php echo strtoupper($data['domainname']) ?></span>&nbsp;&nbsp; | &nbsp;&nbsp;<span style="font-weight:600;">Contact No: </span><?php echo $data['phone'] ?>&nbsp;&nbsp; | &nbsp;&nbsp;<span style="font-weight:600;">Address: </span><?php echo $data['address'] ?>
                </td>
            </tr>
        </tbody>
    </table>
    <table class="no-print" id="printOption" onclick="getPrint();" style="border-collapse: collapse;font-size: 14px; margin: 10px auto; font-family: arial;" width="70%" cellpadding="0" cellspacing="0" border="0">
        <tbody>
            <tr>
                <td align="center"><input style="background:#808080; height:34px; padding:0px 10px; border-radius:4px; border:none; color:#fff; margin:0 2px;" type="button" value="Print" />
            </tr>
        </tbody>
    </table>
</div>
...
?>
    <div style="background:#ccc; width:100%; position:relative" id="tickect_flightreturn">
        <table class="pag_brk" cellpadding="0" border-collapse cellspacing="0" width="100%" style="font-size:12px; font-family: 'Open Sans', sans-serif; width:850px; margin:0px auto;background-color:#fff; padding:45px;border-collapse:separate;color: #000;">
            <tbody>
                ...
                <tr>
                    <td align="right" colspan="4" style="line-height:26px; border-top:1px solid #808080; border-bottom:1px solid #808080;"><span style="font-size:12px;">Status: </span><strong class="<?php echo booking_status_label($booking_transaction_details[1]['status']) ?>" style=" font-size:14px;">
                            <?php
                ...
                <tr>
                    <td style="padding:10px 0;width:2%;"><img style="width:60px;" src="<?= BASE_PATH . SYSTEM_IMAGE_DIR . 'airline_logo/' . $return_segment_details[0]['airline_code'] . '.gif' ?>" /></td>
                    <td style="padding:10px 20; line-height:35px;"><span style="display: block;border-right: 1px solid #999;"><span style="font-size:14px;"><?= @$return_segment_details[0]['airline_name'] ?></span><br><?php echo $airline_number ?></span></td>
                    <td style="padding:10px 0px; padding-left: 8%; line-height:25px;"><span style="font-size:14px;">Agency</span><br><span><img style="vertical-align:middle" src="<?= BASE_PATH . SYSTEM_IMAGE_DIR . 'phone.png' ?>" /> <span style="font-size:16px;color:#808080;vertical-align:middle;font-weight: 600;"> &nbsp;<?php echo $data['phone'] ?></span></span></td>
                        ...
                        <td style="padding:10px 0;text-align: center;">
                            <span style="font-size:14px;display:block">
                                ...
                                <?php if (($booking_transaction_details[1]['status'] == 'BOOKING_CONFIRMED' || $booking_transaction_details[1]['status'] == 'BOOKING_HOLD') && (!empty($return_gds_pnr))) { ?>
                                    <span style="display:block; padding:5px;">GDS PNR: <?= @$return_gds_pnr ?></span>
                                <?php } ?>
                            </span>
                        </td>
                    <?php } ?>
                ...
                <tr>
                    <td colspan="4" style="border:1px solid #808080; padding:0;">
                        <table cellspacing="0" cellpadding="5" width="100%" style="font-size:12px; padding:0;">
                            <tbody>
                                <tr>
                                    <td colspan="2" style="background-color:#808080; color:#fff"><img style="vertical-align:middle" src="<?= BASE_PATH . SYSTEM_IMAGE_DIR . 'flight.png' ?>"> &nbsp;<span style="vertical-align:middle;font-size:13px"><?php echo $Return; ?> Flight Details</span></td>
                                    <td align="right" colspan="2" style="background-color:#808080; color:#fff"><span style="font-size:9px">*Please verify flight times with the airlines prior to departure</span></td>
                                </tr>
                                <?php
                                if (isset($booking_transaction_details) && $booking_transaction_details != "") {
                                    $checkin_baggage = 0;
                ...
                <tr>
                    <td colspan="4" style="border-bottom:1px solid #808080;padding-bottom:15px"><span style="font-size:13px; color:#555;">Customer Contact Details | E-mail : <?= $booking_details['email'] ?> | Contact No : <?= $booking_details['phone'] ?></span></td>
                </tr>
                <tr>
                    <td style="line-height:15px;padding:0;">&nbsp;</td>
                </tr>
        ...
        <table class="no-print" id="printOption" onclick="getPrint2();" style="border-collapse: collapse;font-size: 14px; margin: 10px auto; font-family: arial;" width="70%" cellpadding="0" cellspacing="0" border="0">
            <tbody>
                <tr>
                    <td align="center"><input style="background:#808080; height:34px; padding:0px 10px; border-radius:4px; border:none; color:#fff; margin:0 2px;" type="button" value="Print" />
                </tr>
...
<script type="text/javascript">
    function getPrint() {
        ...
        html += "</head><body>";
        html += document.getElementById('tickect_flightonword').innerHTML;
        html += "</body></html>";
    ...
    function getPrint2() {
        ...
        html += "<style>@media print { .no-print { display: none !important; } }</style>";
        html += "</head><body>";
        html += document.getElementById('tickect_flightreturn').innerHTML;
        html += "</body></html>";
        var printWin = window.open();
        printWin.document.write(html);
        printWin.document.close();
