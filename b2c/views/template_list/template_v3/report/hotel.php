<?php 
$pagination = '<div class="float-end">'.$GLOBALS['CI']->pagination->create_links().'</div>';
?>
					...
					<div class="trvlwrap">
						...
						if(valid_array($table_data['booking_details']) == true) {
							...
							foreach($booking_details as $parent_k => $parent_v) { 
								extract($parent_v);
								if($parent_v['status'] != 'BOOKING_INPROGRESS') {
							?>
								<div class="full_bkingg">
									<div class="bookrow">
										...
										<div class="remful">
											<div class="xlbook col-sm-2 col-3 htlimage_xl">
												<div class="imghtlflt" style="background:url('<?=$GLOBALS['CI']->template->template_images()?>default_hotel_img.jpg') no-repeat;background-size: 100% 100%;">
													<img src="<?=$hotel_image?>" />
												</div>
											</div>
											<div class="xlbook col-sm col-9 htlimage_xl ">
												<div class="htlfltr">
											...
											<div class="xlbook col-3 bordbor">
												<div class="sideprice">
...
{
	$action_tab  = '';
	$ticket_details = '<a href="'.base_url().'voucher/hotel/'.$app_reference.'/'.$booking_source.'" target="_blank" class="viwedetsb">View</a>';
	$ticket_pdf = '<a href="'.base_url().'voucher/hotel/'.$app_reference.'/'.$booking_source.'/'.$status.'/show_pdf" target="_blank" class="viwedetsb">pdf</a>';
	$action_tab .= $ticket_details.$ticket_pdf;
	 if($status == 'BOOKING_CONFIRMED') {
		$cancel_booking = '<a href="'.base_url().'hotel/pre_cancellation/'.$app_reference.'/'.$booking_source.'" class="viwedetsb">Cancel</a>';
	...
	<script>
		$(document).ready(function(){
			...
			$('.print_ticket').click(function () {
				...
				$.get(app_base_url+controller_method+'/get_hotel_voucher?reference_id='+reference_id+'&app_reference='+app_reference+'&status='+status, function (response) {
			...
			$('.print_invoice').click(function () {
				...
				var status = $(this).data('status');
				var controller_method = 'report';
				$.get(app_base_url+controller_method+'/get_hotel_invoice?reference_id='+reference_id+'&app_reference='+app_reference+'&status='+status, function (response) {
			...
			$('#send_mail_btn').click(function (){
				...
				if(user_email_id !='') {
					...
					$.get(app_base_url+controller_method+'/'+app_reference+'/'+reference_id+'/'+status+'/'+user_email_id, function (response) {
		  ...
		  $(document).on('submit', "form[name='shareemail-form']", function(e){
		     ...
		     $("#loading").removeClass("hide");
		      $.ajax({
		          url: app_base_url+'voucher/email_ticket',
		      	  type: "POST",
		          dataType: 'json',
