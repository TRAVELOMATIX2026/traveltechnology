...
{
	return '<a href="'.car_voucher_url($app_reference, $booking_source, $status).'/show_pdf" target="_blank" class="btn btn-sm btn-primary pdf_btn"><i class="far fa-file"></i> Pdf</a>';
}
...
function confirm_hotel_booking_url($app_reference, $booking_source='', $status='')
{
	return base_url().'hotel/confirm_hotel_booking/'.$app_reference.'/'.$booking_source;
}


function confirm_hotel_booking($app_reference, $booking_source='', $status='',$module='')
{
	return '<a href="'.confirm_hotel_booking_url($app_reference, $booking_source, $status).'" target="_blank" class="btn btn-sm btn-success"><i class="far fa-file"></i> Confirm Booking</a>';
}
...
{
	return '<a href="'.hotel_pdf_url($app_reference, $booking_source, $status).'/show_pdf" target="_blank" class="btn btn-sm btn-primary pdf_btn"><i class="far fa-file"></i> Pdf</a>';
}
...
{
	return base_url().'hotel/pre_cancellation/'.$app_reference.'/'.$booking_source.'/'.$status;
}
...
function sightseeing_voucher_url($app_reference, $booking_source='', $status='',$module='')
{
	...
	elseif($module == 'b2b'){
		...
		return base_url().'index.php/voucher/sightseeing/'.$app_reference.'/'.$booking_source.'/'.$status;
	}
...
function sightseeing_pdf($app_reference, $booking_source='', $status='',$module='')
{
	...
	return '<a href="'.sightseeing_pdf_url($app_reference, $booking_source, $status,$module).'/show_pdf" target="_blank" class="btn btn-sm btn-primary  pdf_btn"><i class="far fa-file"></i> Pdf</a>';
}
...
{
	// echo $status;exit;
	return '<a href="'.flight_voucher_url($app_reference, $booking_source, $status, $module).'/show_pdf" target="_blank" class="btn btn-sm btn-primary pdf_btn"><i class="far fa-file"></i> Pdf</a>';
}
...
{
    return '<a href="'.base_url().'index.php/voucher/flight_invoice_GST/'.$app_reference.'/'.$booking_source.'/'.$status.'/'.$module.'" target="_blank" class="btn btn-sm btn-primary invoice_btn"><i class="far fa-file"></i> Invoice</a>';
	//return base_url().'index.php/voucher/flight_invoice/'.$app_reference.'/'.$booking_source.'/'.$status;
}

function hotel_GST_Invoice($app_reference, $booking_source='', $status='',$module='')
{
    return '<a href="'.base_url().'index.php/voucher/hotel_invoice_GST/'.$app_reference.'/'.$booking_source.'/'.$status.'/'.$module.'" target="_blank" class="btn btn-sm btn-primary invoice_btn"><i class="far fa-file"></i> Invoice</a>';
...
{
    return '<a href="'.base_url().'index.php/voucher/activity_invoice_GST/'.$app_reference.'/'.$booking_source.'/'.$status.'/'.$module.'" target="_blank" class="btn btn-sm btn-primary invoice_btn"><i class="far fa-file"></i> Invoice</a>';
...
function check_run_ticket_method($app_reference ,$booking_source, $status,$is_domestic,$travel_date,$amount=0)
{
	$today_date = date('Y-m-d H:i:s');
	if($status == 'BOOKING_HOLD' && $today_date<=$travel_date) {
		return '<a data-app-reference="'.$app_reference.'" data-price="'.$amount.'" data-booking-source="'.$booking_source.'"data-booking-status="'.$status.'" class="btn btn-sm btn-warning  issue_hold_ticket"><i class="far fa-moniter"></i> Confirm Ticket</a>';
	}
}
...
function get_dynamic_booking_parameters($room_index, $room_details, $application_default_currency)
{
	...
	//$conversion_amount_arr = $GLOBALS ['CI']->domain_management_model->get_currency_conversion_rate(admin_base_currency ());

	//$conversion_amount = $conversion_amount_arr['conversion_rate'];
	
	$dynamic_params_url = array();
	$dynamic_params_url['RoomIndex']				= $room_index;
	$dynamic_params_url['RatePlanCode']				= $room_details['RatePlanCode'];
	$dynamic_params_url['RateKey']				= $room_details['rateKey'];
	...
	$dynamic_params_url['RoomPrice']				= roundoff_number($room_details['API_raw_price']['RoomPrice']);
	$dynamic_params_url['PublishedPrice']			= roundoff_number($room_details['API_raw_price']['PublishedPrice']);
	$dynamic_params_url['PublishedPriceRoundedOff']	= roundoff_number($room_details['API_raw_price']['PublishedPriceRoundedOff']);
	$dynamic_params_url['OfferedPrice']				= roundoff_number($room_details['API_raw_price']['OfferedPrice']);
	$dynamic_params_url['OfferedPriceRoundedOff']	= roundoff_number($room_details['API_raw_price']['OfferedPriceRoundedOff']);
	...
	$dynamic_params_url['ServiceTax']				= roundoff_number($room_details['API_raw_price']['ServiceTax']);
	$dynamic_params_url['Tax']						= roundoff_number(@$room_details['API_raw_price']['Tax']);
	$dynamic_params_url['ExtraGuestCharge']			= roundoff_number(@$room_details['API_raw_price']['ExtraGuestCharge']);
	$dynamic_params_url['ChildCharge']				= roundoff_number(@$room_details['API_raw_price']['ChildCharge']);
	$dynamic_params_url['OtherCharges']				= roundoff_number(@$room_details['API_raw_price']['OtherCharges']);
	$dynamic_params_url['Discount']					= roundoff_number(@$room_details['API_raw_price']['Discount']);
	$dynamic_params_url['AgentCommission']			= roundoff_number(@$room_details['API_raw_price']['AgentCommission']);
	$dynamic_params_url['AgentMarkUp']				= roundoff_number(@$room_details['API_raw_price']['AgentMarkUp']);
	$dynamic_params_url['TDS']						= roundoff_number(@$room_details['API_raw_price']['TDS']);
...
function tbo_summary_room_combination($HotelRoomsDetails)
{
	...
	foreach ($HotelRoomsDetails as $__h_key => $__h_val) {
		...
		$summary['DestinationTax']				+= @$__h_val['Price']['DestinationTax'];
		$summary['DestinationTaxAPI']			+= @$__h_val['Price']['DestinationTaxAPI'];
		$summary['DisplayTax']					+= @$__h_val['Price']['DisplayTax'];
...
{
	$hotel_name_filter = '
	<div class="panel card border-primary contrast10 clearfix">
		<div class="card-header"><i class="far fa-filter"></i> <span class="bg-primary">Filter By Hotel Name</span></div>
		<div class="card-body">
	...
	$price_filter = '
	<div class="panel card border-primary contrast10 clearfix">
		<div class="card-header"><i class="far fa-money"></i> <span class="bg-primary">Filter By Price</span></div>
		<div class="card-body">
			<p>
	...
	$rating_filter = '
	<div class="panel card border-primary contrast10 clearfix">
		<div class="card-header"><i class="far fa-star"></i> <span class="bg-primary">Filter By Rating</span></div>
		<div class="card-body">
			<ul class="">
