...
$(document).ready(function() {
	...
	$(document).on('click','.view-photo-btn',function(e){
		...
		var booking_url =app_base_url+'hotel/hotel_details/'+
   ...
   "stylers1": [
	...
	function get_map_attr_obj_new(thisRef)
    {
        ...
        var booking_url = '';
        booking_url = app_base_url+'hotel/hotel_details/'+
  ...
  function set_marker_visibility_old(thisRef)
    ...
    function create_marker(obj, visibility)
    {
        ...
        '</div>'+
        '</div>'+
        '<div class="col-4 nopad"> <a class="book_map" href="'+obj['details_url']+'">Book</a> </div>'+
        '</div>'+
        '</div>'+
        '</div>';
