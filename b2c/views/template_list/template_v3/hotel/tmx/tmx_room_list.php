...
foreach ($_RoomCombinations['RoomCombination'] as $__rc_key => $__rc_value) {
	...
	foreach ($current_combination_wrapper as $__room_index_key => $__room_index_value) {
		...
		if(isset($temp_room_details['LastCancellationDate'])==true&&empty($temp_room_details['LastCancellationDate'])==false){
			if(date('Y-m-d',strtotime($temp_room_details['LastCancellationDate'])) !=date('Y-m-d') && date('Y-m-d',strtotime($temp_room_details['LastCancellationDate'])) > date('Y-m-d')){
				$today_cancel_date=0;
				$room_panel_details .=' <span class="romtypefrecan"><i class="bi bi-check-circle" aria-hidden="true"></i> Free Cancellation<p> till '.local_month_date($temp_room_details['LastCancellationDate']).'</p></span>';
			}else{
				//echo $temp_room_details['LastCancellationDate'];
				$today_cancel_date=1;
		...
		data-non-refundable="'.$non_refundable.'" data-today-cancel-date="'.$today_cancel_date.'">View Cancellation Policy</a>';
			...
			// debug($temp_room_details['Amenities']);exit;
				if($temp_room_details['Amenities']){
						...
						foreach ($temp_room_details['Amenities'] as $a_key=>$__amenity) {
							...
							if($a_key <=1){
								// debug($__amenity);
								$room_panel_details .='<span class="noof_ave" data-bs-toggle="tooltip" data-title="'.$__amenity.'" data-placement="right"><i class="bi bi-check-circle" aria-hidden="true"></i>'.$__amenity.'</span>';
							}				
						}
						if(count($temp_room_details['Amenities'])>2){
							...
							foreach ($temp_room_details['Amenities'] as $m_key => $m_value) {
								...
								if($m_key>=2){
									// echo $m_value;
									$room_panel_details .='<span class="noof_ave" data-bs-toggle="tooltip" data-title="'.$m_value.'" data-placement="right"><i class="bi bi-check-circle" aria-hidden="true"></i>'.$m_value.'</span>';
								}
		...
		if(empty($temp_room_details['Amenities'])==true && empty($temp_room_details['OtherAmennities'])==true){
			$room_panel_details .='<span class="noof_ave"><i class="bi bi-check-circle" aria-hidden="true"></i>'.ucfirst($temp_room_details['room_only']).'</span>';
		}
		$room_panel_details .='</div>';
		$room_panel_details .='</div>';
	...
	$temp_book_link = '<div class="col-sm-4 col-12 nopad full_mobile">
	...
	</div><div class="col-sm-4 col-12 nopad full_mobile">
...
<script>
	...
	$(".cancel-policy-btn").on("click",function(){
			...
			$.ajax({
				url:"<?php echo base_url()?>"+'ajax/get_cancellation_policy',		
				data:{
