...
$common_params_url .= '<input type="hidden" name="HotelName"		value="'.$HotelName.'">';
$common_params_url .= '<input type="hidden" name="StarRating"		value="'.$StarRating.'">';
$common_params_url .= '<input type="hidden" name="HotelImage"		value="'.$HotelImage.'">';//Balu A
$common_params_url .= '<input type="hidden" name="HotelAddress"		value="'.$HotelAddress.'">';//Balu A
...
<?php 
	//---------------------------------------------------------------------------Print Combination - START
foreach ($_HotelRoomsDetails as $__rc_key => $__rc_value) {
	/**
	...
	foreach ($current_combination_wrapper as $__room_index_key => $__room_index_value) {
		...
		$room_panel_details .='<div class="col-sm-4 col-6 nopad full_mobile">';
		$room_panel_details .='<div class="romsfst">';
		$room_panel_details .=' <span class="romtypestd"><i class="bi bi-door-open"></i> '.ucfirst(strtolower($temp_room_details['RoomTypeName'])).'</span>';
		
		// Room Size/Area if available
		if(isset($temp_room_details['RoomSize']) && !empty($temp_room_details['RoomSize'])){
			$room_panel_details .='<span class="room-size-info"><i class="bi bi-arrows-angle-expand"></i> '.$temp_room_details['RoomSize'].' sqft</span>';
		} elseif(isset($temp_room_details['Area']) && !empty($temp_room_details['Area'])){
			$room_panel_details .='<span class="room-size-info"><i class="bi bi-arrows-angle-expand"></i> '.$temp_room_details['Area'].' sqft</span>';
		} elseif(isset($temp_room_details['SquareFeet']) && !empty($temp_room_details['SquareFeet'])){
			$room_panel_details .='<span class="room-size-info"><i class="bi bi-arrows-angle-expand"></i> '.$temp_room_details['SquareFeet'].' sqft</span>';
		} elseif(isset($temp_room_details['RoomArea']) && !empty($temp_room_details['RoomArea'])){
			$room_panel_details .='<span class="room-size-info"><i class="bi bi-arrows-angle-expand"></i> '.$temp_room_details['RoomArea'].' sqft</span>';
		}
		...
		if($adult_count>1){
			...
			<i class="bi bi-people" aria-hidden="true"></i> '.$adult_count.' <span class="mobile_hide">adults</span></span>';
		}else{
			$room_panel_details .='  <span class="noof_adult"><i class="bi bi-person" aria-hidden="true"></i> '.$adult_count.' <span class="mobile_hide">adult</span></span>';
		}
		if($child_count >0){
			if($child_count>1){
				$room_panel_details .='<span class="noof_adult"><i class="bi bi-emoji-smile" aria-hidden="true"></i> '.$child_count.' <span class="mobile_hide">children</span></span>';
			}else{
				$room_panel_details .='<span class="noof_adult"><i class="bi bi-emoji-smile" aria-hidden="true"></i> '.$child_count.' <span class="mobile_hide">child</span></span>';
			}
		}
		$room_panel_details .='<span class="noof_adult"><i class="bi bi-building" aria-hidden="true"></i> <span class="mobile_hide">Rooms:</span> '.$hotel_search_params['room_count'].'</span>';
		...
		if(isset($temp_room_details['LastCancellationDate'])==true&&empty($temp_room_details['LastCancellationDate'])==false){
			...
			$LastCancellationDate = new DateTime($temp_room_details['LastCancellationDate']);

			$RawCancellationDate = $LastCancellationDate->format('Y-m-d');
			// Format the date
			$FormattedLastCancellationDate = $LastCancellationDate->format('jS F Y H:i A');

			if(date('Y-m-d',strtotime($RawCancellationDate)) !=date('Y-m-d') && date('Y-m-d',strtotime($RawCancellationDate)) > date('Y-m-d')){
				$today_cancel_date=0;
				$room_panel_details .=' <span class="romtypefrecan"><i class="bi bi-check-circle"></i> Free Cancellation<p> till '.local_month_date($temp_room_details['LastCancellationDate']).'</p></span>';
			}else{
				$today_cancel_date=1;
				$room_panel_details .=' <span class="romtypefrecan non-refundable"><i class="bi bi-x-circle"></i> Non-Refundable</span>';
			}
		}
		else{
			$today_cancel_date =1;
		...
		$room_panel_details .='<a id="cancel_'.$hotel_code.'_'.$__rc_key.'" class="shwrmsdv cancel-policy-btn"  data-hotel-code="'.$hotel_code.'" data-key="'.$__rc_key.'" data-bs-target="#roomCancelModal" data-bs-toggle="modal" 
		data-cancellation-policy="'.@$temp_room_details['RoomUniqueId'].'"
		data-policy-code="'.@$temp_room_details['cancellation_policy_code'].'"
		data-rate-key="'.$temp_room_details['HotelChain'].'"
		...
		data-tb-search-id="'.$hotel_search_params['search_id'].'"
		...
		if (isset($temp_room_details['Amenities'])) {
			// debug($temp_room_details['Amenities']);exit;
				if($temp_room_details['Amenities']){
						$view_more_btn ='<a data-bs-toggle="collapse" data-bs-target="#datalst_'.$hotel_code.'_'.$__rc_key.'"><span class="noof_view"><i class="fa fa-angle-double-down" aria-hidden="true"></i>&nbsp;View more</span></a>';
						foreach ($temp_room_details['Amenities'] as $a_key=>$__amenity) {
							//echo $__amenity.'<br/>';
							if($a_key <=1){
								// debug($__amenity);
								$room_panel_details .='<span class="noof_ave" data-bs-toggle="tooltip" data-title="'.$__amenity.'" data-placement="right"><i class="bi bi-check"></i> '.$__amenity.'</span>';
							}				
						}
						if(count($temp_room_details['Amenities'])>2){
							...
							foreach ($temp_room_details['Amenities'] as $m_key => $m_value) {
								...
								if($m_key>=2){
									// echo $m_value;
									$room_panel_details .='<span class="noof_ave" data-bs-toggle="tooltip" data-title="'.$m_value.'" data-placement="right"><i class="bi bi-check"></i> '.$m_value.'</span>';
								}
		...
		if (isset($temp_room_details['OtherAmennities'])) {
				if($temp_room_details['OtherAmennities']){
					...
					$view_more_btn ='<a data-bs-toggle="collapse" data-bs-target="#datalst_o_'.$hotel_code.'_'.$__rc_key.'"><span class="noof_view"><i class="fa fa-angle-double-down" aria-hidden="true"></i>&nbsp;View more</span></a>';
		...
		if(empty($temp_room_details['Amenities'])==true && empty($temp_room_details['OtherAmennities'])==true && empty($temp_room_details['RateType'])== false){
			$room_panel_details .='<span class="noof_ave"><i class="bi bi-check"></i> '.ucfirst($temp_room_details['AccommodationType']).'</span>';
		}

		if(isset($temp_room_details['MealPlans']) && is_array($temp_room_details['MealPlans'])==true)
		{
			foreach($temp_room_details['MealPlans'] as $key=>$values)
			{
				$check_data = $values==true?'check':'times';
				$room_panel_details .='<span class="noof_ave"><i class="fa fa-'.$check_data.'" aria-hidden="true"></i>'.ucfirst($key).'</span>';
			}
		}
	...
	$temp_book_link = '<div class="col-sm-6 col-12 nopad full_mobile">
							<div class="romsfst mobile_bg price-section">
	...
	<span class="romtyprice"><i class="" aria-hidden="true"></i>&nbsp;'.$currency_obj->get_currency_symbol($currency_obj->to_currency).' '.$RoomPrice.'<p class="ninenyt">( '.$no_of_nights.' '.$night_str.' )</p></span>
		...
		<div class="col-sm-6 col-12 nopad full_mobile">
							<div class="romsfst mobile_bg button-section">
							<form method="POST" action="'.$booking_url.'">
	'.$common_params_url.$temp_dynamic_params_url.'
    ...
    <div class="modal-dialog modal-lg">
      <div class="modal-content">
        ...
        <div class="modal-body">
        	...
        	<div id='can-model' style="text-align: left;">
	...
	$(".cancel-policy-btn").on("click",function(){
			...
			$.ajax({
				...
				data:{
					...
					room_price:Room_price,non_refundable:Non_refundable,booking_source:Booking_Source,tb_search_id:TB_search_id,currency_symbol:Currency_symbol,policy_details:Policy_details,today_cancel_date:Today_Cancel,hotel_code:Hotel_code
				},
