...
foreach ($private_trip_indicator_group as $__tirp_indicator => $__trip_flights) {
	...
	foreach ($__trip_flights as $__trip_flight_k => $__trip_flight_v) {
		...
		$inner_summary .= '<div class="row"><div class="col-md-12"><h5 class="text-info text-center">Fare Rules <i class="fa fa-money"></i><i class="fa fa-arrow-circle-down float-end"></i></h5></div></div>';
		...
		$inner_summary .= '<div class="row"><div class="col-md-12"><h5 class="text-info text-center">Flight Details <i class="fa fa-plane"></i><i class="fa fa-arrow-circle-down float-end"></i></h5></div></div>';
			$inner_summary .= '<div class="row">';//summary wrapper start
				...
				$inner_summary .= '<div class="' . add_special_class('col-md-4', 'col-md-3', $domestic_round_way_flight) . '">';//price summary start
					$inner_summary .= '<table class="table table-responsive table-sm m-0">';
            ...
            $inner_summary .= '<tbody>';
              $inner_summary .= '<tr class="info">';
                ...
                $inner_summary .= '<td class="text-end"><span class="curr_icon">'.$cur_Currency.'</span> '.number_format($o_BaseFare).'</td>';
              $inner_summary .= '</tr>';
              $inner_summary .= '<tr class="info">';
                $inner_summary .= '<th>Taxes &amp; Fees</th>';
                $inner_summary .= '<td class="text-end"><span class="curr_icon">'.$cur_Currency.'</span> '.number_format($o_Total_Tax).'</td>';
              $inner_summary .= '</tr>';
              $inner_summary .= '<tr class="success">';
                $inner_summary .= '<th>Grand Total</th>';
                $inner_summary .= '<td class="text-end"><span class="curr_icon">'.$cur_Currency.'</span> '.number_format($o_Total_Fare).'</td>';
              $inner_summary .= '</tr>';
						$inner_summary .= '</tbody>';
  				$inner_summary .= '</table>';
		...
		$outer_summary .= '<div class="f-s-d-w ' . add_special_class('col-md-8', 'col-md-9', $domestic_round_way_flight) . '">';
			...
			foreach ($cur_WSSegment as $__segment_k => $__segment_v) {
				...
				$outer_summary .= '<div class="outer-segment-'.$__segment_k.' row rowclear flightRstPad">';
					//airline
					$outer_summary .= '<div class="col-sm-6 p-tb-10 text-center col-3 ' . add_special_class('col-md-4', 'col-md-3', $domestic_round_way_flight) . '">
					...
					//depart
					$outer_summary .= '<div class="col-md-3 col-3 col-sm-6 p-tb-10 txtwrapRow">
					...
					//arrival
					$outer_summary .= '<div class="col-md-3 col-3 col-sm-6 p-tb-10 noPL txtwrapRow">
					...
					$outer_summary .= '<div class="col-sm-6 p-tb-10 col-6 ' . add_special_class('col-md-2', 'col-md-1', $domestic_round_way_flight) . '"><span class="f-d hide">'.$total_segment_travel_duration.'</span><h5>'.get_time_duration_label($total_segment_travel_duration).'</h5> <h5 class="stop-value" data-val="'.($__stop_count).'">Stop:'.($__stop_count).'</h5></div>';
					$outer_summary .= '<div class="clearfix visible-sm-block"></div>';
				$outer_summary .= '</div>';
			}
		$outer_summary .= '</div><div class="clearfix visible-xs-block"></div>';
		$outer_summary .= '
					<div class="price-details-summary-wrapper mobRht ' . add_special_class('col-md-3' , 'col-3', $domestic_round_way_flight) . '">
						...
						<div class="row row-no-gutter">
							...
							<div class="mobHide col-md-3">
								<div role="group" class="btn-group btn-group-vertical ' . add_special_class('btn-group-xs', '', $domestic_round_way_flight) . '">
									<button class="btn btn-secondary b-r-0 send-quotation-details" type="button"><i class="fa fa-envelope"></i></button>
									<button class="btn btn-secondary b-r-0 iti-fare-btn" type="button"><i class="fa fa-money"></i></button>
									<button class="btn btn-secondary b-r-0 iti-btn" type="button">
		...
		$flights .= '
			<div class="card-body text-center p-0 r-r-i t-w-i-'.$__tirp_indicator.'">
			'.$outer_summary.'
			'.$inner_summary.'
			</div>
		';
