...
class Utilities extends MY_Controller {
	...
	public function xeMultiCurrency(){
		$this->CI = &get_instance();
        $this->CI->load->library('Api_Interface','', 'apiInterface');

		$data = $this->custom_db->single_table_records('currency_converter', '*', array('status' => ACTIVE));
		if($data['status'] == SUCCESS_STATUS) {
			foreach ($data['data'] as $val) {
				$curRequest['curFrom'] = UNIVERSAL_DEFAULT_CURRENCY;
				$curRequest['curTo'] = $val['country'];
				$curRequest['amount'] = 1;
				$response = $this->apiInterface->xeMultiCurrency($curRequest);
				$response = json_decode($response, true);
				if (!empty($response)) {
					if ($response['to'][0]['quotecurrency'] == $val['country'] ){
						$updateData['value'] = $response['to'][0]['mid'];
						$updateData['date_time'] = date('Y-m-d h:m:s');
						$this->custom_db->update_record('currency_converter', $updateData, array('id' => $val['id']));
						echo 'currecny updated successfully for '.$val['country'].'<br>';
					}
				}
			}
		}
