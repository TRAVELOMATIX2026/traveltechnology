...
class User extends MY_Controller {
	...
	function profile()
	{
		...
		$promo = $this->discountOffers();
		// debug($promo);exit;
		$page_data['promo'] = $promo['promo'];
		$page_data['promocodeTrackerData'] = $promo['promocodeTrackerData'];
		// $page_data['currency_obj'] = $promo['currency_obj'];
	...
	function update_traveller_details()
	{
		...
		&& isset($post_data['traveller_email']) == true && isset($post_data['traveller_last_name']) == true) {
			...
			if($post_data['passport_issuing_country']==''){
				$post_data['passport_issuing_country'] = 0;
			}
	...
	function discountOffers()
	{
		validate_user_login();
		// echo $this->entity_user_id;exit;
		// echo $this->discount_coupon_origin;exit;
		if ($this->discount_coupon_origin == '') {
			// echo "string";exit;
			$data['promo'] = [];
			$data['status'] = false;
		} else {
			// $result = $this->db->query('SELECT * FROM promo_code_list WHERE module="new_register" AND expiry_date >= CURDATE() AND origin='.$this->discount_coupon_origin)->result_array();
			$result = $this->db->query('SELECT * FROM user AS A INNER JOIN promo_code_list AS B ON A.discount_coupon_origin = B.origin WHERE A.discount_coupon_origin='.$this->discount_coupon_origin. ' AND A.user_id='.$this->entity_user_id)->result_array();
			$promocodeTrackerData = $this->db->query("SELECT * FROM new_user_promocode_track WHERE status = 1 And user_id=".$this->entity_user_id)->result_array();
			// debug($promocodeTrackerData);exit;
			$data['currency_obj'] = $currency_obj;
			$data['promo'] = $result[0];
			$data['promocodeTrackerData'] = $promocodeTrackerData;
			$data['status'] = true;
		}
		// debug($data);exit;
		return $data;
		
		// $this->template->view('user/discount_offers', $data);
	}
}
