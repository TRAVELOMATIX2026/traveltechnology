...
class Ajax extends MY_Controller
{
     ...
     */
    function index() {}
    public function update_flight_lat_long()
    {
        $__airports = $this->flight_model->get_airports_lat_long();
        // print_r($__airports); exit;
        foreach ($__airports as $airport) {
            $jsons = json_decode($airport->airport_json);
            foreach ($jsons as $json) {
                if ($json->iata_code != '') {
                    $this->flight_model->update_airport_lat_long($json->iata_code, $json->latitude, $json->longitude);
                }
            }
        }
        // update_airport_lat_long();
        // get_airports_list_lat_long();
    }
    /**
            ...
            if ($airport->lat != '' && $airport->lon != '') {
                ...
                if (!empty($__near_airports)) {
                    foreach ($__near_airports as $near_airports) {
                        if ($near_airports->lat != '' && $near_airports->lat != null) {
    ...
    public function send_multi_flight_details_mail(){
      ...
      //  $dd['flightdetails']=$cc;
        ...
        // debug($params['data_access_key']);exit;
        $email=$params['email'];
                    ...
                    if ($raw_sightseeing_result['status']) {
                        ...
                        $response['data'] = get_compressed_output(
                            $this->template->isolated_view('sightseeing/viator/viator_search_result', array(
                                ...
                                'raw_sightseeing_list' => $raw_sightseeing_result['data'],
            ...
            switch ($search_params['booking_source']) {
                case PROVAB_HOTEL_BOOKING_SOURCE:
                    ...
                    if ($raw_hotel_list['status']) {
                        ...
                        $response['data'] = get_compressed_output(
                            $this->template->isolated_view('hotel/search_result', array(
                                'currency_obj' => $currency_obj,
            ...
            switch ($post_params['booking_source']) {
                case PROVAB_HOTEL_BOOKING_SOURCE:
                    ...
                    if ($raw_hotel_images['status'] == true) {
                        ...
                        $response['data'] = get_compressed_output(
                            $this->template->isolated_view('hotel/tmx/tmx_hotel_images', array(
     ...
     * Load hotels from different source
     */
    function transfer_list_old($search_id = '')
    {
        //echo $search_id; die;
        $response['data'] = array();
        $response['msg'] = array();
     ...
     public function transfer_list($offset=0){      
        $search_params = $this->input->get();
         //debug($search_params);
        // exit;
        $safe_search_data = $this->transfer_model->get_safe_search_data($search_params['search_id']);
            ...
            switch($search_params['booking_source']) {
                ...
                case PROVAB_TRANSFER_BOOKING_SOURCE :
                    ...
                    if ($raw_transfer_result['status']) {
                        ...
                        $raw_transfer_result = $this->transfer_lib->search_data_in_preferred_currency($raw_transfer_result, $currency_obj,'b2c');
                       ...
                       //Update currency and filter summary appended
                        ...
                        $raw_transfer_result['data'] = $this->transfer_lib->format_search_response($raw_transfer_result['data'], $currency_obj, $search_params['search_id'], 'b2c', $filters);
    ...
    function flight_list($search_id = '')
    {
        ...
        try {
            ...
            if ($search_params['op'] == 'load' && intval($search_params['search_id']) > 0 && isset($search_params['booking_source']) == true) {
                // Load the flight library
                if (!function_exists('load_flight_lib')) {
                    throw new Exception('load_flight_lib function not found');
                }
                
                load_flight_lib($search_params['booking_source']);
                
                // Verify library was loaded
                if (!isset($this->flight_lib)) {
                    throw new Exception('Flight library failed to load for source: ' . $search_params['booking_source']);
                }
                ...
                case PROVAB_FLIGHT_BOOKING_SOURCE:
                    ...
                    // $json = json_encode($raw_flight_list, JSON_FORCE_OBJECT | JSON_PRETTY_PRINT);
                    // file_put_contents('logs/raw_flight_list_tmx.txt',  $json);
                    if ($raw_flight_list['status']) {
                        ...
                        // $json = json_encode($raw_search_result, JSON_FORCE_OBJECT | JSON_PRETTY_PRINT);
                        // file_put_contents('logs/raw_search_result_tmx.txt',  $json);
                        //Display
                        ...
                        if (($route_count > 0)) { //later enable above condition
                            ...
                            $page_params['module'] = 'b2c';
                            // if(1 || !$domestic_round_way_flight){
                            ...
                            $response['status'] = SUCCESS_STATUS;
                            $response['total_count'] = count($raw_flight_list['data']['Flights'][0]);
                            /*
                              session expiry start time and search hash
                             */
                            $response['session_expiry_details'] = $formatted_search_data['session_expiry_details'];
                        } else {
                            // No flights found - return empty result structure
                            $page_params = array(
                                'raw_flight_list' => array('Flights' => array(), 'JourneySummary' => array('IsDomestic' => false)),
                                'search_id' => $search_params['search_id'],
                                'booking_url' => '',
                                'booking_source' => $search_params['booking_source'],
                                'cabin_class' => '',
                                ...
                                'session_expiry_details' => array(),
                                'logo_path' => SYSTEM_IMAGE_DIR,
                                'module' => 'b2c'
                            );
                            echo json_encode($page_params);exit;
                        }
                    } else {
                        // Flight list status is false - return error structure
                        $page_params = array(
                            'raw_flight_list' => array('Flights' => array(), 'JourneySummary' => array('IsDomestic' => false)),
                            'search_id' => $search_params['search_id'],
                            'booking_url' => '',
                            ...
                            'logo_path' => SYSTEM_IMAGE_DIR,
                            'module' => 'b2c',
                            'error' => isset($raw_flight_list['msg']) ? $raw_flight_list['msg'] : 'No flights found'
                        );
                        echo json_encode($page_params);exit;
                    }
                    break;
            ...
            log_message('error', 'Flight List AJAX Error: ' . $e->getMessage());
            $response['status'] = FAILURE_STATUS;
            $response['msg'] = 'An error occurred while loading flight list. Please try again.';
            $response['error'] = $e->getMessage();
            $response['file'] = $e->getFile();
            $response['line'] = $e->getLine();
        } catch (Error $e) {
            // Catch PHP 7+ fatal errors
            log_message('error', 'Flight List AJAX Fatal Error: ' . $e->getMessage());
            $response['status'] = FAILURE_STATUS;
            $response['msg'] = 'A fatal error occurred while loading flight list.';
            $response['error'] = $e->getMessage();
            $response['file'] = $e->getFile();
            $response['line'] = $e->getLine();
        }
        ...
        // log_custom('Ajax:flight_list:output_done');
    }
    /** 
    ...
    function puls_minus_days_fare_list($booking_source)
    {
        ...
        try {
            ...
            if (!isset($params['search_id']) || intval($params['search_id']) <= 0) {
                throw new Exception('Invalid search_id parameter');
            }
            ...
            // Verify library was loaded
            if (!isset($this->flight_lib)) {
                throw new Exception('Flight library failed to load for source: ' . $booking_source);
            }
            ...
            if ($search_data['status'] == SUCCESS_STATUS) {
                ...
                // Fix: $departure_date is a timestamp, not an array. Use the first date from $date_array instead
                $first_date = $date_array[0];
                $date_array[] = date('Y-m', strtotime($first_date . ' +1 month')) . '-1';
            ...
            // Log the error for debugging
            log_message('error', 'Puls Minus Days Fare List AJAX Error: ' . $e->getMessage());
            $response['status'] = FAILURE_STATUS;
            $response['msg'] = 'An error occurred while loading fare list. Please try again.';
            $response['error'] = $e->getMessage();
            $response['file'] = $e->getFile();
            $response['line'] = $e->getLine();
        } catch (Error $e) {
            // Catch PHP 7+ fatal errors
            log_message('error', 'Puls Minus Days Fare List AJAX Fatal Error: ' . $e->getMessage());
            $response['status'] = FAILURE_STATUS;
            $response['msg'] = 'A fatal error occurred while loading fare list.';
            $response['error'] = $e->getMessage();
            $response['file'] = $e->getFile();
            $response['line'] = $e->getLine();
        }
