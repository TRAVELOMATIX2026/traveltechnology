...
class Auth extends MY_Controller
{
		...
		if (is_logged_in_user() == false) {
			...
			if (valid_array($op_data) == true) {
				...
				if ($this->form_validation->run()) {
					//Create New User
					$creation = $this->user_model->create_user($op_data['email'], $op_data['password'], $op_data['country_code'], $op_data['first_name'], $op_data['phone']);
	...
	function register_guest_user()
	{
		...
		if (is_logged_in_user() == false && empty($post_data['username']) == false && empty($post_data['mobile_number']) == false) {
			...
			if ($user_exists == false) { //Check User Exists based on Username
				...
				$user_type = 0;
				$creation = $this->user_model->create_user($user_name, $password, $first_name, 0, $mobile_number, $creation_source, $user_type);
	...
	function forgot_password()
	{
		...
		$condition['user_type'] = B2C_USER;
		$user_record = $this->custom_db->single_table_records('user', 'email, password, user_id, first_name, last_name, image', $condition);
		...
		if (is_logged_in_user() == false) {
			...
			switch ((string)strtolower($domain_name)) {
				case 'google':
					...
					$existing_user = $this->user_model->get_user_details($cond); //debug($existing_user);
					//new user
					if (valid_array($existing_user) == false) {
						$new_user = $this->user_model->create_user($email, 'password', $first_name, '', '0', 'google', '');
						if ($new_user && !empty($new_user['data'])) { //debug($new_user);
							$existing_user = $this->user_model->get_user_details(array('U.user_id', '=', $new_user['data'][0]['user_id']));
						}
					} //exit;
					break;
				case 'facebook':
					...
					//new user
					if (valid_array($existing_user) == false) {
						$new_user = $this->user_model->create_user($email, 'password', $first_name, '', '0', 'facebook', '');
						if ($new_user && !empty($new_user['data'])) {
							debug($new_user);
							$existing_user = $this->user_model->get_user_details(array('U.user_id', '=', $new_user['data'][0]['user_id']));
							//debug($existing_user);
						}
