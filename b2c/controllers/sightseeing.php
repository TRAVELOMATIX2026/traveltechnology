...
class Sightseeing extends MY_Controller
{
	...
	function sightseeing_details()
	{
		...
		if (isset($params['booking_source']) == true) {
			...
			if (
				$params['booking_source'] == PROVAB_SIGHTSEEN_BOOKING_SOURCE && isset($params['result_token']) == true and isset($params['op']) == true and
	...
	public function enable_calendar_availability($calendar_availability_date)
	{
		...
		if (valid_array($calendar_availability_date)) {
			...
			foreach ($calendar_availability_date as $m_key => $m_value) {
				$available_str[] = date('j-n-Y', strtotime($m_value));
	 ...
	 */
	public function enable_calendar_availability_old($calendar_availability_date)
	{
		...
		if (trim($post_params['product_code']) != '') {
			...
			if (trim($post_params['op'] == 'check_tourgrade') && $post_params['product_code'] != '') {
				...
				if ($search_product_tourgrade['status']) {
					...
					//debug($search_product_tourgrade['data']);exit;
					$response['data'] = get_compressed_output($this->template->isolated_view(
						'sightseeing/viator/select_tourgrade',
						array(
							'tourgrade_list' => $search_product_tourgrade['data'],
	...
	function booking()
	{
		...
		if (isset($pre_booking_params['booking_source']) == true) {
			...
			if (
				$pre_booking_params['booking_source'] == PROVAB_SIGHTSEEN_BOOKING_SOURCE && isset($pre_booking_params['tour_uniq_id']) == true and
				...
				if ($pre_booking_params['tour_uniq_id'] != false) {
					...
					if ($trip_block_details['status'] == SUCCESS_STATUS) {
						...
						@$promo_codes = $this->custom_db->get_custom_query("select * from  promo_code_list where status=1 and module in ('activities') AND expiry_date >= CURDATE()");
						$page_data['promo_codes'] = @$promo_codes;
						//debug($page_data['promo_codes']);exit;
	...
	function pre_booking($search_id)
	{
		...
		if ($valid_temp_token != false) {
			...
			$data = $this->sightseeing_lib->save_booking($book_id, $book_params, $currency_obj, $this->current_module);
		...
		if ($domain_balance_status) {
			...
			if ($temp_booking != false) {
				...
				if ($booking['status'] == SUCCESS_STATUS) {
					...
					$data = $this->sightseeing_lib->update_booking_details($book_id, $booking['data'], $ticket_details, $currency_obj, $this->current_module);
					//debug($data);exit;
