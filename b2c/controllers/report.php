...
class Report extends MY_Controller
{
	...
	function bus_booking_details()
	{
		...
		if (
			valid_array($get_data) == true && empty($get_data['status']) == false &&
	...
	*/
	function transfers($offset=0){
		...
		$table_data = $this->booking_data_formatter->format_transfer_booking_data($table_data, 'b2c');
		$page_data['table_data'] = $table_data['data'];
		/** TABLE PAGINATION */
		$this->load->library('pagination');
		if (count($_GET) > 0) $config['suffix'] = '?' . http_build_query($_GET, '', "&");
		$config['base_url'] = base_url().'index.php/report/transfer/';
		$config['first_url'] = $config['base_url'].'?'.http_build_query($_GET);
		$page_data['total_rows'] = $config['total_rows'] = $total_records;
		$config['per_page'] = RECORDS_RANGE_2;
		$this->pagination->initialize($config);
		...
		$page_data['total_records'] = $config['total_rows'];
		$page_data['customer_email'] = $this->entity_email;
		$this->template->view('report/transfer', $page_data);
	}
	...
	function print_voucher()
	{
		...
		if (isset($post_data) && valid_array($post_data)) {
			if (empty($post_data['pnr_number']) == false) {
				...
				if ($post_data['module'] == PROVAB_CAR_BOOKING_SOURCE) {
					$booking_details = $this->car_model->booking_guest_user($post_data['pnr_number'], $post_data['module'], $booking_status);
