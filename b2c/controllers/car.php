...
class Car extends MY_Controller
{
	...
	function car_details($search_id)
	{
		...
		if (isset($params['booking_source']) == true) {
			...
			if (
				$params['booking_source'] == PROVAB_CAR_BOOKING_SOURCE && isset($params['ResultIndex']) == true and isset($params['op']) == true and
				...
				$page_data['promo_codes'] = $this->custom_db->get_custom_query("select * from  promo_code_list where status=1 and module in ('car') AND expiry_date >= CURDATE()");
				...
				if (isset($raw_car_details['status']) && $raw_car_details['status'] == true) {
					...
					if (isset($car_rate_result) && valid_array($car_rate_result)) {
						...
						$policy_text = '';
						if (isset($raw_car_rate_result['CancellationPolicy']) && valid_array($raw_car_rate_result['CancellationPolicy']) && !empty($raw_car_rate_result['CancellationPolicy'])) {
							$policy_text .= '<p class="policy_text">';
							foreach ($raw_car_rate_result['CancellationPolicy'] as $policy) {
						...
						$cancellationPolicy['policy_text'] = $policy_text;
						$cancellationPolicy['non_ref'] = $non_ref;
						...
						$this->template->view('car/car_booking_page', array('currency_obj' => $currency_obj, 'car_rules' => $car_rate_result, 'car_search_params' => $safe_search_data['data'], 'active_booking_source' => $params['booking_source'], 'params' => $params, 'page_data' => $page_data, 'cancellationPolicy' => $cancellationPolicy));
					} else {
	...
	function pre_booking($search_id)
	{
		...
		if ($valid_temp_token != false) {
			...
			//$post_params['token']['token'] = $this->car_lib->convert_token_to_application_currency($post_params['token']['token'], $currency_obj, $this->current_module);
			$post_params['token'] = serialized_data($post_params['token']);
			$temp_token = unserialized_data($post_params['token']);
			$temp_token['default_currency'] = $get_application_currency_preference; //admin_base_currency ();
			...
			$amount = round($temp_token['TotalCharge']['EstimatedTotalAmount'], 2);
			...
			$data = $this->car_lib->save_booking($book_id, $book_params, $currency_obj, $this->current_module);
			//debug($data);exit;
		...
		if ($domain_balance_status) {
			if ($temp_booking != false) {
				...
				if ($booking['status'] == SUCCESS_STATUS) {
					...
					$data = $this->car_lib->update_booking_details($book_id, $booking['data'], $ticket_details, $currency_obj, $this->current_module);
