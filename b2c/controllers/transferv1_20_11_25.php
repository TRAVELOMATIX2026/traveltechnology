	...
	**/
				...
				if ($pre_booking_params['tour_uniq_id'] != false) {
					...
					if ($trip_block_details['status'] == SUCCESS_STATUS) {
						...
						$page_data['phone_code'] =$temp_record['data'];
						 $page_data['promo_codes'] = $this->custom_db->get_custom_query("select * from  promo_code_list where status=1 and module in ('transfers','new_register')");
	...
	function pre_booking($search_id)
	{
		...
		if ($valid_temp_token != false) {
			...
			$data = $this->transferv1_lib->save_booking($book_id, $book_params, $currency_obj, $this->current_module);
			//debug($data);exit;
		...
		if ($domain_balance_status) {
			...
			if ($temp_booking != false) {
				...
				if ($booking['status'] == SUCCESS_STATUS) {
					...
					$data = $this->transferv1_lib->update_booking_details($book_id, $booking ['data'], $ticket_details,$currency_obj,$this->current_module);
					//debug($data);exit;
