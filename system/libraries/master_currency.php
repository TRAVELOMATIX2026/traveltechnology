...
class Master_currency {
    ...
    public $override_conversion_rate;

    public $markup_multiplier;
    ...
    public function currency_conversion_value($override_conversion_rate = false, $from = '', $to = '') {
        ...
        if ($override_conversion_rate == true || empty($this->conversion_cache[$from . $to]) == true) {
            ...
            if ($from != $to) {
                ...
                ///custom ex.com with currecny table
                // $get = $this->getUpdatedCurrency($from_Currency, $to_Currency);
    ...
    function getUpdatedCurrency($from_Currency, $toCurrency) {



        //  if($from_Currency == $toCurrency ){

        //     $conval['currency_value'] = 1;

        //     return json_encode($conval);
        ...
        $cond = 'country = ' . $CI->db->escape($from_Currency);

        $result = $CI->db->query('SELECT * FROM currency_converter WHERE ' . $cond);

        $from_currency_data = $result ? $result->result_array() : array();



        $cond1 = 'country = '. $CI->db->escape($toCurrency);

        $to_currency_data = $CI->db->query('SELECT * FROM currency_converter WHERE ' . $cond1)->result_array();



        $val = 1;

        if(!empty($from_currency_data) && !empty($to_currency_data)){

            $val = ($to_currency_data[0]['value'] / $from_currency_data[0]['value']);

        }



        $conval['currency_value'] = number_format($val, 5, '.', '');



        /* $CI = &get_instance();

        $cond = 'country = ' . $CI->db->escape($toCurrency);

        $currency_data = $CI->db->query('SELECT * FROM currency_converter WHERE ' . $cond)->result_array();

        $val = 1;

        if(!empty($currency_data)){

            $val = $currency_data[0]['value'];

        }

        $conval['currency_value'] = $val;
    ...
    function force_currency_conversion($value, $from = '', $to = '') {
     ...
     # echo $from.$to;exit;
        ...
        return array('default_value' => (floatval($value) * ($this->conversion_cache[$from . $to])), 'default_currency' => $to);
    ...
    protected function add_markup($value, $markup_multiplier = 1, $parent_markup = false, $seller_markup = false, $specific_markup_config = array()) {
