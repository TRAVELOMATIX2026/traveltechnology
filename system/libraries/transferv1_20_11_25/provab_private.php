...
class Provab_private extends Common_Api_Grind {
	...
	*/
	...
	function save_booking($app_booking_id, $book_params, $currency_obj, $module = 'b2c'){
		...
		$promo_currency_obj = @$book_params['token']['promo_currency_obj'];
		...
		if(isset($book_params ['token']['search_params'])){
			...
			$additonal_info = json_decode(base64_decode($book_params ['token'] ['additional_info']),true);
			...
			for ($i = 0; $i < $total_pax_count; $i++){
				...
				$GLOBALS ['CI']->transferv1_model->save_booking_pax_details ( $title,$app_reference, $first_name,$last_name, $phone_number, $email, $passenger_type,$status, serialize ( $attributes ) );
	...
	public function update_booking_details($book_id, $book_params, $ticket_details,$currency_obj,$module = 'b2c'){
        ...
        $saved_booking_data = $GLOBALS['CI']->transferv1_model->get_booking_details($book_id);
        ...
        $GLOBALS['CI']->custom_db->update_record('transferv1_booking_details', array('status' => $master_transaction_status,'booking_id' => $booking_id,'booking_reference' => $BookingRefNo,'confirmation_reference' => $ConfirmationNo), array('app_reference' => $app_reference));
		...
		$GLOBALS['CI']->custom_db->update_record('transferv1_booking_itinerary_details', array('status' => $master_transaction_status), array('app_reference' => $app_reference));

		$GLOBALS['CI']->custom_db->update_record('transferv1_booking_pax_details', array('status' => $master_transaction_status), array('app_reference' => $app_reference));
