	...
	public function send_mail($to_email, $mail_subject, $mail_message, $attachment='', $cc=array(), $bcc=array())
	{
		...
		if ($this->mailer_status == true) 
		{
			if ($to_email != '' && $mail_message != '' && $mail_subject != '') 
			{
			    ...
			    $mail->Host       = isset($this->mail_configuration['host']) == true ? $this->mail_configuration['host'] : "smtp.office365.com"; // SMTP server
			    $mail->SMTPAuth   = true;
			    $mail->SMTPSecure = 'tls';
			    ...
			    $mail->FromName   = 'Kapido';
			    $mail->From   = $this->mail_configuration['from'];
