...
class Provab_private extends Common_Api_Grind {
	...
	private function hotel_search_request($search_params, $source_id) {
		...
		$request ['hotel_version'] = 2;
		$request ['search_type'] = 'city_search';
		 if(PROVAB_HOTEL_BOOKING_SOURCE != $source_id){
	...
	*/
	...
	function save_booking($app_booking_id, $book_params, $currency_obj, $module = 'b2c'){
        ...
        $book_total_fare = $book_params['token'] ['price_summary'] ['OfferedPriceRoundedOff']; // (TAX+ROOM PRICE)
        $room_price = $book_params['token'] ['price_summary'] ['RoomPrice'];

        $total_fare_markup = roundoff_number($book_params['token']['markup_price_summary']['RoomPrice']);

        $total_fare_tax = roundoff_number($book_params['token']['markup_price_summary']['Tax']);

       // debug($total_fare_markup); exit;

        //$total_destination_charge = roundoff_number($params['booking_params']['token']['markup_price_summary']['DestinationTax']);
	...
	*/
	...
	function booking_url($search_id) {
		return base_url () . 'hotel/booking/' . intval ( $search_id );
	}
	/**
	 * Balu A
	 * 
	...
	function format_search_response($hl, $cobj, $sid, $module = 'b2c', $fltr = array()) {
		...
		foreach ( $hl ['HotelSearchResult'] ['HotelResults'] as $hr => $hd ) {
            ...
            if(count($hd['HotelAmenities'])==0)
            {
            	$hd['HotelAmenities'] = [
           "Wi-fi","Car park",
           "Gym","Breakfast and dinner","Laundry service","Lift access"];
            }

			// markup
			//debug( $hd ['Price']);
			$hd ['Price'] = $this->update_search_markup_currency_one_night ( $hd ['Price'], $cobj, $sid, $level_one, $current_domain );
			$hd['booking_source']       = PROVAB_HOTEL_BOOKING_SOURCE;
	...
	*/
	...
	function filter_summary($hl) {
		...
		// Validate structure
		if (
			!isset($hl['HotelSearchResult']['HotelResults']) ||
			!is_array($hl['HotelSearchResult']['HotelResults'])
		) {
			return ['hotel_count' => 0, 'data' => []];
		}

		$h_count = 0;
		$filt = [
			'p' => ['max' => false, 'min' => false],
			'loc' => [],
			'star' => [],
			'fac' => []
		];
		...
		foreach ($hl['HotelSearchResult']['HotelResults'] as $hr => $hd) {
			...
			$HotelLocation = !empty($hd['HotelLocation']) ? $hd['HotelLocation'] : 'Others';
			$HotelFacilities = isset($hd['HotelAmenities']) ? $hd['HotelAmenities'] : [];

			// ✅ Handle Hotel Facilities safely (can be array or string)
			if (is_array($HotelFacilities)) {
				foreach ($HotelFacilities as $key => $val) {
					if (is_array($val) && !empty($val['name'])) {
						$filt['fac'][$val['name']]['c'] = $key;
						$filt['fac'][$val['name']]['v'] = ucwords($val['name']);
					} elseif (is_string($val) && !empty($val)) {
						$filt['fac'][$val]['c'] = $key;
						$filt['fac'][$val]['v'] = ucwords($val);
					}
				}
			} elseif (is_string($HotelFacilities) && !empty($HotelFacilities)) {
				$facilities = array_map('trim', explode(',', $HotelFacilities));
			...
			// ✅ Star rating filters
			if (!isset($filt['star'][$StarRating])) {
				$filt['star'][$StarRating] = ['c' => 1, 'v' => $StarRating];
			} else {
				$filt['star'][$StarRating]['c']++;
			}

			// ✅ Price filters (check if array exists)
			if (isset($hd['Price']['RoomPrice'])) {
				$price = floatval($hd['Price']['RoomPrice']);
				if ($filt['p']['max'] === false || $price > $filt['p']['max']) {
					$filt['p']['max'] = roundoff_number($price);
				}
				if ($filt['p']['min'] === false || $price < $filt['p']['min']) {
					$filt['p']['min'] = roundoff_number($price);
				}
			}

			// ✅ Location filters
			$hloc = ucfirst(strtolower($HotelLocation));
			if (!isset($filt['loc'][$hloc])) {
				$filt['loc'][$hloc] = ['c' => 1, 'v' => $hloc];
			} else {
				$filt['loc'][$hloc]['c']++;
			}
	...
	function filter_summary_old($hl) {
		//debug($hl); exit;
		...
		foreach ( $hl ['HotelSearchResult'] ['HotelResults'] as $hr => $hd ) {
			...
			$HotelFacilities = $hd ['HotelAmenities'];
