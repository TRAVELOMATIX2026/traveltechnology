 ...
 * @version    V1
 */
class Tbo_private extends Common_Api_Grind {
    ...
    public function __construct() {
        ...
        $this->booking_source = TBO_FLIGHT_BOOKING_SOURCE;
        $this->end_user_ip = $_SERVER['SERVER_ADDR'];
        ...
        if($flight_engine_system=="live")
        {
              $this->Url = "https://api.tboair.com/InternalAirService.svc/rest/";
        }else{
              $this->Url = "https://apistaging.tboair.com/InternalAirService.svc/rest/";
         }
        
    }

    public function set_api_session_id($authentication_response = NULL)
    {
        ...
        if (empty($this->api_session_id) == true) {
            if (empty($authentication_response) == false) {
                ...
                if (valid_array($authentication_response) == true && $authentication_response['IsSuccess'] == true && isset($authentication_response['TokenId']) == true && empty($authentication_response['TokenId']) == false) {
                    $session_id = trim($authentication_response['TokenId']);

                    //debug($this->booking_source); exit();

                    $this->update_api_session_id($this->booking_source, $session_id);
                }
            } else {

                $session_expiry_time = 60; //In minutes

                $session_id = $this->get_api_session_id($this->booking_source, $session_expiry_time);
               ...
               // debug($session_id); exit;
                ...
                if (empty($session_id) == true) {
                    ...
                    if ($authentication_request['status'] == SUCCESS_STATUS) 
                    {
                        ...
                        $authentication_response = $this->CI->api_interface->get_json_response_tbo_flight($authentication_request['url'], $authentication_request['request'],$authentication_request['remarks']);
    ...
    public function get_authentication_request($internal_request = false)
    {
        ...
        $response['message'] = array(); // Message to be returned
    ...
    private function authenticate_request()
    {
        ...
        $AuthenticationRequest['BookingMode'] = "API";
        $AuthenticationRequest['UserName'] = "Starlegends";
        $AuthenticationRequest['Password'] = "Sta@84675895";
        $AuthenticationRequest['IPAddress'] = $_SERVER['REMOTE_ADDR'];

        $request['request'] = json_encode($AuthenticationRequest);

        if($this->system=="live")
        {
            $request['url'] = 'https://tboair.techmaster.in/API/API/v1/Authenticate/ValidateAgency/';
        }
        else
        {
            $request['url'] = 'https://tboairdemo.techmaster.in/API/API/v1/Authenticate/ValidateAgency/';
        }
    ...
    {
        $session_id_details = $this->CI->db->query('select session_id from booking_source where source_id="'.$booking_source.'" and (last_updated_datetime + INTERVAL '.intval($session_expiry_time).' MINUTE) >= "'.db_current_datetime().'"')->row_array();
    ...
    {
        $this->CI->custom_db->update_record('booking_source', array('session_id' => trim($session_id), 'last_updated_datetime' => db_current_datetime()), array('source_id' => $booking_source));
    }
    ...
    public function form_curl_params($request, $url)
    {
    ...
    function flight_search_request($search_params) {
        ...
        $cabinClass = $this->get_cabin_class($search_params['v_class']);
        $isReturn = ($search_params['type'] === 'Return');

        foreach ($search_params['from'] as $i => $origin) {
            $segments[] = [
                'Origin' => $origin,
                'Destination' => $search_params['to'][$i],
                'FlightCabinClass' => $cabinClass,
                'PreferredDepartureTime' => $search_params['depature'][$i],
                'PreferredArrivalTime' => $search_params['depature'][$i],
            ];

            if ($isReturn) {
                $segments[] = [
                    'Origin' => $search_params['to'][$i],
                    'Destination' => $search_params['from'][$i],
                    'FlightCabinClass' => $cabinClass,
                    'PreferredDepartureTime' => $search_params['return'][$i],
                    'PreferredArrivalTime' => $search_params['return'][$i],
                ];
            }
        }
        ...
        $request_params['BookingMode'] = "5";
        $request_params['DirectFlight'] = "false";
        $request_params['OneStopFlight'] = "false";
        $request_params['JourneyType'] = $this->get_journey_type($search_params['type']);
        $request_params['PreferredAirlines'] = array($search_params['carrier']);
        $request_params['Sources'] = [];
        

        $request_params['EndUserIp'] = $this->end_user_ip;
        $request_params['TokenId'] = $this->api_session_id;

        $request_params['Segments'] = $segments;
        $request_params['ResultFareType'] = "0";
        $request_params['PreferredCurrency'] = "USD";
        //debug($request_params);exit;
        $response['data']['request'] = json_encode($request_params);
        $response['data']['service_url'] = $this->service_url;


        //debug(json_encode($request_params)); exit;

        return $response;
    }

    function get_cabin_class($cabinClassName)
    {

        switch (strtolower($cabinClassName)) {
            case 'economy':
                $cabinClassCode = 2;
                break;
    ...
    function fare_details_request($data_key, $search_key) {
        ...
        $response['data'] = array();
         ...
         $flight_data = reset(unserialized_data($search_key));

         //debug($flight_data); exit;

        $this->credentials('FareRule');
        if (empty($flight_data['ResultIndex']) == false && empty($flight_data['TraceId']) == false) {

            $fare_rule_request['EndUserIp'] = $this->end_user_ip;
            $fare_rule_request['TokenId'] = $this->api_session_id;
            $fare_rule_request['TraceId'] = $flight_data['TraceId'];
            $fare_rule_request['ResultIndex'] = $flight_data['ResultIndex'];
        } else {
            $response['status'] = FAILURE_STATUS;
        }
        $response['data']['request'] = json_encode($fare_rule_request);
        $response['data']['service_url'] = $this->service_url;
        return $response;
    }
    ...
    function fare_quote_request($data_key, $search_key) {
        ...
        if (empty($flight_data['ResultIndex']) == false && empty($flight_data['TraceId']) == false) {
            $request_params['EndUserIp'] = $this->end_user_ip;
            $request_params['TokenId'] = $this->api_session_id;
            $request_params['TraceId'] = $flight_data['TraceId'];
            $request_params['ResultIndex'] = $flight_data['ResultIndex'];
        } else {
            $response['status'] = FAILURE_STATUS;
        }
        $response['data']['request'] = json_encode($request_params);
    ...
    private function commit_booking_request($booking_params, $app_reference) {
        ...
        $fare_quote_params = unserialized_data($booking_params['fare_quote_params']);

        // debug($fare_quote_params); 
        // exit;  

        $request_params = [];

        // Basic Booking Info
        $request_params['ResultId'] = $fare_quote_params['ResultIndex'];
        $request_params['IPAddress'] = "52.60.180.183";
        $request_params['TokenId'] = $this->api_session_id;
        $request_params['EndUserBrowserAgent'] = $_SERVER['HTTP_USER_AGENT'];
        $request_params['PointOfSale'] = "IN";
        $request_params['RequestOrigin'] = "India-https://www.tboair.com";
        $request_params['UserData'] = $booking_params['Passenger'][0]['Email'].",".$booking_params['Passenger'][0]['Title']." ".$booking_params['Passenger'][0]['FirstName']." ".$booking_params['Passenger'][0]['LastName'];

        $Segments_BE = $fare_quote_params['Segments'][0];
        $FareRules = $fare_quote_params['FareRules'];

        $MiniFareRules = $fare_quote_params['MiniFareRules'];

        $Passengers = array();

        foreach($booking_params['Passenger'] as $key => $passenger)
        {

            if(isset($passenger['SeatId'][0]) && is_array($passenger['SeatId']))
            {
                $PassengerSeat = unserialized_data($passenger['SeatId'][0]);

                if(isset($PassengerSeat[0]) && is_array($PassengerSeat))
                {
                    $Passengers[$key]['Seat'] = $PassengerSeat[0]['Code']; 
                }  
            }

            if(isset($passenger['MealId'][0]) && is_array($passenger['MealId']))
            {
                $PassengerMeal = unserialized_data($passenger['MealId'][0]);

                if(isset($PassengerMeal[0]) && is_array($PassengerMeal))
                {
                    $Passengers[$key]['Meal'] = array('Code'=>$PassengerMeal[0]['Code'],'Description'=>$PassengerMeal[0]['AirlineDescription']); 
                }  
            }


            if(isset($passenger['BaggageId'][0]) && is_array($passenger['BaggageId']))
            {
                $PassengerBaggage = unserialized_data($passenger['BaggageId'][0]);

                if(isset($PassengerBaggage[0]) && is_array($PassengerBaggage))
                {
                    $Passengers[$key]['Baggage'] = array('Code'=>$PassengerBaggage[0]['Code'],'Description'=>$PassengerBaggage[0]['Description']); 
                }  
            }
           ...
           $Passengers[$key] = array(
                        ...
                        'DateOfBirth' => $passenger['DateOfBirth']."T00:00:00",
                        'Email' => $passenger['Email'],
                        'Mobile1' => $passenger['ContactNo'],
                        'Mobile1CountryCode' => str_replace('+','',$passenger['PhoneCountryCode']),
                        ...
                        "FFAirline"=> $passenger['frequent_flyer_airline'],
                        "FFNumber"=> $passenger['frequent_flyer_no'],
                        "PaxKey"=> strtoupper($passenger['FirstName'].$passenger['LastName']),
                        'Nationality' => [
                        ...
                        'Fare_BE' => $fare_quote_params['FareBreakdown'][$key]
                    );
             }

       $request_params['Itinerary'] = [
       ...
       // 'Destination' => 'DXB',
        'LastTicketDate' => $fare_quote_params['LastTicketDate'],
        //'Origin' => 'DEL',
        'ValidatingAirlineCode' => $fare_quote_params['AirlineCode'],
        'IsDomestic' => $fare_quote_params['IsLCC'],
        'NonRefundable' => $fare_quote_params['IsRefundable'],
    ];
        ...
        */
        ...
        //debug(json_encode($request_params,true)); exit();

        $response['data']['request'] = json_encode($request_params,true);

        $response['data']['service_url'] = "https://tboairdemo.techmaster.in/API/API/v1/Booking/Book/";
    ...
    function get_flight_list($search_id = '') {
        ...
        $is_domestic_roundway = $search_data['status'] && $search_data['data']['is_domestic'] && $search_data['data']['is_roundtrip'] ? 1:0;
        ...
        if ($search_data['status'] == true) {
            if ($cache_search === false || ($cache_search === true && empty($cache_contents) == true)) {
                ...
                if ($search_request['status']) {
                    ...
                    $search_response_data = array();

                    $curl_request = $this->form_curl_params($search_request['data']['request'], $search_request['data']['service_url']);

                   // debug($curl_request); exit;

                    $search_response = $this->CI->api_interface->get_json_response_tbo_flight($curl_request['data']['url'], $curl_request['data']['request'],"Search");

                    //debug($search_response); exit;

                    //$search_response = file_get_contents('logs/flight/searchRS.json');

                    $search_response_data = json_decode($search_response, true);

                    //debug($search_response_data); exit;


                    if ($this->valid_api_response($search_response_data) ) 
                    {
                        //$search_response = $this->format_search_response_api();

                        $clean_format_data = $this->format_search_data_response($search_response_data, $search_data ['data']);

                        //debug($clean_format_data); exit;

                        $response['data']['Search'] = $clean_format_data;
    ...
    function get_fare_details($data_row, $search_session_key) {
        ...
        if ($api_request['status']) {
          ...
          $fare_rule_response = array();

            $api_response = $this->CI->api_interface->get_json_response_tbo_flight($api_request['data']['service_url'], $api_request['data']['request'], "FareRule");

            //$api_response = file_get_contents('logs/flight/fareRuleRS.json');

            $fare_rule_response = json_decode($api_response, true);

            if ($this->valid_api_response($fare_rule_response)) {

                $response['data'] = $this->format_fare_rule_response($fare_rule_response);
                $response['status'] = SUCCESS_STATUS;
            }
        }
        return $response;
     ...
     */
     ...
     private function format_api_extra_services($extra_services, $search_id, $IndexType) {
        ...
        $MealPreference = $this->format_meals_preference_details($extra_services, $search_data, $IndexType);

        //SeatPreference
        $SeatPreference = $this->format_seat_preference_details($extra_services, $search_data, $IndexType);
            ...
            foreach ($Baggage as $bag_k => $bag_v) {
                ...
                foreach ($bag_v as $bd_k => $bd_v) {
                    ...
                    if (intval($bd_v['Weight']) > 0) {//NotSet = 0;Segment = 1;FullJourney = 2
                            ...
                            $Origin = $bd_v['Origin'];
                        $Destination = $bd_v['Destination'];

                        $outer_index = $Origin . '-' . $Destination;
                        
                         if (isset($baggage_data[$outer_index]) == false) {
                            $inner_index = 0;
                        } else {
                            $inner_index++;
                        }
                        ...
                        $baggage_data[$outer_index][$inner_index]['BaggageId'] = $baggage_id;
                        $baggage_data[$outer_index][$inner_index]['Origin'] = $bd_v['Origin'];
                        $baggage_data[$outer_index][$inner_index]['Destination'] = $bd_v['Destination'];
                        $baggage_data[$outer_index][$inner_index]['Price'] = $bd_v['Price'];
                        $baggage_data[$outer_index][$inner_index]['Weight'] = $bd_v['Weight'] . 'Kg';
                        $baggage_data[$outer_index][$inner_index]['Code'] = $bd_v['Code'];
     ...
     * @param unknown_type $extra_service_details
     */
    private function format_meals_preference_details($extra_service_details, $search_data, $IndexType) {
        $meals_details = array();
        $meals_details['status'] = FAILURE_STATUS;
        $meals_details['message'] = '';
        $meals_details['data'] = array();
        ...
        if (isset($extra_service_details['Meal']) == true && valid_array($extra_service_details['Meal']) == true) {
            ...
            foreach ($MealPreference as $md_k => $md_v) {
                ...
                if($IndexType==1) {
                     ...
                     $meals_data[0][$md_k]['Origin'] = $Destination;
                     $meals_data[0][$md_k]['Destination'] = $Origin;
                }
     ...
     * @param unknown_type $extra_service_details
     */
    private function format_seat_preference_details($extra_service_details, $search_data, $IndexType) {
        $seat_details = array();
        $seat_details['status'] = FAILURE_STATUS;
        $seat_details['message'] = '';
        $seat_details['data'] = array();
        ...
        if (isset($extra_service_details['SeatPreference']) == true && valid_array($extra_service_details['SeatPreference']) == true) {
            ...
            foreach ($SeatPreference as $sd_k => $sd_v) {
                ...
                $seat_data[0][$sd_k]['Destination'] = $Destination;
                      ...
                      $seat_data[0][$sd_k]['Origin'] = $Destination;
                      $seat_data[0][$sd_k]['Destination'] = $Origin;
                }
    ...
    function format_search_data_response($search_result, $search_data)
    {
        ...
        foreach ($Results as $result_k => $result_v) {
            ...
            if($search_data['is_roundtrip'] == 1 && count($journey_array_v['Segments'])==1)
            {
               continue;
            }
    ...
    private function flight_segment_summary($journey_array, $journey_number, &$key, $cache_fare_object = false)
    {
        ...
        $MiniFareRules = $journey_array['MiniFareRules'];
        ...
        $ValidatingAirline = $journey_array['ValidatingAirline'];
        ...
        foreach ($segments as $k => $v) {
            ...
            foreach ($legs as $l_k => $l_v) {
                ...
                if (($maximum_value - 1) == $l_k) {
                    $AccumulatedDuration = $l_v['AccumulatedDuration'];
                }
                ...
                $details[$k][] = $this->format_summary_array($origin_airline_name, $destination_airline_name, $journey_number, $origin_code, $destination_code, $departure_dt, $arrival_dt, $operator_code, $operator_name, $flight_number, $no_of_stops, $cabin_class, '', '', $total_duration, $is_leg, $attr, $stop_over, $org_terminal, $AccumulatedDuration, '', $dest_terminal, '', '', '', $ValidatingAirline);
                $flightNumberList[] = $l_v['Airline']['FlightNumber'];
                $is_leg = false;
            }
        }

        //debug($itineray_price); exit();

        //Fare
        $price = $this->format_itineray_price_details($itineray_price);
        $MiniFareRules1 = $this->format_minirules_details($MiniFareRules);

        $flight_details['Details'] = $details;
        $flight_ar['FlightDetails'] = $flight_details;
        $flight_ar['Price'] = $price;
        $flight_ar['tboMiniFareRules'] = $MiniFareRules1;
    ...
    private function format_minirules_details($MiniFareRules)
    {
        // debug($MiniFareRules);exit;
        foreach ($MiniFareRules as $key => $value) {
            //debug($value);exit;
            $miniRules = $value;
        }
        // debug($miniRules);exit;
        return $miniRules;
    }

    protected function format_summary_array($origin_airline_name, $destination_airline_name, $journey_number, $origin_code, $destination_code, $departure_dt, $arrival_dt, $operator_code, $operator_name, $flight_number, $no_of_stops, $cabin_class = NULL, $origine_name = NULL, $destination_name = NULL, $duration = NULL, $is_leg = true, $attr = array(), $stop_over = NULL, $org_terminal = NULL, $AccumulatedDuration, $des_terminal = NULL, $carrier_code = NULL, $operated_by_text=NULL, $equipment=NULL,$ValidatingAirline=NULL, $seatremaining=NULL) {
        $CI = & get_instance ();
        // debug($destination_airline_name);exit;
        ...
        $summary_array ['Origin'] = array (
                ...
                'NewAirportname' => $origin_airline_name,
        ...
        $summary_array ['Destination'] = array (
                ...
                'NewAirportname' => $destination_airline_name,
                'DateTime' => $arrival_dt,
        ...
        if(!empty($seatremaining) || $seatremaining==0)
        {
            $summary_array ['seatremaining'] = $seatremaining;    
        }
        
        $summary_array ['CabinClass'] = $cabin_class;
        $summary_array ['Operatedby'] = $operated_by_text;
        $summary_array ['equipment'] = $equipment;
    ...
    {
        //debug($itineray_price['FareBreakdown']); exit;
        ...
        //debug($pax_wise_other_charges); exit;
    ...
    private function run_fare_quote($data_key, $search_access_key) {
        ...
        if ($api_request['status']) {
            ...
            $fare_quote_response = array();

           $api_response = $this->CI->api_interface->get_json_response_tbo_flight($api_request['data']['service_url'], $api_request['data']['request'], "FareQuote");

            //$api_response = file_get_contents('logs/flight/FareQuoteRS.json');

            $fare_quote_response = json_decode($api_response, true);

            // debug($fare_quote_response); 
            // exit;

            if ($this->valid_api_response($fare_quote_response)) {

               $formatted_fare_quote_response  = $this->format_update_fare_quote_response($fare_quote_response['Response'], $search_id);

                $response['data'] = $formatted_fare_quote_response['JourneyList'][0][0];
                $response['data']['fareQuoteOriginal'] = $fare_quote_response['Response']['Results'];
    ...
    function format_update_fare_quote_response($update_fare_quote_response, $search_id) {
        ...
        $search_data_info = $search_data['data'];
    ...
    private function run_extra_services($data_key, $search_access_key, $journey_type) {
        ...
        // debug(unserialized_data($data_key));
        ...
        if ($api_request['status']) {
            ...
            $extra_services_response = array();

            $api_response = $this->CI->api_interface->get_json_response_tbo_flight($api_request['data']['service_url'], $api_request['data']['request'], "ExtraServices");

           // $api_response = file_get_contents('logs/flight/SSR_RS.json');

            $extra_services_response = json_decode($api_response, true);

            // debug($extra_services_response); 
            // exit;

            if ($this->valid_api_response($extra_services_response)) {

                # Identify The domestic one way or roundtrip / Balu A
                if(isset($api_request['request']))
                {
                    $ssr_request1= json_decode($api_request['request']);
                     $ResultIndex=$ssr_request1->ResultIndex;
                     
                      # Index Type 1 for onward and 2 for return journey
                if (strpos($ResultIndex, 'OB') !== false) {
                    $IndexType=1;
                }
                else {
                    $IndexType=2;
                ...
                $formatted_extra_services_response = $this->format_api_extra_services($extra_services_response['Response'], $search_id, $IndexType);

                //debug($formatted_extra_services_response); exit;

                $response['data']['ExtraServiceDetails'] = $this->extra_services_in_preferred_currency($formatted_extra_services_response, $journey_type);
                $response['status'] = SUCCESS_STATUS;
            }
        }
        return $response;
    ...
    function book_flight($book_id, $booking_params) {
        ...
        if (count($unique_search_access_key) == 1) { // Single session is one request
            if (count($token_wrapper['search_access_key']) == 1) {
                ...
                $token_wrapper['token'][0]['fare_quote_params'] = $booking_params['fare_quote_params'];
    ...
    private function valid_flight_booking_status($booking_status) {
        if (in_array($booking_status, array(BOOKING_CONFIRMED, BOOKING_HOLD, BOOKING_PENDING)) == true) {
            return true;
        } else {
            return false;
        }
    ...
    function run_commit_booking($op, $book_id, $token, $passenger, $search_access_key) {
        ...
        $booking_params['fare_quote_params'] = $token['fare_quote_params'];
        $api_request = $this->commit_booking_request($booking_params, $book_id);
           // debug($api_request);die;
        //get data
        if ($api_request['status']) {

            $book_response_data = array();
            $curl_request = $this->form_curl_params($api_request['data']['request'], $api_request['data']['service_url']);

            $api_response = $this->CI->api_interface->get_json_response_tbo_flight($curl_request['data']['url'], $curl_request['data']['request'],"Book");

          // $search_response = file_get_contents('logs/flight/searchRS.json');

            $book_response_data = json_decode($api_response, true);

            //debug($book_response_data); exit;


            if ($this->valid_commit_booking_response($book_response_data) == true) {
                ...
                $response['status'] = $book_response_data['Status'];
            } else {
...
//debug($pax_ticket_ids);exit;
    ...
    private function WSPassenger($passenger) {
        ...
        for ($i = 0; $i < $total_pax_count; $i++) {
            ...
            if (empty($passenger['passport_number'][$i]) == false and empty($passenger['passport_expiry_date'][$i]) == false) {
                ...
                $tmp_passenger[$i]['PassportIssueCountryCode'] = $passenger['passenger_passport_issuing_country'][$i];
            } else {
            ...
            $tmp_passenger[$i]['PhoneCountryCode'] = $passenger['phone_country_code'];
            ...
            if(isset($passenger['frequent_flyer_airline'][$i]) == true && isset($passenger['frequent_flyer_no'][$i]) == true && valid_array($passenger['frequent_flyer_airline'][$i]) == true && valid_array($passenger['frequent_flyer_no'][$i]) == true)
            {
                $tmp_passenger[$i]['frequent_flyer_airline'] = $passenger['frequent_flyer_airline'][$i];
                $tmp_passenger[$i]['frequent_flyer_no'] = $passenger['frequent_flyer_no'][$i];
            }
            else
            {
                $tmp_passenger[$i]['frequent_flyer_airline'] = null;
                $tmp_passenger[$i]['frequent_flyer_no'] = null;
            }
    ...
    private function extract_passenger_info($booking_params, $SequenceNumber) {
        ...
        if(empty($params['frequent_flyer_airline']) == false && is_array($params['frequent_flyer_airline']))
        {
            foreach ($passenger['frequent_flyer_airline'] as $__k => $__v) {
              $passenger['frequent_flyer_airline'][$__k] = $__v;  
            }
        }

        if(empty($params['frequent_flyer_no']) == false && is_array($params['frequent_flyer_no']))
        {
            foreach ($passenger['frequent_flyer_no'] as $__k => $__v) {
              $passenger['frequent_flyer_no'][$__k] =  $__v;  
            }
        }
        ...
        $passenger['phone_country_code'] = $booking_params['phone_country_code'];
    ...
    function valid_api_response($api_response) {
        if (empty($api_response) == false && valid_array($api_response) == true and isset($api_response['Response']['ResponseStatus']) == true and $api_response['Response']['ResponseStatus'] == SUCCESS_STATUS) {
            return true;
        } else {
            return false;
        }
    ...
    private function valid_commit_booking_response($api_response) {
        if (empty($api_response) == false && valid_array($api_response) == true and isset($api_response['Status']) == true and in_array($api_response['Status'], array(SUCCESS_STATUS, BOOKING_CONFIRMED, BOOKING_PENDING, BOOKING_HOLD))) {
            return true;
        } else {
            return false;
        }
    ...
    function tax_service_sum($markup_price_summary, $api_price_summary, $retain_commission = false) {
       ...
       // debug($markup_price_summary);exit;
        ...
        // debug($markup_price);
        // debug($markup_price_summary);
        // debug($commission);
        // debug($commission_tds);
    ...
    function b2b_price_details($api_price_details, $admin_price_details, $agent_price_details, $currency_obj) {
        ...
        $total_price['TotalTax'] = $total_price['_TaxSum'];
        $total_price['_GST'] = $gst_value;
        // debug($total_price);
        // exit;
        return $total_price;
    }
    private function get_gst_amount($total_markup){
        $gst_value = 0;
        if($total_markup > 0 ){
            $gst_details = $GLOBALS['CI']->custom_db->single_table_records('gst_master', '*', array('module' => 'flight'));
            if($gst_details['status'] == true){
     ...
     * return booking form
     */
    function booking_form($isDomestic, $token = '', $token_key = '', $search_access_key = '',  $booking_source = TBO_FLIGHT_BOOKING_SOURCE,$session_id ='',$ShoppingResponseId='') {
        $booking_form = '';
    ...
    public function extract_flight_segment_fare_details($flights, $currency_obj, $search_id, $module, $ins_token = false, & $token = array(), $flight_index = 0) {
        ...
        foreach ($flights as $list_k => $list_v) {
            ...
            $markup_data = $flight_list[$list_k]['FareDetails']['markup_data'];
            ...
            $flight_list[$list_k]['PassengerFareBreakdown'] = $this->add_markup_to_pax($list_v['PassengerFareBreakdown'], $markup_data);
			...
			$flight_list[$list_k]['totalStops'] = $segments['stops_val'];

            $flight_list[$list_k]['booking_source'] = TBO_FLIGHT_BOOKING_SOURCE;
            $flight_list[$list_k]['deptCat'] = self::get_time_category($segments['segment_summary'][0]['OriginDetails']['_DateTime']);
			$flight_list[$list_k]['arrCat'] = self::get_time_category($segments['segment_summary'][0]['DestinationDetails']['_DateTime']);
            //Hold Ticket
	...
	private static function get_time_category($time){
		// Convert the input time to a DateTime object
		$inputTime = new DateTime($time);

		// Define the time boundaries for comparison
		$time1 = new DateTime('06:00:00');
		$time2 = new DateTime('12:00:00');
		$time3 = new DateTime('18:00:00');
	
		// Compare the input time with the defined time ranges and return corresponding category ID
		if ($inputTime < $time1) {
			return 1; // Time is before 06:00:00
		} elseif ($inputTime < $time2) {
			return 2; // Time is before 12:00:00
		} elseif ($inputTime < $time3) {
			return 3; // Time is before 18:00:00
		} else {
			return 4; // Time is 18:00:00 or later
		}
	}
    private function add_markup_to_pax($PassengerFareBreakdown, $markup_data){
        if($PassengerFareBreakdown){
            foreach($PassengerFareBreakdown as $type=>$passenger){
                if($type == 'ADT'){
                    $total_price = $passenger['TotalPrice'];
                    $pax_count = $passenger['PassengerCount'];
                    $currency_obj = new Currency ( array ('module_type' => 'flight','from' => admin_base_currency() ,'to' => get_application_currency_preference ()));
                    $pax_total = ($total_price/$pax_count);
                    $total_markup = 0;
                    if($markup_data){
                        foreach($markup_data as $rec){
                            if($rec['original_markup']){
                                $markup = 0;
                                if($rec['markup_type'] == 'Pecentage'){
                                    $markup = (($pax_total/100) * $rec['original_markup']);
                                }else if($rec['markup_type'] == 'Plus'){
                                    $markup = get_converted_currency_value($currency_obj->force_currency_conversion($rec['original_markup']));
                                }
                                $total_markup += $markup;
                                $pax_total += $markup;
                            }
                    ...
                    $gst_amount = $total_markup ? $this->get_gst_amount($total_markup) : 0;
                    $pax_total = number_format(($pax_total+$gst_amount), 2, '.', '');
                    $PassengerFareBreakdown[$type]['total_price_markup'] = $pax_total;
                }
            }
        }
    ...
    private function get_fare_object($flight_details, $currency_obj, $search_id, $module) {
        ...
        if($flight_details['Attr']['SpecialFare']==true)
        {
            $flight_details['FlightDetails']['Details'][0][0]['Attr']['SpecialFare']=true;
        }
        ...
        if ($module == 'b2c') {
            ...
            $admin_price_details = $this->update_search_markup_currency($flight_details['FareDetails'], $currency_obj, false, true, $search_id, $specific_markup_config,'b2c'); //B2c:DONT CHANGE
             ...
             // debug($flight_details['FareDetails']);
             // exit;
           ...
           // echo $o_Total_Fare;exit;
            ...
            $markup_data[] = array('markup_type'=>$admin_price_details['markup_type'],'original_markup'=>$admin_price_details['original_markup']);
            ...
            $markup_data[] = array('markup_type'=>$agent_price_details['markup_type'],'original_markup'=>$agent_price_details['original_markup']);
...
//            debug($admin_price_details);debug($agent_price_details);exit;
        ...
        $FareDetails['markup_data'] = $markup_data;
        return $FareDetails;
    }
    ...
    private function preferred_currency_fare_object($fare_details, $currency_obj, $default_currency = '') {
        ...
        //commented by Sunil, Sep 12, 2023
       $OfferedFare = ($FareDetails['PublishedFare'] - $FareDetails['AgentCommission']);

        // $OfferedFare = ($published_fare - $agent_commission);
        // $FareDetails['OfferedFare'] = ceil($OfferedFare);
        $FareDetails['OfferedFare'] = number_format($OfferedFare, 2, '.', '');
    ...
    public function preferred_currency_paxwise_breakup_object($fare_details, $currency_obj) {
        ...
        foreach ($fare_details as $k => $v) {
            ...
            $PassengerFareBreakdown[$k]['Tax'] = get_converted_currency_value($currency_obj->force_currency_conversion($PassengerFareBreakdown[$k]['Tax']));
            $PassengerFareBreakdown[$k]['TotalPrice'] = get_converted_currency_value($currency_obj->force_currency_conversion($PassengerFareBreakdown[$k]['TotalPrice']));
        }
        return $PassengerFareBreakdown;
    }
    ...
    public function get_airline_specific_markup_config($segment_details) {
        $specific_markup_config = array();
        $segment_details[0][0]['Attr']['SpecialFare'];
        ...
        // }
        if($segment_details[0][0]['Attr']['SpecialFare']==true) {
            if(isset($segment_details[0][0]['OperatorCode'])){
                $airline_code = $segment_details[0][0]['OperatorCode'].'_SPL';
            }else{
                $airline_code = $segment_details[0][0]['AirlineDetails']['AirlineCode'].'_SPL';
            }
           
        } else if(isset($segment_details[0][0]['OperatorCode'])) {
            $airline_code = $segment_details[0][0]['OperatorCode'];
        } else {
            $airline_code = $segment_details[0][0]['AirlineDetails']['AirlineCode'];
        }
        ...
        foreach ($segment_details as $seg_k => $seg_v) {
            ...
            foreach ($seg_v as $seg_details_k => $seg_details_v) {
                ...
                if (isset($seg_v[$seg_details_k + 1]) == true) {
                    ...
                    $duration_time = calculate_duration($seg_details_v['Destination']['DateTime'], $next_seg_info['Origin']['DateTime']);
                    $WaitingTime = (get_time_duration_label($duration_time));
                }
                $Baggage = '';
                $CabinBaggage = '';
                ...
                if (isset($WaitingTime) == true) {
                    if($seg_count != $seg_details_k+1){
                        ...
                        $hours = floor(($duration_time) / 3600);
						$minutes = floor(($duration_time - ($hours * 3600)) / 60);			
						$segment_full_details[$seg_k][$seg_details_k]['WaitingTimeHM'] = "$hours.$minutes"+0;
                    }
                   
                }
                if($total_stops > 0 && $seg_details_k !=0){
					$Stopdetails[]=array(
						'AirportCode'=>$seg_details_v['Origin']['AirportCode'],
						'CityName'=>$seg_details_v['Origin']['CityName'],
						'AirportName'=>$seg_details_v['Origin']['AirportName']
					);
					// $Stopdetails[$seg_details_k-1]['AirportCode'] = $seg_details_v['Departure']['AirportCode'];
					// $Stopdetails[$seg_details_k-1]['CityName'] = @$cities_data[$seg_details_v['Departure']['AirportCode']];
					// $Stopdetails[$seg_details_k-1]['AirportName'] = $seg_details_v['Departure']['AirportName'];
				}
            }
            $segment_summary[$seg_k]['Stopdetails'] = $Stopdetails;
        }
        // exit;
        $data['segment_summary'] = $segment_summary;
        $data['segment_full_details'] = $segment_full_details;
		$stops = array_column($segment_summary, 'TotalStops');
		$stops = array_unique($stops);
		$data['stops_val'] = count($stops) == 1 ? $stops[0] : min($stops) + 0.5;
        return $data;
    }
  ...
  //       echo "<br/>";
    ...
    public function read_token($token_key) {
        $token_key = explode(DB_SAFE_SEPARATOR, unserialized_data($token_key));//debug($token_key);exit;
    ...
    private function get_final_booking_price_details($Fare, $multiplier, $specific_markup_config, $currency_obj, $deduction_cur_obj, $module) {
       ...
       // echo "core_agent_commision".$core_agent_commision.'<br/>';
        ...
        //commented on Sep 13,2023
     //   $commissionable_fare = $Fare['PublishedFare'];
        $commissionable_fare = $Fare['BaseFare']+$Fare['Tax']+$Fare['AgentCommission'];
    ...
    function save_booking($app_booking_id, $book_params, $currency_obj, $module = 'b2c') {
        ...
        foreach ($token as $token_index => $token_value) {
            ...
            if($token_value['Attr']['SpecialFare']==true)
                {
                    $segment_details[0][0]['Attr']['SpecialFare']=true;
                }
            ...
            $transaction_insert_id = $GLOBALS['CI']->flight_model->save_flight_booking_transaction_details(
                    ...
                    // debug($transaction_insert_id);exit;$trans_total_fare
            ...
            for ($i = 0; $i < $total_pax_count; $i++) {
                ...
                $date_of_birth = @date('Y-m-d' , strtotime($book_params['date_of_birth'][$i]));
            ...
            foreach ($segment_details as $seg_k => $seg_v) {
                ...
                foreach ($seg_v as $ws_key => $ws_val) {
                    ...
                    $GLOBALS['CI']->flight_model->save_flight_booking_itinerary_details(
                            $app_reference, $segment_indicator, $airline_code, $airline_name, $flight_number, $fare_class, $from_airport_code, $from_airport_name, $to_airport_code, $to_airport_name, $departure_datetime, $arrival_datetime, $iti_status, $operating_carrier, json_encode($attributes), $FareRestriction, $FareBasisCode, $FareRuleDetail, $airline_pnr, 
                            $cabin_baggage, $checkin_baggage, $is_refundable);
                }
        ...
        if (valid_array($ticket_details) == true) {
            ...
            foreach ($ticket_details as $ticket_index => $ticket_value) {
                ...
                if ($this->valid_flight_booking_status($ticket_value['Status']) == true) { //IF Ticket is HOLD/CONFIRMED
                    ...
                    if($book_params['token']['token'][0]['Attr']['SpecialFare']==true)
                    {
                        $specialfare = array('SpecialFare' => true);
                        $segment_details[0][0]['Attr']= $specialfare;
                    }
