...
class Provab_private extends Common_Api_Grind {
    ...
    function get_flight_list($search_id = '') {
        ...
        if ($search_data['status'] == true) { // && !$is_domestic_roundway
            if ($cache_search === false || ($cache_search === true && empty($cache_contents) == true)) {
                ...
                //get data
                if ($search_request['status']) {
                    //$this->CI->custom_db->generate_static_response(json_encode($search_request));
                    $search_response = $this->CI->api_interface->get_json_response($search_request['data']['service_url'], $search_request['data']['request'], $header_info);
                    //debug($search_response);exit;
                    //$this->CI->custom_db->generate_static_response(json_encode($search_response));
                    //$search_response = $this->CI->flight_model->get_static_response(552);//336=>oneway;=>domestic roundway;=> Multicity
...
//debug($pax_ticket_ids);exit;
    ...
    public function search_data($search_id) {
        ...
        if (empty($this->master_search_data) == true and valid_array($this->master_search_data) == false) {
            ...
            if ($clean_search_details['status'] == true) {
                ...
                // Fix: Check if carrier is array before imploding, otherwise use as-is
                if (is_array($clean_search_details['data']['carrier'])) {
     ...
     * return booking form
     */
    function booking_form($isDomestic, $token = '', $token_key = '', $search_access_key = '',  $booking_source = PROVAB_FLIGHT_BOOKING_SOURCE,$session_id ='',$ShoppingResponseId='') {
        $booking_form = '';
