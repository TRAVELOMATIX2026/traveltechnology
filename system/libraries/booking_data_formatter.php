...
class Booking_Data_Formatter {
	...
	function format_hotel_booking_data($complete_booking_details, $module)
	{
		...
		foreach($master_booking_details as $book_k => $book_v) {
			...
			$agent_markup = 0;
			$gst = 0;
			if(valid_array($booking_itinerary_details)){
			...
			$adult_count = 0;
			$child_count = 0;
			if(valid_array($booking_customer_details)){
			...
			$core_booking_details['rate_comments'] = @$attributes['RateComments'];
	...
	function format_flight_booking_data($complete_booking_details, $module, $convert_currency = true)
	{
		...
		foreach($master_booking_details as $book_k => $book_v) {
			...
			foreach($booking_transaction_details as $transaction_k => $transaction_v) {
				...
				$gst 				+= floatval(@$transaction_v['gst']);
	...
	private function format_itinerary_details($itinerary_details)
	{
		...
		foreach($itinerary_details as $itinerary_k => $itinerary_v) {
			...
			$duration = 0;
			$attributes = json_decode($itinerary_v['attributes'], true);
			if($attributes && !empty($attributes['ws_val']['SegmentDuration'])) {
				$duration = $this->convertToMinutes($attributes['ws_val']['SegmentDuration']);
			}else{
				if(isset($itinerary_v['from_airport_code'])&&isset($itinerary_v['to_airport_code'])&&isset($itinerary_v['arrival_datetime'])&&isset($itinerary_v['departure_datetime'])){
					$duration = $this->flight_segment_duration($itinerary_v['from_airport_code'], $itinerary_v['to_airport_code'], $itinerary_v['departure_datetime'], $itinerary_v['arrival_datetime']);
				}
			}
	...
	private function convertToMinutes($input){
		...
		if (preg_match('/(\d+)\s*h/', $input, $matches)) {
			// Convert hours to minutes and add to total
			$totalMinutes += $matches[1] * 60;
		}
		
		if (preg_match('/(\d+)\s*m/', $input, $matches)) {
			// Add minutes directly to the total
	...
	private function format_segment_details($itinerary_details)
	{
		...
		foreach($itinerary_details as $itinerary_k => $itinerary_v) {
			...
			if($attributes && !empty($attributes['ws_val']['SegmentDuration'])) {
				...
				$duration =  $this->flight_segment_duration($itinerary_v['from_airport_code'], $itinerary_v['to_airport_code'], $itinerary_v['departure_datetime'], $itinerary_v['arrival_datetime']);			// $itinerary_v['total_duration'] = get_duration_label(calculate_duration($itinerary_v['arrival_datetime'], $itinerary_v['departure_datetime']));
			}
			$duration=$duration*60;
            $itinerary_v['total_duration'] =get_time_duration_label($duration);
	...
	function get_booking_counts()
	{
		...
		$data['bus_booking_count'] = 0;
		//$data['bus_booking_count'] = $GLOBALS['CI']->bus_model->booking($condition, true);
		$data['hotel_booking_count'] = $GLOBALS['CI']->hotel_model->booking_count($condition, true);
		$data['flight_booking_count'] = $GLOBALS['CI']->flight_model->booking_count($condition, true);
		$data['car_booking_count'] = $GLOBALS['CI']->car_model->booking_count($condition, true);
		$data['sightseeing_booking_count'] = $GLOBALS['CI']->sightseeing_model->booking_count($condition, true);

		$data['transfer_booking_count'] = $GLOBALS['CI']->transfer_model->booking_count($condition, true);
	 ...
	 */
	function format_master_transaction_balance($transacion_data = array(),$module = 'b2c'){
	...
	private function flight_segment_duration_old($departure_airport_code, $arrival_airport_code, $departure_datetime, $arrival_datetime)
    {
	...
	private function flight_segment_duration($departure_airport_code, $arrival_airport_code, $departure_datetime, $arrival_datetime)
	{
		...
		$departure_timezone_offset = $this->get_airport_timezone_offset($departure_airport_code, $departure_datetime);
		$arrival_timezone_offset = $this->get_airport_timezone_offset($arrival_airport_code, $arrival_datetime);
		if(!$departure_timezone_offset || !$arrival_timezone_offset){
			return 0;
		}
    ...
    private function get_airport_timezone_offset($airport_code,$journey_date)
    {
		...
		$query = 'select FAL.airport_code,FAT.start_month,FAT.end_month,FAT.timezone_offset from flight_airport_list FAL
                    join flight_airport_timezone_offset FAT on FAT.flight_airport_list_fk=FAL.origin
                    where airport_code = "'.$airport_code.'" and '.$journey_month.' BETWEEN start_month AND end_month 
                    order by 
                    CASE
                    WHEN start_month    = '.$journey_month.' THEN 1
                    WHEN end_month  = '.$journey_month.' THEN 2
	...
	function format_transfer_booking_data($complete_booking_details, $module)
	{
		$response['status']	= SUCCESS_STATUS;
		$response['data']	= array();
		$booking_details = array();
		$currency_obj = new Currency();
		$master_booking_details = $complete_booking_details['data']['booking_details'];
	  // debug($complete_booking_details['data']['payment_details']);
		$itinerary_details = $this->format_itinerary_details($complete_booking_details['data']['booking_itinerary_details']);
	    //echo debug($itinerary_details);exit;
		$payment_details = @$this->format_payment_details($complete_booking_details['data']['payment_details']);
		//debug($payment_details);exit;
		$customer_details = $this->format_customer_details($complete_booking_details['data']['booking_customer_details']);		
		$cancellation_details = $this->format_transfer_cancellation_details($complete_booking_details['data']['cancellation_details']);
		$booking_payment_details = array();
		foreach($master_booking_details as $book_k => $book_v) {
			$core_booking_details = $book_v;
			...
			$attributes = json_decode($core_booking_details['attributes'], true);
			// debug($master_booking_details);exit;
			$core_booking_details['transferType'] = $master_booking_details[0]['transfer_type'];
			$core_booking_details['vehcileName'] = $master_booking_details[0]['vehicle_name'];
			$core_booking_details['categoryName'] = $master_booking_details[0]['vehicle_name'];
			$core_booking_details['TM_Cancellation_Policy'] = $attributes['TM_Cancellation_Policy'];
			$no_of_paxes = $attributes['TotalAdults'].' Adult(s)';
			if($attributes['TotalChilds'] > 0){
				$no_of_paxes.= $attributes['TotalChilds'].' Children(s) Age-'.$attributes['ChildAge'];
			}
			$core_booking_details['adult_count'] = $attributes['TotalAdults'];
			$core_booking_details['child_count'] = $attributes['TotalChilds'];
			$core_booking_details['child_ages'] = $attributes['ChildAge'];
			$core_booking_details['no_of_pax'] = $no_of_paxes;
    ...
    private function format_transfer_cancellation_details($cancellation_details)
	{
		$booking_cancellation_details = array();
		foreach($cancellation_details as $cancel_k => $cancel_v) {
			$booking_cancellation_details[$cancel_v['app_reference']][] = $cancel_v;
