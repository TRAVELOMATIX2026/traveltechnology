...
class Provab_private extends Common_Api_Grind {
    ...
    function BookCar_request($booking_params, $booking_source, $booking_id) {
        ...
        $passenger['DateOfBirth'] = date("Y-m-d", strtotime($date_of_birth));
    ...
    function save_booking($app_booking_id, $book_params, $currency_obj, $module = 'b2c'){
        ...
        $book_total_fare = round($book_params ['token']['TotalCharge']['EstimatedTotalAmount']);
        ...
        $currency = $book_params ['token']['TotalCharge']['CurrencyCode'];
        $transaction_currency = get_application_currency_preference();
        $supplier_identifier = '';
        ...
        $this->CI->car_model->save_booking_details ( $domain_origin, $status, $app_reference, $booking_source, $currency, $phone_number, 
                ...
                $car_to_date, $pickup_time, $drop_time, $car_pickup_location, $car_drop_location, $car_drop_address, $car_pickup_address, $final_cancel_date, $transfer_type,$oneway_fee, $total_fare_markup, $total_fare_tax ,$transaction_currency,$agentServicefee);
        ...
        $attributes['vehicle_class_size'] = $car_data['VehClassSizeName'];
        ...
        $city = $book_params['city_name'];

        $pincode = $book_params['postal_code'];

        $adress1 = $book_params['address'];

        $adress2 = $book_params['address'];
    ...
    public function update_booking_details($book_id, $book_params, $ticket_details,$currency_obj,$module = 'b2c'){
        ...
        $master_search_id = $book_params['temp_booking']['book_attributes']['search_id'];
        ...
        $saved_booking_data = $GLOBALS['CI']->car_model->get_booking_details($book_id);
        ...
        $car_master_booking_status = $master_transaction_status;
        ...
        $booking_id = $book_params ['result'] ['BookingDetails']['BookingId'];

        $booking_reference = $book_params ['result'] ['BookingDetails']['BookingRefNo'];

        $supplier_identifier = $book_params ['result'] ['BookingDetails']['SupplerIdentifier'];

        $book_total_fare = round($book_params ['temp_booking']['book_attributes']['token']['TotalCharge']['EstimatedTotalAmount']);

        $phone_number = $book_params ['temp_booking']['book_attributes']['passenger_contact'];

        //debug($book_total_fare);
        ...
        $GLOBALS['CI']->custom_db->update_record('car_booking_details', array('status' => $master_transaction_status,'booking_id' => $booking_id,'booking_reference' => $booking_reference,'supplier_identifier' => $supplier_identifier), array('app_reference' => $app_reference));



        $GLOBALS['CI']->custom_db->update_record('car_booking_itinerary_details', array('status' => $master_transaction_status), array('app_reference' => $app_reference));

        $GLOBALS['CI']->custom_db->update_record('car_booking_pax_details', array('status' => $master_transaction_status), array('app_reference' => $app_reference));
        ...
        if ($module == 'b2c') {
            ...
            $discount = $book_params['temp_booking']['book_attributes']['promo_code_discount_val'];

            $discount = floatval(preg_replace('/[^\d.]/', '', $discount));

            $promo_code = @$book_params['temp_booking']['book_attributes']['promo_code'];
           ...
           $ac_var = ($cd['AirConditionInd']=='true')? 'AC': 'Non AC';
            ...
            if(
                ...
                (valid_array($fltr['_vehicle_Category']) == false ||
                    ...
                    (valid_array($fltr['_vehicle_Category']) == true && in_array($cd['VehicleCategoryName'], $fltr['_vehicle_Category']))

                    )&&

                (valid_array($fltr['_vehicle_Size']) == false ||

                    (valid_array($fltr['_vehicle_Size']) == true && in_array($cd['VehClassSizeName'], $fltr['_vehicle_Size']))
                ...
                (@$fltr ['min_price'] <= ceil ( $cd ['TotalCharge'] ['EstimatedTotalAmount'] ) && (@$fltr ['max_price'] != 0 && @$fltr ['max_price'] >= floor ( $cd ['TotalCharge'] ['EstimatedTotalAmount'] )))&&
...
//                  debug($category_list); exit;
                ...
                //$total_price = @$hd['total_amount'];
                 ...
                 $total_price = @$hd['TotalCharge']['EstimatedTotalAmount'];
