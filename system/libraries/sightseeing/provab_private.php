...
class Provab_private extends Common_Api_Grind {
	...
	*/
	...
	function process_booking($book_id, $booking_params,$module='b2c') {
		...
		if($book_request['data']['status']){
     	...
     	//debug($book_response);
			...
			//debug($api_book_response_status);exit;
			/**
			 * PROVAB LOGGER *
			 */
	...
	function save_booking($app_booking_id, $book_params, $currency_obj, $module = 'b2c'){
		...
		$product_name = $book_params ['token'] ['ProductName'];
		$star_rating = $book_params ['token'] ['StarRating'];
		$product_code = $book_params ['token'] ['ProductCode'];
		$phone_number = $book_params ['passenger_contact'];
		$phone_code = $book_params ['phone_country_code'];
		$alternate_number = 'NA';
		$travel_date = $book_params ['token'] ['booking_date'];
		$payment_mode = $book_params ['payment_method'];
		$grade_code = $book_params ['token'] ['GradeCode'];
		$grade_desc = @$book_params ['token'] ['GradeDescription'];

		if(isset($book_params ['token']['search_params'])){
			if($book_params ['token']['search_params']){
				//debug('hiii');exit;
				$search_params_list = json_decode(base64_decode($book_params ['token']['search_params']),true);

				$additonal_info = $book_params['token']['AdditionalInfo'];
				$paxes = $book_params['token']['RequestAgeBands'];
			...
			$inclusions = json_decode(base64_decode($book_params ['token'] ['inclusions']),true);
			
			$exclusions = json_decode(base64_decode($book_params ['token'] ['exclusions']),true);
			

			$short_desc = base64_decode($book_params ['token'] ['short_desc']);

			$voucher_req = base64_decode($book_params['token']['voucher_req']);
		}
		//debug($inclusions);exit;
		...
		$attributes = array (
				...
				'ProductCode' => @$book_params ['token'] ['ProductCode'],
				'search_id' => @$book_params ['token'] ['search_id'],
				
				'ProductName' => @$book_params ['token'] ['ProductName'],
				'StarRating' => @$book_params ['token'] ['StarRating'],
				'ProductImage' => @$book_params ['token'] ['ProductImage'],
				'SupplierName' =>$book_params ['token'] ['SupplierName'],
				'SupplierPhoneNumber' =>$book_params ['token'] ['SupplierPhoneNumber'],

				'GradeCode' => @$book_params ['token'] ['GradeCode'],
				'GradeDescription'=>@$book_params['token']['GradeDescription'],
				'Destination' => @$book_params  ['token'] ['Destination'],
				'DeparturePoint'=>@$book_params['token']['DeparturePoint'],
				'DeparturePointAddress' => @$book_params ['token'] ['DeparturePointAddress'],
				'Duration'=>@$book_params['token']['Duration'],
		...
		//debug($created_by_id);exit;
		$sightseeing_details = $GLOBALS ['CI']->sightseeing_model->save_booking_details ( $domain_origin, $status, $app_reference, $booking_source, $booking_id, $booking_reference, $confirmation_reference, $product_name, $star_rating, $product_code,$grade_code,$grade_desc,$phone_number, $alternate_number, $email, $travel_date,$payment_mode, json_encode ( $attributes ), $created_by_id, $transaction_currency, $currency_conversion_rate, $phone_code );
	    ...
	    $attributes = '';
		$location = $book_params ['token'] ['Destination'];
		//debug($location);exit;
		...
		for ($i = 0; $i < $total_pax_count; $i++){
			...
			$GLOBALS ['CI']->sightseeing_model->save_booking_pax_details ($title,$app_reference, $first_name,$last_name, $phone_number, $email, $passenger_type,$status, serialize ( $attributes ) );
		}
		/**
		...
		*/
		...
		if ($module == 'b2c') {
			$pax_data = $book_params['token']['AgeBands'];
	...
	public function update_booking_details($book_id, $book_params, $ticket_details,$currency_obj,$module = 'b2c'){
        ...
        $saved_booking_data = $GLOBALS['CI']->sightseeing_model->get_booking_details($book_id);
        ...
        $sightseeing_master_booking_status = $master_transaction_status;
        $transaction_currency = get_application_currency_preference();
        $application_currency = admin_base_currency();
        $currency_conversion_rate = $currency_obj->transaction_currency_conversion_rate();
        ...
        $multiplier  =1;
        $GLOBALS['CI']->custom_db->update_record('sightseeing_booking_details', array('status' => $master_transaction_status,'booking_id' => $booking_id,'booking_reference' => $BookingRefNo,'confirmation_reference' => $ConfirmationNo), array('app_reference' => $app_reference));
	    ...
	    $attributes = '';
	    $location = $book_params ['booking_params']['token'] ['Destination'];
		...
		$admin_net_fare_markup = 0;
		$GLOBALS['CI']->custom_db->update_record('sightseeing_booking_itinerary_details', array('status' => $master_transaction_status), array('app_reference' => $app_reference));
		$GLOBALS['CI']->custom_db->update_record('sightseeing_booking_pax_details', array('status' => $master_transaction_status), array('app_reference' => $app_reference));
		/*** ************ Update Convinence Fees And Other Details Start *****************
		*/
		// Convinence_fees to be stored and discount
		$convinence = 0;
		...
		$convinence_per_pax = 0;
		if ($module == 'b2c') {
			$pax_data = $book_params['booking_params']['token']['AgeBands'];
			$total_transaction_amount = $trans_total_fare+$book_domain_markup+$gst_value;
				$convinence = $currency_obj->convenience_fees ($total_transaction_amount ,$pax_data );
			$convinence_row = $currency_obj->get_convenience_fees ();
			$convinence_value = $convinence_row ['value'];
	...
	function save_booking_old($app_booking_id, $params, $module = 'b2c') {
		// Need to return following data as this is needed to save the booking fare in the transaction details
			// Need to return following data as this is needed to save the booking fare in the transaction details
		$response ['fare'] = $response ['domain_markup'] = $response ['level_one_markup'] = 0;
	...
	*/
	...
	public function update_block_details($trip_details, $booking_parameters,$cancel_currency_obj,$module='b2c') {
		...
		$booking_parameters['RequestAgeBands'] = $trip_details['BlockTripResult']['RequestAgeBands'];
		$booking_parameters['ProviderName'] = $trip_details['BlockTripResult']['ProviderName'];
		$booking_parameters['AdditionalInfo'] = $trip_details['BlockTripResult']['AdditionalInfo'];
		...
		$booking_parameters['Remarks'] = $trip_details['BlockTripResult']['Remarks'];
	...
	*/
	...
	function update_markup_currency(& $price_summary, & $currency_obj, $no_of_nights = 1, $level_one_markup = false, $current_domain_markup = true, $module='') {
        ...
        $markup_summary['TotalDisplayFare'] = $markup_summary['TotalDisplayFare']+$gst_value;
        $markup_summary['NetFare'] = $markup_summary['NetFare']+$gst_value;
	...
	function format_search_response($sl, $cobj, $sid, $module = 'b2c', $fltr = array()) {
		...
		foreach ( $sl ['SSSearchResult'] ['SightSeeingResults'] as $hr => $hd ) {
			...
			$ActivityCategory =  $hd ['Cat_Ids'];
			foreach($ActivityCategory as $cat_key => $category){
				$hd['category_cstr'] .= '_'.$cat_key;
				
			}
			$Duration =  $hd ['DurationId'];
			foreach($Duration as $du_key => $duartion){
				$hd['duration_cstr'] .= '_'.$du_key;
				
			}
			$Recom =  $hd ['Recommended_For_Ids'];
			foreach($Recom as $r_key => $recommnedation){
				$hd['recommendation_cstr'] .= '_'.$r_key;
	...
	function filter_summary($sl) {
		...
		foreach ( $sl ['SSSearchResult'] ['SightSeeingResults'] as $hr => $hd ) {
			...
			$Duration =  $hd ['DurationId'];
			$Recommended =  $hd ['Recommended_For_Ids'];
			...
			if(valid_array($ActivityCategory)){
				foreach($ActivityCategory as $c_key => $category){
					if (isset ( $filt ['categories'] [$category] ) == false) {
						$filt ['categories'] [$category] ['c'] = 1;
						$filt ['categories'] [$category] ['v'] = $category;
						$filt ['categories'] [$category] ['code'] = $c_key;
					} else {
						$filt ['categories'] [$category] ['c'] ++;
					}
				}
			}
			if(valid_array($Duration)){
				foreach($Duration as $c_key => $duration_val){
					if (isset ( $filt ['duration'] [$duration_val] ) == false) {
						$filt ['duration'] [$duration_val] ['c'] = 1;
						$filt ['duration'] [$duration_val] ['v'] = $duration_val;
						$filt ['duration'] [$duration_val] ['code'] = $c_key;
					} else {
						$filt ['duration'] [$duration_val] ['c'] ++;
					}
				}
			}
			if(valid_array($Recommended)){
				foreach($Recommended as $c_key => $recom){
					if (isset ( $filt ['recommended'] [$recom] ) == false) {
						$filt ['recommended'] [$recom] ['c'] = 1;
						$filt ['recommended'] [$recom] ['v'] = $recom;
						$filt ['recommended'] [$recom] ['code'] = $c_key;
					} else {
						$filt ['recommended'] [$recom] ['c'] ++;
					}
				}
			}
			$filters ['data'] = $filt;
