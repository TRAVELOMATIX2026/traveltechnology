...
class Template {
	...
	public function template_css_dir($file_name='')
	{
		if (empty($file_name) == true) {
			...
			$ve = '?v=';
			$v = '';
			if (strpos($file_name, $ve) !== false) { 
				$file_arr = explode($ve,$file_name);
				$file_name = $file_arr[0];
				$v = $ve . $file_arr[1];
			}
			if (file_exists(SYSTEM_TEMPLATE_LIST_RELATIVE_PATH.'/'.$this->get_application_template().TEMPLATE_CSS_DIR.$file_name)) {
				return SYSTEM_TEMPLATE_LIST.'/'.$this->get_application_template().TEMPLATE_CSS_DIR.$file_name.$v;
			} else {
				return SYSTEM_TEMPLATE_LIST.'/'.DEFAULT_TEMPLATE.TEMPLATE_CSS_DIR.$file_name.$v;
			}
		}
	}
	...
	public function template_js_dir($file_name='')
	{
		if (empty($file_name) == true) {
			...
			if (file_exists(SYSTEM_TEMPLATE_LIST_RELATIVE_PATH.'/'.$this->get_application_template().TEMPLATE_JS_DIR.$file_name)) {
				return SYSTEM_TEMPLATE_LIST.'/'.$this->get_application_template().TEMPLATE_JS_DIR.$file_name.$v;
			} else {
				return SYSTEM_TEMPLATE_LIST.'/'.DEFAULT_TEMPLATE.TEMPLATE_JS_DIR.$file_name.$v;
			}
		}
	}
