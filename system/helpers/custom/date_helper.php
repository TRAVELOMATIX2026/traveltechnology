...
function formatDate($db_datetime)
{
	$db_datetime = strtotime ($db_datetime);
	if ($db_datetime > 0) {
		return date("F j, Y", $db_datetime );
	}
}

function getAllDatesBetween($date, $days) {
    ...
    // Subtract the given number of days from the start date to get the before date
    $startDate->modify("-{$days} days");
    
    // Create the end date by adding the given number of days to the original date
    ...
    // Use DatePeriod to generate dates between start and end
