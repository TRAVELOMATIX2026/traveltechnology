...
Class Car_Model extends Transaction
{
    ...
    function save_booking_details ( $domain_origin, $status, $app_reference, $booking_source, $currency, $phone_number, 
        ...
        $car_to_date, $pickup_time, $drop_time, $car_pickup_lcation, $car_drop_location, $car_drop_address, $car_pickup_address, $final_cancel_date, $transfer_type,$oneway_fee,$total_fare_markup = 0, $total_fare_tax = 0 ,$transaction_currency = 'USD',$agentServicefee = 0){
	...
	function filter_booking_report($search_filter_condition = '', $count=false, $offset=0, $limit=100000000000)
    {
        ...
        if ($count) {
            ...
            if(empty($app_reference_ids) == false) {
                ...
                $cd_query = 'select * from car_booking_pax_details AS CD 
                            WHERE  CD.app_reference IN ('.$app_reference_ids.') ';
                $cancellation_details_query = 'select HCD.* from car_cancellation_details AS HCD WHERE HCD.app_reference='.$this->db->escape($app_reference_ids);
                $booking_itinerary_details  = $this->db->query($id_query)->result_array();
                $booking_customer_details   = $this->db->query($cd_query)->result_array();
