...
Class Module_Model extends CI_Model
{
	...
	function serialize_temp_booking_record_req($booking_params, $module)
	{
		$temp_booking['domain_list_fk']		= get_domain_auth_id();
		$temp_booking['book_id']			= $booking_params['book_id'];
		...
		return array('book_id' => ($booking_params['book_id']), 'temp_booking_origin' => $temp_booking_origin['insert_id']);
	}
	...
	function get_user_privillages()
	{
		...
		if(!empty($user_type))
		{
		$query = 'select PL.privilege_key, PL.url from agent_privilege_list PL JOIN agent_privileges AS P ON P.p_no = PL.origin where P.user_type = ' . $user_type . ' AND P.user_id = ' . $user;
