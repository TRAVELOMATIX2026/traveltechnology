...
Class Transaction extends CI_Model
{
	 ...
	 */
	public function create_payment_record($app_reference, $booking_fare, $firstname, $email, $phone, $productinfo, $convenience_fees=0, $promocode_discount=0,$currency_conversion_rate,$agent_payment_option='wallet')
	{	
		...
		if ($duplicate_pg == false) {
			...
			$data['amount'] = $booking_fare; //ceil($booking_fare+$convenience_fees-$promocode_discount);
			...
			$data['agent_payment_option'] = $agent_payment_option;
			// debug($data);exit;
	...
	function update_payment_record_status_req($app_reference, $encryptedCode, $response_params=array())
	{
		$cond['app_reference'] = $app_reference;
		if (valid_array($response_params) == true) {
			$data['encryptedCode'] = $encryptedCode;
			$data['api_request_params'] = json_encode($response_params);
		}
		$this->custom_db->update_record('payment_gateway_details', $data, $cond);
	}
	function update_payment_record_status_res($app_reference, $response_params=array())
	{
		$cond['app_reference'] = $app_reference;
		if (valid_array($response_params) == true) {
			$data['invoice_params'] = json_encode($response_params);
		}
		$this->custom_db->update_record('payment_gateway_details', $data, $cond);
	}
}
