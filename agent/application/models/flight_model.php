...
LIMIT 20';
	...
	function get_monthly_booking_summary()
	{
		...
		BD.domain_origin='.get_domain_auth_id().' AND BD.created_by_id ='.$GLOBALS['CI']->entity_user_id.' AND BD.status = "BOOKING_CONFIRMED"
	...
	$transaction_currency, $currency_conversion_rate, $gst_details, $phone_country_code,$total_fare_tax = 0 , $total_fare_markup = 0, $agentServicefee = 0)
	{
		...
		$data['agentServicefee'] = $agentServicefee;
		...
		if ($booking_details['status'] == SUCCESS_STATUS && $response['status'] == SUCCESS_STATUS) {
			switch($booking_source){
				case CLARITYNDC_FLIGHT_BOOKING_SOURCE:
					...
					//$PaymentStatus=$order['Order'][0]['PaymentStatus'];
					$TicketStatus=$order['Order'][0]['TicketStatus'];
					//$PnrStatus=$order['Order'][0]['PnrStatus'];
					$ticket_info = !empty($order['TicketDocInfos']['TicketDocInfo']) ? $order['TicketDocInfos']['TicketDocInfo'] : '';
					$customer_rec = $booking_details['data']['booking_customer_details'][0];
					if($customer_rec && !$customer_rec['TicketNumber'] && $ticket_info){
						foreach($booking_details['data']['booking_customer_details'] as $k=>$booking_customer_data){
							$update_ticket_info = array();
					...
					$booking_detail = $booking_details['data']['booking_details'][0];
					if($OrderStatus == 'CANCELED' || $TicketStatus == 'VOIDED'){
						if(($booking_transaction_details && $booking_transaction_details['status'] != 'BOOKING_CANCELLED') || 
							($booking_detail && $booking_detail['status'] != 'BOOKING_CANCELLED')
						){
							$this->custom_db->update_record('flight_booking_transaction_details', ['status'=>'BOOKING_CANCELLED'], array('origin' =>$booking_transaction_details['origin']));
							$this->custom_db->update_record('flight_booking_details', array('status'=>'BOOKING_CANCELLED'),array('app_reference' => $app_reference));
							$this->custom_db->update_record('flight_booking_passenger_details',array('status'=>'BOOKING_CANCELLED') ,array('app_reference' => $app_reference));
							$return_response = SUCCESS_STATUS;
						}
					}else if($TicketStatus == 'TICKETED' || $OrderStatus == 'BOOKED'){
						if($booking_detail && $booking_detail['status'] == 'BOOKING_HOLD'){
							$this->custom_db->update_record('flight_booking_details', array('status'=>'BOOKING_CONFIRMED'),array('app_reference' => $app_reference));
							$this->custom_db->update_record('flight_booking_passenger_details',array('status'=>'BOOKING_CONFIRMED') ,array('app_reference' => $app_reference));
							$this->custom_db->update_record('flight_booking_transaction_details', ['status'=>'BOOKING_CONFIRMED'], array('origin' =>$booking_transaction_details['origin']));
							$return_response = SUCCESS_STATUS;
						}
					}
					break;
