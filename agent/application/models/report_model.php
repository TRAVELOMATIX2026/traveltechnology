...
Class Report_Model extends CI_Model
{
	...
	function auto_suggest_transfer_booking_id($chars, $limit=15)
	{
		$query = 'select distinct(BD.app_reference)  from transfer_booking_details AS BD
						WHERE BD.domain_origin='.get_domain_auth_id().' and BD.created_by_id ='.$GLOBALS['CI']->entity_user_id.' 
						and BD.app_reference!="" and (BD.app_reference like "%'.$chars.'%" OR BD.booking_reference like "%'.$chars.'%")
						order by BD.origin desc limit 0, '.$limit;
		return $this->db->query($query)->result_array();
	}
}