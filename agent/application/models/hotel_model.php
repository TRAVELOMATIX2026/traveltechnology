...
where   hotel_name LIKE '.$search_chars.'
	...
	function tbo_hotel_city_id($city, $country)
	{
		...
		$response['data'] = array();
		$location_details = $this->custom_db->single_table_records('all_api_city_master', 'country_code, origin', array('city_name like' => $city, 'country_name like' => $country));
	...
	$hotel_check_in, $hotel_check_out, $payment_mode,	$attributes, $created_by_id, $transaction_currency, $currency_conversion_rate ,$phone_code = '+1',$total_fare_markup = 0 ,$total_fare_tax = 0, $agentServicefee = 0,$total_destination_charge=0)
	{
	 ...
	 * @param string $attributes
	 */
	function save_booking_itinerary_details($app_reference, $location, $check_in, $check_out, $room_type_name, $bed_type_code, $status, $smoking_preference, $total_fare, $admin_markup, $agent_markup, $currency, $attributes,
	...
	function clean_search_data_old($temp_search_data)
	{
		$success = true;
		//make sure dates are correct
    ...
    public function get_property_ids($city_id){
    	$result = $this->db->select('hotel_id')->get_where('hg_hotels_list',array('city_origin'=>$city_id))->result_array();
    	$return = array();
    	if($result){
