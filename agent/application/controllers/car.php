...
class Car extends MY_Controller {
	...
	function pre_booking($search_id)
	{
		...
		if ($valid_temp_token != false) {
			...
			$convenience_fees = 0; //$currency_obj->convenience_fees($amount, $search_id);
			/********* Convinence Fees End ********/
			...
			//details for PGI
			$amount = $convenience_fees+$amount-$promocode_discount;
                ...
                switch ($temp_booking['booking_source']) {
                    case PROVAB_CAR_BOOKING_SOURCE :
                        ...
                        if ($booking['status'] == SUCCESS_STATUS) {
							...
							// debug($data);exit;
							$this->domain_management_model->update_transaction_details('car', $book_id, $data['fare'], $data['admin_markup'], $data['agent_markup'], $data['convinence'], $data['discount'],$data['transaction_currency'], $data['currency_conversion_rate'],$temp_booking['book_attributes']['agent_payment_option'] );
