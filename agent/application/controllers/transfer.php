    ...
    function booking()
    {
        ...
        if (isset($pre_booking_params['booking_source']) == true) {
            ...
            if ($pre_booking_params['booking_source'] == PROVAB_TRANSFER_BOOKING_SOURCE)
            {
                ...
                $transfer_block_details = $this->transfer_lib->transferblock_data_in_preferred_currency($transfer_block_details, $currency_obj,'b2b');                
                ...
                $pre_booking_params = $this->transfer_lib->update_block_details($transfer_block_details['data']['BlockTransfer'], $pre_booking_params,$currency_obj,'b2b');
                #debug($pre_booking_params);
                #exit;
                $pre_booking_params['markup_price_summary'] =  $this->transfer_lib->update_commission_markup_module_wise($pre_booking_params['Price'],$currency_obj);     
    ...
    function pre_booking($search_id)
    {
        ...
        if ($valid_temp_token != false) {
            ...
            if ($post_params['booking_source'] == PROVAB_TRANSFER_BOOKING_SOURCE) {
                $amount   = $this->transfer_lib->total_price($temp_token['markup_price_summary'],'b2b');
...
//          $verification_amount = roundoff_number($amount+$convenience_fees-$promocode_discount);
            ...
            //debug($agent_paybleamount);exit;
            $currency = $agent_paybleamount['currency'];
    ...
    */
    ...
    function secure_booking()
    {
        ...
        $total_booking_price = $this->transfer_lib->total_price($temp_booking['book_attributes']['token']['markup_price_summary'],'b2b');
        $currency_obj = new Currency ( array (
                    'module_type' => 'transfer',
                    'from' => admin_base_currency (),
                    'to' => admin_base_currency () 
        ...
        if ($domain_balance_status) {
            ...
            if ($temp_booking != false) {
                ...
                if ($booking['status'] == SUCCESS_STATUS) {
                    ...
                    $this->domain_management_model->update_transaction_details('transfer', $book_id, $data['fare'], $data['admin_markup'], $data['agent_markup'], $data['convinence'], $data['discount'],$data['transaction_currency'], $data['currency_conversion_rate'],'wallet' );

                    redirect(base_url().'index.php/voucher/transfers/'.$book_id.'/'.$temp_booking['booking_source'].'/'.$data['booking_status'].'/booking_voucher');
                } else {
    ...
    function cancel_booking($app_reference, $booking_source)
    {
        if(empty($app_reference) == false) {
            ...
            if ($master_booking_details['status'] == SUCCESS_STATUS) {
                $this->load->library('booking_data_formatter');
                $master_booking_details = $this->booking_data_formatter->format_transfer_booking_data($master_booking_details, 'b2b');
