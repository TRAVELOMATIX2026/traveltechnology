...
class Management extends MY_Controller {
	...
	function b2b_transfer_markup() {
		...
		$markup_module_type = 'b2b_transfer';
		$page_data ['form_data'] = $this->input->post ();
		if (valid_array ( $page_data ['form_data'] ) == true) {
			switch ($page_data ['form_data'] ['form_values_origin']) {
				case 'generic' :
	 ...
	 */
	function b2b_balance_manager($balance_request_type = "Cash", $tranid = '')
	{	
		...
		switch (strtoupper ( $balance_request_type )) {
			...
			case 'CASH' :
				...
				break;
			case 'ONLINE' :
				$page_data ['balance_page_obj'] = new Provab_Page_Loader ( 'balance_request_online' );
		...
		if (valid_array ( $page_data ['form_data'] ) == true) {
			...
			//if ($this->form_validation->run ()) {
				...
				if ($page_data ['form_data'] ['origin'] == '') {
					...
					// debug( $page_data );
					// exit;
					if($balance_request_type == 'ONLINE'){
						$system_transaction_id = 'DEP-'.$this->entity_user_id.time();
						$page_data ['form_data']['system_transaction_id'] = $system_transaction_id;
						$page_data ['form_data']['paylink'] = @$page_data ['form_data']['payment_link'][0];
						$currency_obj = new Currency ();
						$currency_conversion_rate = $currency_obj->payment_gateway_currency_conversion_rate();
						$page_data ['form_data']['conversion_value'] = $currency_conversion_rate;

						$page_data ['form_data']['topup_marchantfee'] = $this->custom_db->single_table_records('convenience_fees', 'value', array('module' => 'topup'))['data'][0]['value'];
					...
					exit;*/
					if($balance_request_type == 'ONLINE'){
						redirect ( APP_ROOT_DIR2.'/payment_gateway/payment/'.$system_transaction_id);
		...
		if (!is_array($page_data['form_data'] ?? null)) {
			$page_data['form_data'] = [];
		}
		...
		if($tranid != ''){
			...
			$pg_record = $this->transaction->read_payment_record($tranid);
			$params = json_decode($pg_record['invoice_params'], true); 
			$page_data ['tstatus'] = (!empty($pg_record))?true: false;
			$page_data ['tdetails'] = $params;
		}
