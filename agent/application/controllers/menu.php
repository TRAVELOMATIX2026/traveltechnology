...
class Menu extends MY_Controller
{
	...
	{
		error_reporting(0);
		// if(empty($this->entity_user_id)==true)
		// {
		// 	return redirect('/');
		// }
...
ini_set('display_errors', 1);*/
		...
		if (web_page_access_privilege('p1')) {
			...
			$car_earning  = $sightseeing_earning = $transfers_earning = array();
			$hotel_earning = $module_total_earning = $flight_earning = $bus_earning = $time_line_report = array();
			...
			if (!empty($active_transfersv1)) {
				$page_data['transferv1_booking_count'] = $this->transferv1_model->booking($condition, true);
			}
			if (!empty($active_transfers)) {
				$page_data['transfer_booking_count'] = $this->transfer_model->booking($condition, true);
			}
			$condition = array();
			...
			$page_data['notification_list_details'] = $this->application_logger->get_events_notification(0, 5, $condition, false);
			/********************************** SEARCH ENGINE START **********************************/
			/*Package Data*/
