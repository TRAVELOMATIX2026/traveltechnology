...
<?=$GLOBALS['CI']->template->isolated_view('management/paymentlink')?>
<!-- HTML BEGIN -->
<?php
			...
			<div class="panel_title">
				<ul class="nav nav-tabs b2b_navul" role="tablist" id="myTab">
					...
					<li role="presentation" class="<?php echo $tab2; ?>">
						<a href="#tableList" aria-controls="profile" role="tab" data-bs-toggle="tab">
		...
		<div class="panel_bdy"><!-- PANEL BODY START -->
			<div class="tab-content">
				...
				<div class="panel_bdy mb-3">
					<div class="card">
						<div class="card-header">
							<h5 class="card-title mb-0"><i class="bi bi-funnel"></i> Request Type</h5>
						</div>
						<div class="card-body">
							<div class="row">
								<div class="col-md-6">
									<div class="form-group">
										<label for="balance_request_type" class="form-label">Request Type</label>
				...
				<div class="panel_bdy mb-3">
					<div class="card">
						<div class="card-header">
							<h5 class="card-title mb-0"><i class="bi bi-bank"></i> Deposit Request Details</h5>
						</div>
						<div class="card-body">
							<?php
			...
			<div class="card">
				<div class="card-header">
					<h5 class="card-title mb-0"><i class="bi bi-search"></i> Search Panel</h5>
				</div>
				<div class="card-body">
					<form method="GET" autocomplete="off">
						<div class="row">
							<div class="col-md-3">
								<div class="form-group">
									<label for="system_transaction_id" class="form-label">Transaction Number</label>
									<input type="text" id="system_transaction_id" class="form-control" name="system_transaction_id" value="<?=@$system_transaction_id?>" placeholder="Transaction Number">
								</div>
							</div>
							<div class="col-md-3">
								<div class="form-group">
							...
							<div class="col-md-3">
								<div class="form-group">
									<label for="created_datetime_from" class="form-label">Request From</label>
							...
							<div class="col-md-3">
								<div class="form-group">
									<label for="created_datetime_to" class="form-label">Request To</label>
						...
						<div class="form-group mt-3">
							<button type="submit" class="btn btn-sm btn-success"><i class="bi bi-search"></i> Search</button> 
							<button type="reset" class="btn btn-sm btn-warning"><i class="bi bi-arrow-counterclockwise"></i> Reset</button> 
							<a href="<?php echo base_url(); ?>index.php/management/b2b_balance_manager" id="clear-filter" class="btn btn-sm btn-secondary"><i class="bi bi-x-circle"></i> Clear Filter</a>
						</div>
					</form>
				</div>
			</div>
			...
			<div class="card">
				<div class="card-body">
					<div class="d-flex align-items-center gap-2">
						<strong class="text-nowrap">Current Balance:</strong>
...
function get_table($table_data='')
{
   ...
   <table class="table table-hover table-striped table-bordered" id="balance_request_table">';
...
$(document).ready(function() {
	...
	/* var tstatus = '<?php echo $tstatus ?>';
	console.log(tstatus);
	
	if(tstatus){
		$("#mail_paymentlink_modal").modal('show');
	}

	$('#send_mail_btn').on('click', function(e) {
		$('#mail_error_message').empty();
		email = $("#recipient_email").val();
		app_reference = $("#app_reference").val();
		var emailReg = /^([\w-\.]+@([\w-]+\.)+[\w-]{2,4})?$/;
		if(email != ''){
			if(!emailReg.test(email)){
				$('#mail_error_message').empty().text('Please Enter Correct Email Id');
					return false;    
					}
			
				var _opp_url = app_base_url+'index.php/ajax/sendlink/'+app_reference+'/'+email;
				toastr.info('Please Wait!!!');
				$.get(_opp_url, function() {
					toastr.info('Email sent  Successfully!!!');
					$("#mail_paymentlink_modal").modal('hide');
				});
		}else{
			$('#mail_error_message').empty().text('Please Enter Email ID');
		}
