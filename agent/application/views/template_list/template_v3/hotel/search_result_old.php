...
<div class="rowresult r-r-i item">
	<div class="madgrid forhtlpopover shapcs" id="result_<?=$hd_key?>" data-key="<?=$hd_key?>" data-hotel-code="<?=$hotel_code?>" data-access-key="<?=@$hd['Latitude'].'_'.@$hd['Longitude']?>">
		<div class="col-4 nopad listimage full_mobile">
...
<!-- <img src="" alt="Hotel img" data-src="<?php //echo base_url().'index.php/hotel/image_cdn/'.$hd['ResultIndex'].'/'.$search_id.'/'.base64_encode($hd['HotelCode'])?>" class="lazy h-img"> -->
					...
					<div class="owl-carousel owl-theme htl_img_listing" style="opacity: 1; display: block;">
						<?php foreach($hd['HotelPicture'] as $image){ ?>
							<div class="item">
								<img alt="Hotel Image" data-src="<?=$image?>" class="lazy h-img load-image hide">
							</div>
						<?php } ?>
						<!-- <div class="item">
							<img alt="Hotel Image" data-src="https://photos.hotelbeds.com/giata/00/009487/009487a_hb_r_001.jpg" class="lazy h-img load-image hide">
						</div>
						<div class="item">
							<img alt="Hotel Image" class="lazy h-img load-image" src="https://photos.hotelbeds.com/giata/00/006807/006807a_hb_r_001.jpg">
						</div> -->
					</div>
				...
				<?php
					...
					$RoomPrice = round($hd['Price']['RoomPrice']);
					$Roomtax =  round(@$hd['Price']['Tax']+@$hd['Price']['DestinationTax']);
					$DiscountPrice = round($hd['Price']['Discount']);
					//$NetPrice = round($RoomPrice+$Roomtax-$DiscountPrice);
					$NetPrice = round(@$hd['Price']['NetPrice']);
					?>
				<!-- <img src="" alt="Hotel img" data-src="<?//=$hd['HotelPicture'] ?>" class="lazy h-img"> -->
				<?php if($hd['HotelPicture']&&$image_found==true):?>
				<a data-bs-target="map-box-modal" data-result-token="<?=urlencode($hd['ResultToken'])?>" data-booking-source="<?=urlencode($booking_source)?>" data-price="<?=$RoomPrice?>" data-star-rating="<?=$current_hotel_rate?>"  data-hotel-name="<?php echo $hd['HotelName']?>" id="map_id_<?=str_replace("!","H",$hd['HotelCode'])?>" data-trip-url="<?=$hd['trip_adv_url']?>" data-trip-rating="<?=$hd['trip_rating']?>" data-id="<?=str_replace("!","H",$hd['HotelCode'])?>" class="hotel-image-gal mapviewhtlhotl fal fa-image view-photo-btn hide" data-hotel-code="<?=$hd['HotelCode']?>"></a> 
				<?php endif;?>
				<a class="hotel_location" data-lat="<?=@$hd['Latitude']?>" data-lon="<?=@$hd['Longitude']?>"></a>
				
			</div>
		</div>
		<div class="col-8 listfull full_mobile">
			<div class="sidenamedesc">
					...
					<div class="innd">
					    <div class="imptpldz">
							...
							<div class="shtlnamehotl">
								<a class="hotelName" data-cls="f-sub-<?=$hd_key?>" ><span class="h-name"><?php echo $hd['HotelName']?></span></a> 
							</div>
							...
							<div class="adreshotle h-adr hide" data-bs-toggle="tooltip" data-placement="top" data-original-title="<?php echo $hd['HotelAddress']?>"><?php echo $hd['HotelAddress']?>
							</div>
							<div class="clearfix"></div>
							<div class="htl_lstng_nearby">
								<p> <span><i class="fas fa-map-marker"></i></span> 
									<?php if(!empty($hd['near_by_location'])){
											echo  ($hd['near_by_location'][0]['distance']/1000).' km '.$hd['near_by_location'][0]['poiName'];
										}else{
											echo $hd['HotelAddress'];
										}
									?>
								</p>
							</div>
							...
							<div class="bothicntri">
								<div class="mwifdiv">
								<ul class="htl_spr"> 
								<?php ///// overriding Hotel Amenities, if you don't want please remove this array only
									$hd['HotelAmenities'] = [ 'Breakfast available','Free WiFi', 'Outdoor pool','Free self parking' ];
									?>                         
										...
										<?php if($hd['HotelAmenities']):?>
											...
											?>
												<?php if($internet_result||$innternet_result|| $wf_result):?>
													<li class="wf" data-bs-toggle="tooltip" data-placement="top" title="Wifi">
														<!-- <span>Wifi</span> -->
														<span><i class="fi-rr-internet-speed-wifi"></i></span>
													</li>
												...
												<?php if($b_result||$bf_result):?>
													<li class="bf" data-bs-toggle="tooltip" data-placement="top" title="Breakfast">
														<!-- <span>Breakfast</span> -->
														<span><i class="fi-rr-bowl-spoon"></i></span>
													</li>
												...
												<?php if($p_result || $cp_result):?>
														<li class="pr" data-bs-toggle="tooltip" data-placement="top" title="Parking">
															<!-- <span>Parking</span> -->
															<span><i class="fi-rr-parking"></i></span>
														</li>
												...
												<?php if($s_result||$sw_result):?>
														<li class="sf" data-bs-toggle="tooltip" data-placement="top" title="Swimming pool">
															<!-- <span>Swimming pool</span> -->
															<span><i class="fi-rr-gym"></i></span>
														</li>
				...
				<div class="celhtl width30">
					<div class="sidepricewrp">
						<?php if(@$hd['Price']['Discount']<0) { ?>
						<p class="desl_ds">DEAL OF THE DAY</p>
						<?php } ?>
						<div class="priceflights">
							<!-- <strong>  </strong> -->
							<span class="h-p">
								<p><?php echo $currency_obj->get_currency_symbol($currency_obj->to_currency); ?> <?=roundoff_number($RoomPrice)?></p>
								<?php if(@$hd['Price']['Discount']<0 ) { ?>
								<strike><?= @$NetPrice; ?></strike>
								<?php } ?>
							</span>
							<div class="prcstrtingt">+ <?= $Roomtax; ?> taxes & fees <p class="hotl_per_nyt">per night</p></div>
						</div>
						<form id="f-sub-<?=$hd_key?>" action="<?php echo base_url().'index.php/hotel/hotel_details/'.($search_id)?>" target="_blank">
