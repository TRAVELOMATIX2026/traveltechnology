	...
	foreach ($current_combination_wrapper as $__room_index_key => $__room_index_value) {
		...
		if(isset($temp_room_details['LastCancellationDate'])==true&&empty($temp_room_details['LastCancellationDate'])==false){
			...
			if(date('Y-m-d',strtotime($RawCancellationDate)) !=date('Y-m-d') && date('Y-m-d',strtotime($RawCancellationDate)) > date('Y-m-d')){
				...
				$room_panel_details .=' <span class="romtypefrecan"> Last Cancellation Date<p> '.$FormattedLastCancellationDate.'</p></span>';
			}else{
				//echo $temp_room_details['LastCancellationDate'];
				$today_cancel_date=1;
			...
			// debug($temp_room_details['Amenities']);exit;
				if($temp_room_details['Amenities']){
						...
						foreach ($temp_room_details['Amenities'] as $a_key=>$__amenity) {
							...
							if($a_key <=1){
								// debug($__amenity);
								$room_panel_details .='<span class="noof_ave" data-bs-toggle="tooltip" data-title="'.$__amenity.'" data-placement="right"><i class="fa fa-check" aria-hidden="true"></i>'.$__amenity.'</span>';
							}				
						}
						if(count($temp_room_details['Amenities'])>2){
							...
							foreach ($temp_room_details['Amenities'] as $m_key => $m_value) {
								...
								if($m_key>=2){
									// echo $m_value;
									$room_panel_details .='<span class="noof_ave" data-bs-toggle="tooltip" data-title="'.$m_value.'" data-placement="right"><i class="fa fa-check" aria-hidden="true"></i>'.$m_value.'</span>';
								}
		...
		if(empty($temp_room_details['Amenities'])==true && empty($temp_room_details['OtherAmennities'])==true && empty($temp_room_details['RateType'])== false){
			$room_panel_details .='<span class="noof_ave"><i class="fa fa-check" aria-hidden="true"></i>'.ucfirst($temp_room_details['AccommodationType']).'</span>';
		}
