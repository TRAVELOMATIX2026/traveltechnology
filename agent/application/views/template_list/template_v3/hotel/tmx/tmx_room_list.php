...
foreach ($_RoomCombinations['RoomCombination'] as $__rc_key => $__rc_value) {
	...
	foreach ($current_combination_wrapper as $__room_index_key => $__room_index_value) {
		...
		$today_cancel_date=0;
		$is_refundable_status = "Non-Refundable";
		if(isset($temp_room_details['LastCancellationDate'])==true&&empty($temp_room_details['LastCancellationDate'])==false){
			if(date('Y-m-d',strtotime($temp_room_details['LastCancellationDate'])) !=date('Y-m-d') && date('Y-m-d',strtotime($temp_room_details['LastCancellationDate'])) > date('Y-m-d')){
				$today_cancel_date=0;
				$room_panel_details .=' <span class="romtypefrecan refund_txt"><i class="far fa-check-circle"></i> Refundable</span>';
				$is_refundable_status = "Refundable";
			}else{
				$today_cancel_date=1;
				$room_panel_details .=' <span class="romtypefrecan non_refund_txt"><i class="far fa-exclamation-triangle"></i> Non-Refundable</span>';
				$is_refundable_status = "Non-Refundable";
			}
		}
		else{
			$today_cancel_date=1;
			$room_panel_details .=' <span class="romtypefrecan refund_txt"><i class="far fa-exclamation-triangle"></i> Non-Refundable</span>';
			$is_refundable_status = "Non-Refundable";
		}	
		...
		if($is_refundable_status=="Refundable")
		{
			$free_cancellation = '<span class="free_cncl_till"> Free Cancellation till '.local_month_date($temp_room_details['LastCancellationDate']).'</span>';
		}
		...
		data-non-refundable="'.$non_refundable.'" data-today-cancel-date="'.$today_cancel_date.'">View Cancellation</a>'.$free_cancellation.'<div class="clearfix"></div>';
		if (isset($temp_room_details['Amenities'])) {
				...
				foreach ($temp_room_details['Amenities'] as $a_key=>$__amenity) {
					...
					if($a_key <=1){
						//debug($__amenity);
						$room_panel_details .='<span class="noof_ave" data-bs-toggle="tooltip" title="'.$__amenity.'" data-placement="right"><i class="fa fa-check" aria-hidden="true" ></i>'.$__amenity.'</span>';
					}				
				}
	...
	$clean_room_list .= '<div class="romconoutdv"  data-bed="'.$temp_room_details['RoomTypeName'].'" data-category="'.$temp_room_details['room_only'].'" data-cancellation="'.$is_refundable_status.'">';
