                ...
                <div class="search_fltr_section">
                    <form method="GET" role="search" class="search-form-container" id="auto_suggest_booking_id_form">
                        ...
                        <div class="search-button-group">
                            ...
                            <a title="Clear Search & Show All" class="btn btn-clear" href="<?=base_url().'index.php/report/bus'?>">
					...
					<table class="table table-sm table-bordered">
						...
						<?php
							if (isset($table_data) == true and valid_array($table_data['booking_details']) == true) {
								...
								?>
									<tr>
										<td data-label="SNO"><?php echo ($current_record++)?></td>
										<td data-label="Application Reference"><?php echo $app_reference;?></td>
										<td data-label="PNR"><?php echo $pnr?></td>
										<td data-label="Customer Name"><?php echo $booking_customer_details[0]['name']?></td>
										<td data-label="From"><?php echo $departure_from?></td>
										<td data-label="To"><?php echo $arrival_to?></td>
										...
										<td data-label="GST"><?php echo roundoff_number($gst)?></td>
										<td data-label="Total Fare"><?php echo $grand_total?></td>
										<td data-label="Travel Date"><?php echo app_friendly_absolute_date($journey_datetime)?></td>
...
function bus_voucher_email($app_reference, $booking_source,$status,$recipient_email)
{
	...
	return '<a class="btn btn-sm btn-primary send_email_voucher" data-app-status="'.$status.'"   data-app-reference="'.$app_reference.'" data-booking-source="'.$booking_source.'"data-recipient_email="'.$recipient_email.'"><i class="bi bi-envelope"></i> Email Ticket</a>';
}
function get_cancellation_details_button($app_reference, $booking_source, $status)
{
	if($status == 'BOOKING_CANCELLED'){
		return '<a target="_blank" href="'.base_url().'bus/cancellation_refund_details?app_reference='.$app_reference.'&booking_source='.$booking_source.'&status='.$status.'" class="col-md-12 btn btn-sm btn-info "><i class="bi bi-info-circle"></i> Cancellation Details</a>';
	}
}
?>
<script>
