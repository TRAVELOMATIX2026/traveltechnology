                ...
                <div class="search_fltr_section">
                    <form method="GET" role="search" class="search-form-container" id="auto_suggest_booking_id_form">
                        ...
                        <div class="search-button-group">
                            ...
                            <a title="Clear Search & Show All" class="btn btn-clear" href="<?=base_url().'index.php/report/hotel'?>">
...
function get_table($table_data, $total_rows)
{
	...
	$report_data .= '<table class="table table-sm table-bordered">
		...
		$report_data .= '<tr>
					...
					<td data-label="Confirmation Number" class="">'.$confirmation_reference.'</span></td>
					<td data-label="Customer Name">'.$customer_details[0]['title'].' '.$customer_details[0]['first_name'].' '.$customer_details[0]['last_name'].'</td>
					<td data-label="Area">'.$hotel_location.'</td>
					<td data-label="Fare"><span title="('.$total_fare_markup.'+'.$total_fare_tax.')">'.(roundoff_number($total_fare_markup+$total_fare_tax)).'</span></td>
					<td data-label="Markup">'.roundoff_number($agent_markup).'</td>
					<td data-label="Service Fees">'.roundoff_number($agentServicefee).'</td>
					<td data-label="GST">'.roundoff_number($gst).'</td>
					<td data-label="Total Fare">'.roundoff_number($total_fare_markup+$total_fare_tax+$agent_markup+$agentServicefee+$gst).'</td>
					<td data-label="Check In / Check Out">'.app_friendly_absolute_date($hotel_check_in).'/<br/>'.app_friendly_absolute_date($hotel_check_out).'</td>
			...
			$report_data .= '<tr><td colspan="14">No Data Found</td></tr>';
		}
	$report_data .= '</tbody>
	</table>
			</div>';
...
function get_cancellation_details_button($app_reference, $booking_source, $status)
{
	if($status == 'BOOKING_CANCELLED'){
		return '<a target="_blank" href="'.base_url().'hotel/cancellation_refund_details?app_reference='.$app_reference.'&booking_source='.$booking_source.'&status='.$status.'" class="col-md-12 btn btn-sm btn-info "><i class="bi bi-info-circle"></i> Cancellation Details</a>';
	}
}
