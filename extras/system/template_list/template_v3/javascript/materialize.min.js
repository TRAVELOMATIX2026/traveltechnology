...
M.anime = function() {
  ...
  i.el.M_Datepicker = i;
  i.options = g.extend({}, B.defaults, e);

  // Merge i18n if present
  if (e && e.hasOwnProperty("i18n") && typeof e.i18n === "object") {
    i.options.i18n = g.extend({}, B.defaults.i18n, e.i18n);
  }

  // ✅ Ensure minDate is a Date object before using setHours
  if (i.options.minDate) {
    if (!(i.options.minDate instanceof Date)) {
      i.options.minDate = new Date(i.options.minDate);
    }
    if (!isNaN(i.options.minDate)) {
      i.options.minDate.setHours(0, 0, 0, 0);
    }
  }

  // ✅ Ensure maxDate is a Date object before using setHours
  if (i.options.maxDate) {
    if (!(i.options.maxDate instanceof Date)) {
      i.options.maxDate = new Date(i.options.maxDate);
    }
    if (!isNaN(i.options.maxDate)) {
      i.options.maxDate.setHours(0, 0, 0, 0);
    }
  }

  // Setup internal variables and UI
  i.id = M.guid();
  i._setupVariables();
  ...
  // ✅ Ensure defaultDate is valid
  if (!i.options.defaultDate && i.el.value) {
    var parsedDate = Date.parse(i.el.value);
    if (!isNaN(parsedDate)) {
      i.options.defaultDate = new Date(parsedDate);
    }
  }

  var n = i.options.defaultDate;
  if (B._isDate(n)) {
    if (i.options.setDefaultDate) {
      i.setDate(n, !0);
      i.setInputValue();
    } else {
      i.gotoDate(n);
