$(document).ready(function(e){
	...
	var cache = {};
	$('.user_traveller_details').each(function(){
	$(this).catcomplete({
		source:  function( request, response ) {
			...
			if ( term in cache ) {
				...
				$.getJSON( app_base_url+"ajax/user_traveller_details", request, function( data, status, xhr ) {
					cache[ term ] = data;
					response( cache[ term ] );
				});
			}
		...
		select: function(event,ui){
			...
			if(expiry_days >= 0 && passport_number !='' && passport_issuing_country !='') {//If Passport Details are valid 
				...
				parent_obj.find("#passenger_passport_expiry_day_"+$(this).data('row-id')).val(parseInt(passport_expiry_day));
			...
			// console.log("item:",item,':',auto_suggest_value);
