...
function check_empty_search_result() {
	...
	**/
					...
					<div class="popconyent">
						<div class="contfare">
							<ul role="tablist" class="nav nav-tabs flittwifil">
								<li class="active" data-role="presentation"><a data-bs-toggle="tab" datrole="tab" href="#iti_det_${gen_id}">Itinerary</a></li>
								<li data-role="presentation"><a data-bs-toggle="tab" class="iti-fare-btn" role="tab" href="#fare_det${gen_id}" data-form-id="form-id-${gen_id}">Fare Details</a></li>
								<li data-role="presentation"><a data-bs-toggle="tab" class="iti-fare-btn" role="tab" href="#fare_break_${gen_id}">Fare Breakup</a></li>
								<li data-role="presentation"><a data-bs-toggle="tab" class="iti-fare-btn" role="tab" href="#baggage_inclusions_${gen_id}">Baggage & Inclusions</a></li>
							</ul>
		...
		outerSummary += '<div class="madgrid" data-key="' + rootIndicator + flightKey + '">';
			...
			outerSummary += `
				<div class="col-4 nopad wayfour full_same">
					...
					${resultSummery.total_pax > 1 ? `<div class="col-12 trvlrs_amt_wrappr nopad"><div class="text-center trvlrs_amt">${currencySymbol} <span class="travl_amt_lrg">${formatIndianNumber(t_price[0])}</span>.<span class="travl_amt_sml">${t_price[1]}</span> <span class="all_trvlrs">for all travellers</span></div></div>` : ''}
				</div>
			`;
	...
	$(document).on('click', '.stopone', function(e) {
	    e.preventDefault(); // Prevent default <a> click behavior
