$(document).ready(function() {
	...
	// Show result skeleton loader (for initial page load only)
	window.showCarResultLoader = function() {
		var $container = $('#car_search_result');
		if (!$container.length) {
			console.warn('Car search result container not found');
			return;
		}
		
		var $skeleton = $('#result-skeleton-loader');
		
		// Show skeleton loader only (not overlay)
		if ($skeleton.length) {
			$skeleton.removeClass('hide');
		}
	}

	// Hide result skeleton loader
	window.hideCarResultLoader = function() {
		var $skeleton = $('#result-skeleton-loader');
	...
	// Show overlay loader (for sorting and filtering only)
	window.showCarOverlayLoader = function() {
	...
	// Hide overlay loader
	window.hideCarOverlayLoader = function() {
		var $loader = $('#result-loader-overlay');
	...
	window.process_result_update = function(result) {
		...
		// Hide skeleton loader when results are loaded (for initial load and filtering)
		if (typeof window.hideCarResultLoader === 'function') {
			window.hideCarResultLoader();
		}
		
		// Hide overlay loader when results are loaded (for sorting only)
		if (typeof window.hideCarOverlayLoader === 'function') {
			window.hideCarOverlayLoader();
		}
	...
	// Show skeleton loader on page load (not overlay)
	if (typeof window.showCarResultLoader === 'function') {
		window.showCarResultLoader();
	}
	...
	function enable_sort() {
		...
		$(".price-l-2-h").click(function() {
			// Show overlay loader for sorting
			if (typeof window.showCarOverlayLoader === 'function') {
				window.showCarOverlayLoader();
			}
			...
			// Hide overlay loader after sorting
			setTimeout(function() {
				if (typeof window.hideCarOverlayLoader === 'function') {
					window.hideCarOverlayLoader();
				}
...
//alert('hai');
	...
	function filter_rom(_filter_trigger, _filter, attr) {
		...
		if (dynamic_page_data == false) {
			...
			$('.r-r-i').each(function(key, value) {
				...
				var _cartype = $('.car_type', this).text();

				var _door_count = $('.door_count', this).text();

				var _vehicle_size = $('.vehicle-size-f', this).text();
	...
	function dynamic_filter_rom() {
		...
		// Show skeleton loader for filtering
		if (typeof window.showCarResultLoader === 'function') {
			window.showCarResultLoader();
		}
	...
	if(!mailformat)
	{
	   ...
	   return false;
		...
		$.ajax({
			...
			type:'POST',
			data:{'email':input_text,'cardetails':cardetails,'module':'car'},
