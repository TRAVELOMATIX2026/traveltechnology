...
window.process_result_update = function(result, total_api_count, api_count) {
    ...
    if (result.status) {
        if ($('.round-domestk').length) {
            console.log("asn1");
            $('#t-w-i-1').append($(result.data).find('#t-w-i-1').html());
            $('#t-w-i-2').append($(result.data).find('#t-w-i-2').html());
        } else {
            console.log("asn2", get_time());
            var one_way_only = $(result.data).find('.one_way_only').html();
            console.log('api_result_counter:', api_result_counter, get_time());
            console.log('r-r-i:first_index:', $('.r-r-i:first').index(), get_time());

            if (one_way_only && api_result_counter > 1 && $('.r-r-i:first').index() !== -1) {
                console.log("asn3", get_time());
                $('.one_way_only').append(one_way_only);
                console.log("asn33", get_time());
            } else {
                console.log("asn4", get_time());
                $('#flight_search_result').append(result.data);
                console.log("asn44", get_time());
            }
            brw_load++;
        }
        console.log("call updateFilters", get_time());
        updateFilters(api_result_counter);
        update_total_count_summary();
        default_col2x(total_api_count);
        hide_result_pre_loader();
        $('.r-r-i:first').show();
    } else if (total_api_count === api_result_counter) {
        check_empty_search_result();
    }
};
	...
	function updateFilters() {
		console.log("updateFilters",get_time());
		//update filters
		...
		var minLay = 0;//$('.layover-duration:first', temp_minimum_record).data('layoverduration');

		var maxLay = 50;//$('.layover-duration:first', temp_minimum_record).data('layoverduration');
		...
		console.log("before sort:",get_time());
		...
		// 	is_num: true
		// });
		console.log("after sort:",get_time());
		console.log("api_result_counter:1:",api_result_counter,get_time());
		console.log("brw_load:",brw_load,get_time());
		var _total_records = $('.r-r-i').length;
		//$('#total_records').text(_total_records);
		console.log("total_rec:",_total_records,get_time());
		var __visible_records = parseInt($('.r-r-i:visible').length);
		console.log("visible_rec:",__visible_records,get_time());
		// $('#flight_search_result #t-w-i-1 > .rowresult').hide(); // Hide all child elements
		$('#flight_search_result #t-w-i-1 > .rowresult:lt(10)').show(); 
		var children = $('#flight_search_result #t-w-i-1 > .rowresult');
		console.log("loop started",get_time());
		children.each(function(index) {
				const element = $(this);
				if (!$(this).is(':visible')){
					setTimeout(function() {
						element.fadeIn(400);
						// console.log(index,get_time());
		...
		if($('#flight_search_result #t-w-i-2').length > 0){
			var children2 = $('#flight_search_result #t-w-i-2 > .rowresult');
		...
		// brw_load++;
		// console.log("brw_load 1:",brw_load);
		// setTimeout(function () {
		// 	$('#reset_filters').click();
		// }, 5000);
		$('.r-r-i').each(function (key, value) {
			...
			originairport = $('.org-airport:first', this).data('airport');
			destinationairport = $('.dst-airport:first', this).data('airport');
			layoverairport = $('.layover-airport:first', this).data('airport');
			...
			if (originAirportList.hasOwnProperty(originairport) == false) {
				originAirportList[originairport] = $('.org-airport:first', this).data('airport');
				//cloneList[originairport + 'MINP'] = key;
				cloneCriteriaList[originairport + 'MINP'] = price;
			}

			if (destinationAirportList.hasOwnProperty(destinationairport) == false) {
				destinationAirportList[destinationairport] = $('.dst-airport:first', this).data('airport');
				//cloneList[originairport + 'MINP'] = key;
				cloneCriteriaList[destinationairport + 'MINP'] = price;
			}

			if (layoverAirportList.hasOwnProperty(layoverairport) == false) {
				layoverAirportList[layoverairport] = $('.layover-airport:first', this).data('airport');
				//cloneList[layoverairport + 'MINP'] = key;
				cloneCriteriaList[layoverairport + 'MINP'] = price;
			}
			// console.log(layoverAirportList);
			...
			if (parseFloat(original_price) < parseFloat(original_minPrice)) {
			...
			if (parseFloat(original_price) > parseFloat(original_maxPrice)) {
		...
		$('.layover-duration').each(function (key, value) {

			layoverTimeData = $(this).data('layoverduration');

			layoverTime.push(layoverTimeData);

		});

		// minLay = 0;//Math.min.apply(Math,layoverTime);

		minLay = Math.min.apply(Math,layoverTime);

		maxLay = Math.max.apply(Math, layoverTime) + 1;
		...
		loadLayRangeSelector(minLay, maxLay);

		LoadOriginAirportList(originAirportList);
		LoadDestinationAirportList(destinationAirportList);
		LoadLayoverAirportList(layoverAirportList);
		...
		//added for flight sorting
		console.log("airlineList",airlineList,get_time());
		// $("#flight_search_result #t-w-i-1").jSort({
		// 	sort_by: '.f-p:first',
		// 	item: '.r-r-i',
		// 	order: 'asc',
		...
		console.log('cloneList',cloneList,get_time());

		best_cheapest_shortest_flexible_flight_prices();
		console.log('best_cheapest_shortest_flexible_flight_prices',get_time());
		...
		$('#lay_minimum_range_value', '#lay_min_max_slider_values').val(Math.ceil(minLay));
	...
	function LoadLayoverAirportList(airportlist) {

		var filterList = '';
		var index = 1;
		if ($('.layover-airport').length > 0) {
			$.each(airportlist, function (key, value) {
				if (key != "" && key != "null" && typeof key != "undefined") {
					filterList += '<li>';
					filterList += '<div class="squaredThree airlinesrt">';
					filterList += '<input type="checkbox" value="' + key + '" name="check" class="layovercheckbox" id="squaredThree3' + index + '">';
					filterList += '<label for="squaredThree3' + index + '"></label>';
					filterList += '</div>';
					filterList += '<label for="squaredThree3' + index + '" class="lbllbl">';

					filterList += '<span class="airline_namesrch">' + value + '<span class="airline_smldts"></span>';
					filterList += '</span>';
					filterList += '</label>';
					filterList += '</li>';
					index++;
		...
		$('#layovers').html('<ul class="locationul">' + filterList + '</ul>');
	}

	function LoadOriginAirportList(airportlist) {

		var filterList = '';
		var index = 1;
		if ($('.org-airport').length > 0) {
			$.each(airportlist, function (key, value) {
				if (key != "" && key != "null" && typeof key != "undefined") {
					filterList += '<li>';
					filterList += '<div class="squaredThree airlinesrt">';
					filterList += '<input type="checkbox" value="' + key + '" name="check" class="originairportcheckbox" id="squaredThree1orga' + index + '">';
					filterList += '<label for="squaredThree1orga' + index + '"></label>';
					filterList += '</div>';
					filterList += '<label for="squaredThree1orga' + index + '" class="lbllbl">';
		...
		$('#originairport').html('<ul class="locationul">' + filterList + '</ul>');
	}

	function LoadDestinationAirportList(airportlist) {

		var filterList = '';
		var index = 1;
		if ($('.dst-airport').length > 0) {
			$.each(airportlist, function (key, value) {
				if (key != "" && key != "null" && typeof key != "undefined") {
					filterList += '<li>';
					filterList += '<div class="squaredThree airlinesrt">';
					filterList += '<input type="checkbox" value="' + key + '" name="check" class="dstairportcheckbox" id="squaredThree2dsta' + index + '">';
					filterList += '<label for="squaredThree2dsta' + index + '"></label>';
					filterList += '</div>';
					filterList += '<label for="squaredThree2dsta' + index + '" class="lbllbl">';
		...
		$('#destinationairport').html('<ul class="locationul">' + filterList + '</ul>');
	}
	...
	function loadFlighttypeFilter(airlineList) {
		...
		if ($('.n-r').length > 0) {
			...
			$.each(airlineList, function (key, value) {
				...
				filterList += '<label for="squaredThreeflighttype' + index + '" class="lbllbl" style="margin-left:20px;margin-top:-8px;">' + value + '</label>';
	...
	function loadPriceRangeSelector(minPrice, maxPrice, original_minPrice = '', original_maxPrice = '') {
		...
		$("#slider-range").slider({
			...
			change: function (event, ui) {
				// console.log(ui.values[0] +' - '+min_amt);
				// console.log(ui.values[1] +' - '+max_amt);
	...
	function loadLayRangeSelector(minLay, maxLay) {
		...
		$("#slider-range-lay").slider({
			...
			change: function (event, ui) {
				// console.log(ui.values[0] +'--'+ min_lay);
				// console.log(ui.values[1] +'--'+ max_lay);
	...
	function set_slider_lay_label(val1, val2) {
		if (val2 == "-Infinity") {
			...
			$("#layover").text(val1 + " H - " + val2 + " H");
		}
	}

	function set_slider_label(val1, val2, original_minPrice = '', original_maxPrice = '') {

		$("#amount").html("<span class='filt-currency' >" + sliderCurrency + " </span>" + "<span class='filt-min_price' data-min_price='" + original_minPrice + "' >" + val1 + " </span>" + " - " + "<span class='filt-currency' >" + sliderCurrency + " </span>" + "<span class='filt-max_price' data-max_price='" + original_maxPrice + "' >" + Math.ceil(val2) + " </span>");
	...
	function cloneSlider(slides) {
		...
		$.each(cloneList, function (k, v) {
			...
			$('#flight_search_result .r-r-i').each(function (k) {

				let airlineName = $(this).find('.a-n:first').text();

				let airlineCode = $(this).find('.a-n:first').data('code');

				let airlinePrice = $(this).find('.price:first').data('price');

				let airlineStops = $(this).find('.slider_stop:first').data('slider_stop');
			...
			function getStopsByAirlineName(airlineName) {

				let stops = airlineData.filter(airline => airline.name === airlineName).map(airline => airline.stops);

				let uniqueStops = [...new Set(stops)];
			...
			airline_stops = $('#flight_search_result .r-r-i:nth-child(' + k + ') .slider_stop:first').data('slider_stop');

			// Filter airline data based on airline name and stops

			function filterAirlineData(airlineName, stops) {

				return airlineData.filter(airline => {

					return airline.name === airlineName && stops.includes(airline.stops);

				});

			}

			// Getting lowest fare for stops based on airline name

			function getLowestPricesByStops(airlineName) {

				let stopsForSpecificAirline = getStopsByAirlineName(airlineName);

				// Filter data for 0 or 1 stops and more than 1 stop

				let dataFor0Or1Stops = filterAirlineData(airlineName, [0, 1]);

				let dataForMoreThan1Stop = filterAirlineData(airlineName, stopsForSpecificAirline.filter(stop => stop > 1));

				// Calculate lowest prices

				let lowestPrice0Or1Stops = getLowestPrice(dataFor0Or1Stops);

				let lowestPriceMoreThan1Stop = getLowestPrice(dataForMoreThan1Stop);

				return {

					'0-1': lowestPrice0Or1Stops,

					'2+': lowestPriceMoreThan1Stop

				};

			}

			// Helper function to get lowest price from an array of airline data

			function getLowestPrice(dataArray) {

				let lowestPrice = null;

				dataArray.forEach(airline => {

					if (lowestPrice === null || airline.price < lowestPrice) {

						lowestPrice = airline.price;
			...
			let lowestPrices = getLowestPricesByStops(airline_name);

			// console.log('Lowest price for 0-1 stops:', lowestPrices['0-1']);

			// console.log('Lowest price for 2+ stops:', lowestPrices['2+']);
			...
			list += '<div class="imgemtrx"><img  alt="Flight" src="' + airline_image + '">';

			list += '<strong title="' + airline_name + '">' + airline_name + '</strong>';
			...
			if (lowestPrices['0-1'] == '--') {
				...
				$stop01Currency = sliderCurrency;
			}

			if (lowestPrices['2+'] == '--') {
				$stop1PlusCurrency = '--';
			} else {
				$stop1PlusCurrency = sliderCurrency;
			}

			list += '<span class="mtrxprice"><span class="clone_side_org_price hide">' + airline_original_price + '</span><span class="clone_side_curr">' + $stop01Currency + '</span> <span class="clone_side_price">' + lowestPrices['0-1'] + '</span></span>';

			list += '</div>';

			list += '<div class="one-stop-alsmtrx">';

			list += '<span class="mtrxprice"><span class="clone_side_org_price hide">' + airline_original_price + '</span><span class="clone_side_curr">' + $stop1PlusCurrency + '</span> <span class="clone_side_price">' + lowestPrices['2+'] + '</span></span>';
		...
		$("#arlinemtrx1").empty();
		$('#arlinemtrx1').html('<div id="arlinemtrx" class="owl-carousel matrixcarsl">'+list+'</div>');
		carousel();
		// var firstItemCheckbox = $('#arlinemtrx1 .owl-item').first().find('.airline-slider');
                    
		// if (firstItemCheckbox.length > 0) {
		// 	firstItemCheckbox.trigger('click');
		// }
	}
	...
	$(document).on('change', '#destinationairport .dstairportcheckbox', function () {

		filter_row_origin_marker();
	});

	$(document).on('change', '#originairport .originairportcheckbox', function () {

		filter_row_origin_marker();
	});

	$(document).on('change', '#layovers .layovercheckbox', function () {
    ...
    var airlineListLength = airlineList.length;
    var stopCountListLength = stopCountList.length;
    var flighttypelistLength = flighttypelist.length;
    var deptimeListLength = deptimeList.length;
    var arrtimeListLength = arrtimeList.length;
    var originAirportListLength = originAirportList.length;
    var destinationAirportListLength = destinationAirportList.length;
    var LayoverAirportListLength = LayoverAirportList.length;

    $('.r-r-i' + visibility).each(function () {
        var $this = $(this);
        var cond = true;

        var layoverDurations = $('.layover-duration', this);
        if (layoverDurations.length > 0) {
            layoverDurations.each(function () {
                var b = parseFloat($(this).data('layoverdurationhm'));
                if (b < min_lay || b > max_lay) {
                    cond = false;
                    return false; // Break out of layoverDurations.each
                }
            });
        } else {
            if (min_lay > 0) {
        ...
        var price = parseFloat($('.price:first', this).data('price'));
        if (price < min_price || price > max_price) {
            $this.hide();
            return;
        }

        var airlineCode = $('.a-n:first', this).data('code');
        if (airlineListLength && $.inArray(airlineCode, airlineList) === -1) {
            $this.hide();
            return;
        }

        var stopCount = parseInt($('.stp:first', this).data('category'));
        if (stopCountListLength && $.inArray(stopCount, stopCountList) === -1) {
            $this.hide();
            return;
        }

        var flightTypeCode = $('.n-r:first', this).data('code');
        if (flighttypelistLength && $.inArray(flightTypeCode, flighttypelist) === -1) {
            $this.hide();
            return;
        }

        var depTimeCategory = parseInt($('.dep_dt:first', this).data('category'));
        if (deptimeListLength && $.inArray(depTimeCategory, deptimeList) === -1) {
            $this.hide();
            return;
        }

        var arrTimeCategory = parseInt($('.arr_dt:first', this).data('category'));
        if (arrtimeListLength && $.inArray(arrTimeCategory, arrtimeList) === -1) {
            $this.hide();
            return;
        }

        var originAirport = $('.org-airport:first', this).data('airport');
        if (originAirportListLength && $.inArray(originAirport, originAirportList) === -1) {
            $this.hide();
            return;
        }

        var destinationAirport = $('.dst-airport:first', this).data('airport');
        if (destinationAirportListLength && $.inArray(destinationAirport, destinationAirportList) === -1) {
            $this.hide();
            return;
        }

        var layoverAirport = $('.layover-airport:first', this).data('airport');
        if (LayoverAirportListLength && $.inArray(layoverAirport, LayoverAirportList) === -1) {
            $this.hide();
            return;
        }
	...
	**/
	...
	$('.price-ofr-tab a').click(function () {
	...
	function best_cheapest_shortest_flexible_flight_prices() {
		...
		airlineData.sort(function (a, b) {

			return a.price - b.price;

		});

		let cheapestFlight = airlineData[0];
		// console.log(cheapestFlight);

		// Best price

		let sortedByStops = [...airlineData].sort(function (a, b) {

			return a.stops - b.stops;

		});

		let bestFlight = sortedByStops[0];

		// Shortest/Direct

		let shortestFlight = [...airlineData].reduce((shortest, flight) => {

			if (!shortest || flight.stops < shortest.stops) {
		...
		$('.cheapest .price').html(`${sliderCurrency} ${cheapestFlight.price}`);

		$('.best .price').html(`${sliderCurrency} ${bestFlight.price}`);

		$('.shortest .price').html(`${sliderCurrency} ${shortestFlight.price}`);

		$('.flexiable .price').html(`${sliderCurrency} ${shortestFlight.price}`);
