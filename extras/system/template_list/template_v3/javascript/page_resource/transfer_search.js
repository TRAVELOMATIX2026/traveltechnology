$(document).ready(function() {
		...
		if (_next_page == true && $('#npl_img').get(0) && $('#npl_img').get(0).scrollHeight != 0) {
			...
			if ($(window).scrollTop() + $(window).height() >= $('#npl_img').get(0).scrollHeight) {
				if(!$('#npl_img').attr('loaded')) {
					...
					//ajax goes here
					load_transfers(process_result_update, _offset, _ini_fil);
				}
			}
		}
	}
	...
	window.ini_result_update = function(result) {
		...
		if (result.hasOwnProperty('status') == true && result.status == true) {
				...
				enable_vehicle_selector(result.filters.vehicle);
				//enable_suitcase_selector(result.filters.suitcases);
	...
	$(document).on('change', 'input.categorycheckbox', function(e) {
		loader();
		ini_transfer_categoryf();
		filter_tour();
	});
	$(document).on('change', 'input.vehiclecheckbox', function(e) {
		loader();
		ini_transfer_vehiclef();
		filter_tour();
	});
	$(document).on('change', 'input.suitcasecheckbox', function(e) {
		loader();
		ini_transfer_suitcasef();
		filter_tour();
	});
	...
	_ini_fil['cat'] = [];
	function ini_transfer_categoryf()
	{
		_ini_fil['cat'] = $('.categorycheckbox:checked', '#transfer-category-wrapper').map(function() {
			return this.value;
		}).get();
		//alert(_ini_fil['cat']);
	}
	_ini_fil['veh'] = [];
	function ini_transfer_vehiclef()
	{
		_ini_fil['veh'] = $('.vehiclecheckbox:checked', '#transfer-vehicle-wrapper').map(function() {
			return this.value;
		}).get();
		//alert(_ini_fil['cat']);
	}
	_ini_fil['suit'] = [];
	function ini_transfer_suitcasef()
	{
		_ini_fil['suit'] = $('.suitcasecheckbox:checked', '#transfer-suitcase-wrapper').map(function() {
			return parseInt(this.value);
		}).get();
		console.log(_ini_fil['suit']);
	}
	...
	function filter_tour(_filter_trigger, _filter, attr) {
		...
		if (dynamic_page_data == false) {
			...
			$('.r-r-i').each(function(key, value) {
				...
				var _category = $('.category_code', this).val();
				var _vehicle = $('.vehicle_code', this).val();
				var _suitcase = parseInt($('.suitcase', this).val());
				...
				if (					
					...
					((_ini_fil['cat'].length == 0) || ($.inArray(_category, _ini_fil['cat']) != -1)) &&
					((_ini_fil['veh'].length == 0) || ($.inArray(_vehicle, _ini_fil['veh']) != -1)) &&
					((_ini_fil['suit'].length == 0) || ($.inArray(_suitcase, _ini_fil['suit']) != -1))
					) {
					++_fltr_r_cnt;
	...
	function enable_categories_selector(category_list){
	    ...
	    $.each(category_list, function(key, value) {
	                    ...
	                    filterList += '<input type="checkbox" value="' + key + '" name="check" class="categorycheckbox" id="squaredCategory'+index+'">';
	                    filterList += '<label for="squaredCategory'+index+'"></label>';
	                    filterList += '</div>';
	                    filterList += '<label for="squaredCategory'+index+'" class="lbllbl">' + value['v'] + ' ('+value['c']+')</label>';
	                    filterList += '</li>';
	                    index++;
	                });
	        if(filterList){
	            $('#transfer-category-wrapper').html('<ul class="locationul">' + filterList + '</ul>');
	        }
	        else{
	         	$('#category_div').addClass('hide');
       		}
	}
	function enable_vehicle_selector(vehicle_list){
		var filterList = '';
	    var index = 1;
	    $.each(vehicle_list, function(key, value) {
	                    filterList += '<li>';
	                    filterList += '<div class="squaredThree">';
	                    filterList += '<input type="checkbox" value="' + key + '" name="check" class="vehiclecheckbox" id="squaredVehicle'+index+'">';
	                    filterList += '<label for="squaredVehicle'+index+'"></label>';
	                    filterList += '</div>';
	                    filterList += '<label for="squaredVehicle'+index+'" class="lbllbl">' + value['v'] + ' ('+value['c']+')</label>';
	                    filterList += '</li>';
	                    index++;
	                });
	        if(filterList){
	            $('#transfer-vehicle-wrapper').html('<ul class="locationul">' + filterList + '</ul>');
	        }
	        else{
	         	$('#vehicle_div').addClass('hide');
       		}
	}
	function enable_suitcase_selector(suitecases){
		var filterList = '';
	    var index = 1;
	    $.each(suitecases, function(key, value) {
	                    filterList += '<li>';
	                    filterList += '<div class="squaredThree">';
	                    filterList += '<input type="checkbox" value="' + key + '" name="check" class="suitcasecheckbox" id="squaredSuitcase'+index+'">';
	                    filterList += '<label for="squaredSuitcase'+index+'"></label>';
	                    filterList += '</div>';
	                    filterList += '<label for="squaredSuitcase'+index+'" class="lbllbl">' + value['v'] + ' ('+value['c']+')</label>';
	                    filterList += '</li>';
	                    index++;
	                });
	        if(filterList){
	            $('#transfer-suitcase-wrapper').html('<ul class="locationul">' + filterList + '</ul>');
	        }
	        else{
	         	$('#suitcase_div').addClass('hide');
       		}
	}
