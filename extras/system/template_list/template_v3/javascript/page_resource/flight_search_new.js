...
function check_empty_search_result() {
	...
	function set_slider_label(val1, val2, original_minPrice = '', original_maxPrice = '') {
		$("#amount").html("<span class='filt-currency' >" + sliderCurrency + " </span>" + "<span class='filt-min_price' data-min_price='" + original_minPrice + "' >" + val1.toFixed(2) + " </span>" + " - " + "<span class='filt-currency' >" + sliderCurrency + " </span>" + "<span class='filt-max_price' data-max_price='" + original_maxPrice + "' >" + val2.toFixed(2) + " </span>");
	}
	function carousel() {
	...
	**/
	...
	function fare_breakup(id,currencySymbol,baseFare,totalTax,totalFare){
		return `<div id="fare_break_${id}" class="tab-pane i-i-s-t">
					<div class="col-12 nopad full_wher">
							...
							<div class="inboundivinr">
								<div class="rowfare">
									...
									<div class="col-4 nopad">
										<span class="pricelbl">${currencySymbol} ${baseFare.toFixed(2)}</span>
									</div>
								</div>
								<div class="rowfare">
									<div class="col-8 nopad">
									...
									<div class="col-4 nopad">
										<span class="pricelbl">${currencySymbol} ${totalTax.toFixed(2)}</span>
									</div>
								</div>
								<div class="rowfare grandtl">
									...
									<div class="col-4 nopad">
										<span class="pricelbl">${currencySymbol} ${totalFare}</span>
									</div>
								</div>
							</div>
						</div>
	...
	function baggage_inclusions(id, curSegmentDetails) {
		...
		const htmlParts = [
			`<div id="baggage_inclusions_${id}" class="baggage_inclusions tab-pane i-i-s-t">
				<div class="col-12 nopad full_wher">
					<div class="inboundiv sidefare">`
		];
	...
	function flight_detail(segmentDetail, next_seg_info, segKey) {
		...
		htmlParts.push(`
			<div class="col-8 nopad5">
				<div class="col-5 text-center nopad">
				...
				<div class="col-2 nopad">${loc_dir_icon}</div>
				<div class="col-5 text-center nopad">
		...
		htmlParts.push(`
			<div class="col-4 col-md-2 nopad5">
				<div class="ritstop">
		...
		if (segmentDetail.Baggage) {
			htmlParts.push(`<div class="termnl1 flo_w"><em><i class="fa fa-suitcase bag_icon"></i>${segmentDetail.Baggage}</em></div>`);
		}
	...
	function generate_flight(flightDetails, flightKey){
		...
		var curIsRefundable = flightDetails.Attr.IsRefundable;
		...
		if (curSegmentDetails[0][0].Baggage) {
			avilSeats += '<div class="termnl1 flo_w"><em><i class="fa fa-suitcase bag_icon"></i>' + curSegmentDetails[0][0].Baggage + '</em></div>';
		}
		...
		var gen_id = String(crs) + String(rootIndicator) + String(flightKey);
            ...
            <div class="modal-dialog" style="margin: 200px auto;">
                <div class="modal-content">
                    <div class="modal-header">
                        <button type="button" class="close" data-bs-dismiss="modal" id="close_modal_multi${emailcount}${gen_id}">&times;</button>
                    ...
                    <div class="modal-body">
                        <div class="form-group">
                            ...
                            <input type="hidden" class="form-control mfc" id="flightdetails_multi_${emailcount}${gen_id}" placeholder="Enter email" name="email" value="${btoa(JSON.stringify(flightDetails))}" />
                        ...
                        <button type="button" id="send_email_btn_not_multi${emailcount}${gen_id}" class="btn btn-secondary flteml" onclick="sendflightdetails_multi('${emailcount}${gen_id}')">Send</button>
                    </div>
                </div>
            </div>
        </div>`;
					...
					<div class="popconyent">
						<div class="contfare">
							<ul role="tablist" class="nav nav-tabs flittwifil">
								<li class="active" data-role="presentation"><a data-bs-toggle="tab" data-role="tab" href="#iti_det_${gen_id}">Itinerary</a></li>
								<li data-role="presentation"><a data-bs-toggle="tab" data-form-id="form-id-${gen_id}" class="iti-fare-btn" data-role="tab" href="#fare_det${gen_id}">Fare Details</a></li>
								<li data-role="presentation"><a data-bs-toggle="tab" class="iti-fare-btn" data-role="tab" href="#fare_break_${gen_id}">Fare Breakup</a></li>
								<li data-role="presentation"><a data-bs-toggle="tab" class="iti-fare-btn" data-role="tab" href="#baggage_inclusions_${gen_id}">Baggage & Inclusions</a></li>
							</ul>
							<div class="tab-content">
								<div id="fare_det${gen_id}" class="tab-pane Fare_Rules i-i-f-s-t">
									<div class="text-center loader-image">Please Wait</div>
									<div class="i-s-s-c tabmarg"></div>
								</div>
								<div id="iti_det_${gen_id}" class="tab-pane active i-i-s-t">
									<div class="tabmarg">
										<div class="alltwobnd">
											<div class="col-12 nopad full_wher">`;
												...
												$.each(curSegmentDetails, function(__segment_k, curSegmentDetail) {
													if (flightDetails.SegmentSummary[0].TotalStops > 0) {
														stop_air_layover.push(curSegmentDetail[0].DestinationDetails.AirportName);
													}
		...
		outerSummary += '<div class="madgrid" data-key="' + rootIndicator + flightKey + '">';
			outerSummary += '<div class="f-s-d-w col-8 nopad wayeght full_same">';
					...
					if (segment.TotalStops > 0) {
						...
						$.each(curSegmentDetails[segmentIndex], function(index, segmentD) {
							...
							if (index < total_segments - 1) {
								var stopCode = '<span data-bs-toggle="tooltip" data-placement="bottom" title="' + segmentD['DestinationDetails']['CityName'] + '">' + segmentD['DestinationDetails']['AirportCode'] + '</span> - ' +
								...
								layOvers.push('<span class="hide layover-duration" data-layoverduration="' + Math.floor(segmentD.WaitingTime) + '" data-layoverdurationhm="' + segmentD.WaitingTime + '"></span>');
							}
						});
				...
				<div class="col-4 nopad wayfour full_same">
					...
					<div class="priceanbook">
						<div class="col-6 nopad wayprice">
							<div class="insidesame">
								<div class="priceflights"><strong>${currencySymbol}</strong><span class="f-p">${single_price[0]}<small>.${single_price[1]}</small></span></div>
								<span class="hide price" data-price="${priceSingle}" data-currency="${currencySymbol}"></span>
					...
					${resultSummery.total_pax > 1 ? `<div class="col-12 trvlrs_amt_wrappr nopad"><div class="text-center trvlrs_amt">${currencySymbol} <span class="travl_amt_lrg">${t_price[0]}</span>.<span class="travl_amt_sml">${t_price[1]}</span> <span class="all_trvlrs">for all travellers</span></div></div>` : ''}
				</div>
			`;
			...
			if (typeof currencyList !== 'undefined' && Array.isArray(currencyList) && currencyList.length > 0) {
				sendmail += `<a class="detailsflt iti-btn" data-bs-toggle="modal" data-bs-target="#sendmail_multi_${emailcount}${gen_id}" data-backdrop="static"><span class="fal fa-envelope"></span> Send Mail</a>`;
			}
			...
			outerSummary += `
				<div class="clearfix"></div>
				<div class="mrinfrmtn">
					<a class="detailsflt iti-btn" data-id="fdp_${gen_id}"><span class="badge"></span><span class="fal fa-info-circle fldetail" data-bs-toggle="tooltip" title="" data-original-title="Flight Itinerary"></span>Flight Details ${remarkSeparator}</a>
	...
	function apply_filters(){ console.log('apply_filters');
		loader();
		visibility = '';
		...
		}).get();//console.log('stopCountList',stopCountList);
	...
	window.generate_flight_list = function(result,api_count){
		...
		var bookingButton;
		if (domesticRoundWayFlight && bookingSource != 'CLARITYNDC_FLIGHT_BOOKING_SOURCE') {
		...
		flightsData.forEach(function(flight, tripIndicator) {
			...
			$('.cheapest .price').html(`${sliderCurrency} ${minPrice}`);
			$('.best .price').html(`${sliderCurrency} ${bestPrice.price}`);
			$('.shortest .price').html(`${sliderCurrency} ${directPrice.price}`);
			$('.flexiable .price').html(`${sliderCurrency} ${bestPrice.price}`);
					...
					<div class="airlinesd">
						...
						<div class="alsmtrx">
							<span class="mtrxprice">
								...
								<span class="clone_side_price">${$stop01Currency}</span>
							</span>
						</div>
						<div class="one-stop-alsmtrx">
							<span class="mtrxprice">
								...
								<span class="clone_side_price">${$stop1PlusCurrency}</span>
							</span>
						</div>
					</div>
				</div>
	...
	function getMinPriceByCodeAndCat(data) {
		...
		data.forEach(item => {
			...
			if (!result[code][cat] || price < result[code][cat]) {
	...
	function loadStopFilterr(stopCountList) {
		// Cache the stop count elements
		const stopCounts = $('.stopCountWrapper .stopcount');
	
		// Cache the enabled/disabled stop categories
		// const enabledStops = new Set([0, 1]);
		const enabledStops = new Set(stopCountList);
		//const greaterThanOneExists = stopCountList.some(val => val > 1);
	
		// Loop through each stop count
		stopCounts.each(function() {
			const stopCat = parseInt($(this).val(), 10); // Convert value to integer
	
			// Check if the current stop category is in the list of enabled stops
			//if (enabledStops.has(stopCat) || (greaterThanOneExists && stopCat > 1)) {
			if (enabledStops.has(stopCat)) {
				// Enable the stop option
				$(this).closest('.stop-wrapper').removeClass('disabled').addClass('enabled').find('input').prop('disabled', false);
			} else {
				// Disable the stop option
				$(this).closest('.stop-wrapper').removeClass('enabled').addClass('disabled').find('input').prop('disabled', true);
			}
		});
	}
	...
	$(document).on('change', '.stopCountWrapper .stop-wrapper.enabled .stopcount, #departureTimeWrapper .time-wrapper.enabled .time-category, #arrivalTimeWrapper .time-wrapper.enabled .time-category', debounce(function () {
		apply_filters();
	}, 300));
