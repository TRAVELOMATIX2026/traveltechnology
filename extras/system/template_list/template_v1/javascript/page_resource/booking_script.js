$(document).ready(function() {
	...
	$('#make-payment-btn').on('click', function(){
		...
		var agent_payment_option = $('input[name=agent_payment_option]:checked').val();
		if(agent_payment_option == 'online')
		{
			balance_check = 0;
		}
		if(balance_check == 1){
	...
	*/
	...
	$('[type="submit"]').on('click', function(e) {
		...
		if (_status == false) {
			...
			var agent_payment_option = $('input[name=agent_payment_option]:checked').val();
			console.log(agent_payment_option);
			...
			var ticket_method = $("#ticket_method").val();
			var book_source = $("input[name=booking_source]").val();
			var gAuthRequired = false;
			if(book_source=='PTBSID0000000017'){
				if(ticket_method !=='hold_ticket'){
					var frm_valid = true;
	...
	*/
	...
	function booking_not_allowed_alert()
	{
		...
		modal_content += '<button type="button" class="close" data-bs-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>';
		...
		modal_content += '<button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>';
		modal_content += '</div>';
		modal_content += '</div>';
		modal_content += '</div>';
		modal_content += '</div>';
    ...
    $.ajax({
        ...
        url: app_base_url+'index.php/management/promocode',
         ...
         //dataType: "text", 
        data: {promocode: $("#code").val(), moduletype: $("#module_type").val(), total_amount_val: $("#total_amount_val").val(), convenience_fee: $("#convenience_fee").val(),email: $("#email").val(),currency_symbol: $("#currency_symbol").val(),currency: $("#currency").val(),pax_count: $("#pax_count").val(),extra_baggage:$("#extra_baggage_charge").text(),extra_meal:$("#extra_meal_charge").text(),extra_seat:$("#extra_seat_charge").text()},
	...
	$("#remove").click(function() {
	    $("#code").val('');
	    $(".promo_discount_val").html('0');
	    $("#promo_code_discount_val").val('0');
	    $("#promocode_val").val('');
	    var total_amount_val = $("#total_amount_val").val();
	    var convenience_fee = $("#convenience_fee").val();
	    var grandtotal = 0;
	    grandtotal = parseFloat(total_amount_val) + parseFloat(convenience_fee);
	    $(".grandtotal").html(grandtotal);
	    $("#total_booking_amount").html($("#total_amount_val").val());
	    $(".error_promocode").html('Promo code removed successfully').removeClass('text-danger').addClass('text-success');
	    $("#remove").css("display", "none");
	    $("#apply").css("display", "inline-block");
	    $(".promo_code_discount").addClass('hide');
	});
	window.gbuild_confirm_modal = build_confirm_modal;
});
