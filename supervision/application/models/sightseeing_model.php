	...
	function booking($condition=array(), $count=false, $offset=0, $limit=100000000000)
	{
		...
		if ($count) {
			...
			if(empty($app_reference_ids) == false) {
				...
				$cancellation_details_query = 'select * from sightseeing_cancellation_details AS SCD 
...
function b2c_sightseeing_report($condition=array(), $count=false, $offset=0, $limit=100000000000)
	{
		$condition = $this->custom_db->get_custom_condition($condition);
		...
		if ($count) {
			...
			$bd_query = 'select BD.* ,U.user_name,U.first_name,U.last_name from sightseeing_booking_details AS BD
...
function b2b_sightseeing_report($condition=array(), $count=false, $offset=0, $limit=100000000000)
	{
		$condition = $this->custom_db->get_custom_condition($condition);
	...
	function get_booking_details($app_reference, $booking_source, $booking_status='')
	{
		...
		$cancellation_details_query = 'select SCD.* from sightseeing_cancellation_details AS SCD WHERE SCD.app_reference='.$this->db->escape($app_reference);
	...
	{
		//BT, CD, ID
		$query = 'select * from sightseeing_booking_details where domain_origin='.get_domain_auth_id();
	...
	{
		$query = 'select count(*) AS total_search, MONTH(created_datetime) as month_number from search_sightseen_history where
		(YEAR(created_datetime) BETWEEN '.$year_start.' AND '.$year_end.') AND domain_origin='.get_domain_auth_id().' 
		AND search_type="'.META_SIGHTSEEING_COURSE.'"
	...
	{
		$query = 'select count(*) AS total_search, CONCAT(destination_name) label from search_sightseen_history where
		(YEAR(created_datetime) BETWEEN '.$year_start.' AND '.$year_end.') AND domain_origin='.get_domain_auth_id().' 
		AND search_type="'.META_SIGHTSEEING_COURSE.'"
