...
function b2c_hotel_report($condition=array(), $count=false, $offset=0, $limit=100000000000)
	{
		...
		if ($count) {
			$query = 'select count(distinct(BD.app_reference)) as total_records
					from hotel_booking_details BD
					left join hotel_booking_itinerary_details AS HBID on BD.app_reference=HBID.app_reference
					left join payment_option_list AS POL on BD.payment_mode=POL.payment_category_code
			...
			$bd_query = 'select BD.* ,U.user_name,U.first_name,U.last_name,BS.name as api_source_name from hotel_booking_details AS BD
...
function b2b_hotel_report($condition=array(), $count=false, $offset=0, $limit=100000000000)
	{
		...
		if ($count) {
			...
			$booking_customer_details	= array();
			$cancellation_details = array();
			$bd_query = 'select BD.* ,U.agency_name,U.first_name,U.last_name,BS.name as api_source_name from hotel_booking_details AS BD
					     left join user U on BD.created_by_id =U.user_id
						 left join booking_source BS on BS.source_id = BD.booking_source 					     
	...
	public function unserialize_temp_booking_record($book_id){
		$temp_booking_details = $this->custom_db->single_table_records('temp_booking', 'domain_list_fk, booking_source, book_id, book_attributes', array('book_id' => $book_id));
	...
	public function get_hotel_source_options()
	{
		$this->db->select('name, source_id');
		$this->db->where('meta_course_list_id', META_ACCOMODATION_COURSE);
