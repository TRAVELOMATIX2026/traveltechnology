...
class Module_Model extends CI_Model
{
	...
	function update_top_destination($status, $origin)
	{
		...
		$this->db->update('hotel_top_destinations', $data);
	}
	function update_activity_top_destination($status, $origin)
	{
		$data = array(
			'top_destination' => $status
		);
		$this->db->where('origin', $origin);
		$this->db->update('hb_activity_destinations', $data);
	}
	function update_activity_top_destination_old($status, $origin)
	{
		$data = array(
			'top_destination' => $status
		);
		$this->db->where('origin', $origin);
		$this->db->update('api_sightseeing_destination_list', $data);
	}
	/**
	...
	function get_course_list($condition = '')
	{
		...
		$filter_condition .= ' ASM.domain_origin=' . intval(get_domain_auth_id()) . ' and MCL.status = "1" and BS.booking_engine_status="1"  and ';
		//		}
		...
		group_concat(BS.name separator "' . DB_SAFE_SEPARATOR . '") as booking_source, group_concat(BS.origin separator "' . DB_SAFE_SEPARATOR . '") as bs_origin, group_concat(concat(BS.name , " ", ASM.status) separator "' . DB_SAFE_SEPARATOR . '") as asm_status_label, group_concat(BS.source_id separator "' . DB_SAFE_SEPARATOR . '") as booking_source_id, group_concat(ASM.status separator "' . DB_SAFE_SEPARATOR . '") as asm_status FROM meta_course_list AS MCL JOIN user AS U ON MCL.created_by_id=U.user_id LEFT JOIN activity_source_map ASM ON ASM.meta_course_list_fk=MCL.origin LEFT JOIN booking_source BS ON BS.origin=ASM.booking_source_fk ' . $filter_condition . '  GROUP BY MCL.course_id order by MCL.priority_number';
		//  echo $query;exit;
		return $this->db->query($query)->result_array();
	}
	/**
	...
	public function promocode_module_options()
	{
		...
		$inactive_module = array(META_PACKAGE_COURSE,META_FLIGHT_HOTEL_COURSE);
		//debug($inactive_module);exit;
		...
		//$promocode_module_options['new_register'] = 'New Register';
