...
/**
	...
	*/
	...
	{
		$data = array();
		if (is_array($condition) && isset($condition['all']) == true) {
			$order = ' order by U.agency_name asc';
		} else {
			$order = ' order by TL.created_datetime desc';
		}
		$condition = $this->custom_db->get_custom_condition($condition);
		$agent_filter = '';
		...
		if($count){
			...
			transaction_log TL 
			join user U on U.user_id = TL.transaction_owner_id 
			where 1=1 and date(TL.created_datetime)>= "'.$transaction_activated_from_date.'" '.$agent_filter.' '.$condition.$order.' limit '.$offset.', '.$limit;
		  //echo $query;
		 //exit;
