...
Class User_Model extends CI_Model
{
	...
	public function get_user_details($condition=array(), $count=false, $offset=0, $limit=10000000000, $order_by=array())
	{
		...
		if (valid_array($condition) == true) {
			...
			if(isset($condition[0]) == true && count($condition[0])>1)
			{
			...
			{
				$filter_condition .= 'user_id='.$condition['user_id'].' and ';
			}
			
		}
		//debug($filter_condition); exit;
	...
	function get_domain_user_list($condition=array(), $count=false, $offset=0, $limit=10000000000, $order_by=array())
	{
		...
		if (!$count) {
			...
			MAX(LM.login_date_time) as last_login,CASE WHEN EXISTS (SELECT 1 FROM login_manager l WHERE l.user_id = U.uuid AND l.logout_date_time IS NULL) THEN 0 ELSE MAX(LM.logout_date_time) END AS logout_date_time,
			CL.destination as city_name, SL.state_province as state_name, ACL.name as country_name, b2b_a.c_certificate, b2b_a.c_gst, b2b_a.c_owner_proof, b2b_a.c_address_proof
			...
			LEFT JOIN api_state_list as SL on SL.origin=U.state 
	...
	function get_logged_in_users($condition=array(), $count=false, $offset=0, $limit=10000000000)
	{
		...
		$where = ' LM.origin=(select max(ml.origin) from login_manager as ml where ml.user_id=U.uuid) ';
		//$where = $logged ? ($ids ? ' U.user_id NOT IN('.(implode(',',$ids)).') AND ' : '')." DATE(LM.logout_date_time) = CURDATE() " : " LM.logout_date_time IS NULL ";
		//$where =" (DATE(LM.logout_date_time) = CURDATE() OR LM.logout_date_time IS NULL) ";
		$filter_condition .=" DATE(LM.login_date_time) = CURDATE() ";
		//$filter_condition = rtrim($filter_condition, 'and ');
		if (!$count) {

			return $this->db->query('SELECT U.*, UT.user_type, max(LM.login_date_time) as login_time,LM.logout_date_time as logout_time,LM.login_ip
			FROM user AS U
			JOIN user_type AS UT ON U.user_type=UT.origin
			LEFT JOIN api_country_list AS ACL ON U.country_code=ACL.origin
			JOIN login_manager AS LM ON U.user_type=LM.user_type and U.uuid=LM.user_id
			WHERE '.$where.' '.$filter_condition.' group by U.user_id order by LM.logout_date_time IS NULL DESC, LM.logout_date_time desc limit '.$limit.' offset '.$offset)->result_array();
			...
			JOIN login_manager AS LM ON U.user_type=LM.user_type and U.uuid=LM.user_id
		    WHERE '.$where.' '.$filter_condition.' group by U.user_id')->row();
	...
	function create_login_auth_record($user_id, $user_type, $user_origin=0, $username='customer')
	{
		...
		//logout all users which doesn't active from the last 24 hours
		$this->db->set('logout_date_time', date('Y-m-d H:i:s'));
		$this->db->where('logout_date_time IS NULL');
		$this->db->where('login_date_time <', 'NOW() - INTERVAL 1 DAY', false);
		$this->db->update('login_manager');
...
// echo $query;exit;
	...
	function get_agent_privilage_list($user_id, $filter_text = '') {
		$filter_condition = $filter_text;
		$query = 'SELECT PL.*, P.origin AS pl FROM `agent_privilege_list` AS PL LEFT JOIN `agent_privileges` AS P ON P.p_no = PL.origin AND P.user_id = ' . $user_id . $filter_condition . ' ORDER BY PL.p_no';
		// echo $query;exit;
		//ALTER TABLE `privilege_list` ADD `p_no` INT(12) NOT NULL DEFAULT '0' AFTER `description`;
		return $this->db->query ( $query )->result_array ();
	}

	function edit_agent_user_privileges($user_id, $privileges_origin) {
		if (is_array ( $privileges_origin ) == false) {
			$privileges_origin = ( array ) $privileges_origin;
		}
		
		$previlages ['user_id'] = intval ( $user_id );
		$previlages ['user_type'] = 3;
		if ($this->custom_db->delete_record ( 'agent_privileges', array (
				'user_id' => intval ( $user_id ) 
		) ) == QUERY_SUCCESS) {
			foreach ( $privileges_origin as $k => $v ) {
				$previlages ['p_no'] = intval ( $v );
				$this->custom_db->insert_record ( 'agent_privileges', $previlages );
			}
		}
	}
	 ...
	 * 	Get Currencies
	 */
	function get_currency_user_creation($from=array('k' => 'id', 'v' => 'country'), $condition=array('country !=' => ''))
	{
		$this->db->select('*');
		$this->db->from('currency_converter');
		$this->db->where_in('country', ['PHP','AUD','CAD','EUR','GBP','USD']);
		...
		if ($result) {
			...
			foreach ($result as $item) {
				// Create a new array with 'k' as 'id' and 'v' as 'country'
				$transformed[] = [
					'k' => $item['id'],
					'v' => $item['country']
				];
