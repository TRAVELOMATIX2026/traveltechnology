...

{
...

	{
		...
		if ($count) {
			...
			$bd_query = 'select BD.* ,U.user_name,U.first_name,U.last_name, BS.name as api_source_name from flight_booking_details AS BD
...

	{
		...
		if ($count) {
			...
			$bd_query = 'select BD.*,U.agency_name,U.first_name,U.last_name, BS.name as api_source_name from flight_booking_details AS BD
...

	{
		...
		where (YEAR(BD.created_datetime) BETWEEN '.date('Y').' AND '.date('Y', strtotime('+1 year')).') AND BD.domain_origin='.get_domain_auth_id().' AND BD.status = "BOOKING_CONFIRMED"
		...
		if ($booking_details['status'] == SUCCESS_STATUS && $response['status'] == SUCCESS_STATUS) {
			...
			switch($booking_source){
				...
				case CLARITYNDC_FLIGHT_BOOKING_SOURCE:
					...
					//$booking_rec = $booking_details['data']['booking_details'][0];
					...
					if($OrderStatus == 'CANCELED' || $TicketStatus == 'VOIDED'){
						...
						if($booking_detail && in_array($booking_detail['status'],array('BOOKING_HOLD','BOOKING_FAILED'))){
	...
	public function get_flight_source_options()
	{
		$this->db->select('name, source_id');
		$this->db->where('meta_course_list_id', META_AIRLINE_COURSE);
		$res = $this->db->get('booking_source');
	...
	function daily_search_history($year, $month)
	{
	    $query = 'SELECT COUNT(*) AS total_search, DAY(created_datetime) AS day_number 
	              FROM search_flight_history 
	              WHERE YEAR(created_datetime) = ' . $year . ' 
	              AND MONTH(created_datetime) = ' . $month . ' 
	              AND domain_origin=' . get_domain_auth_id() . ' 
	              AND search_type="' . META_AIRLINE_COURSE . '" 
