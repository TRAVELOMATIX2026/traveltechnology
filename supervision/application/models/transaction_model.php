...
Class Transaction_Model extends CI_Model
{
	...
	{	
		...
		if (is_array($condition) && isset($condition['all']) && $condition['all'] == true) {
			$order = ' order by agent_name asc';
		} else {
			$order = ' order by TL.origin desc';
		}
		...
		if (is_domain_user()) {
			if ($count) {
				...
				$query = 'select TL.currency, TL.system_transaction_id,TL.transaction_type,TL.domain_origin,TL.app_reference,
				...
				TL.remarks,TL.created_datetime,TL.transaction_owner_id, concat(U.first_name, " ", U.last_name) as username, CASE WHEN agency_name IS NULL OR agency_name = "" THEN "B2C" ELSE agency_name END AS agent_name from transaction_log TL LEFT JOIN user U ON TL.transaction_owner_id=U.user_id where TL.domain_origin='.get_domain_auth_id().' '.$condition.$order.' limit '.$offset.', '.$limit;
			...
			if ($count) {
				...
				TL.fare,TL.domain_markup as admin_markup,TL.domain_markup as profit,TL.level_one_markup as agent_markup,TL.convinence_fees as convinence_amount,TL.promocode_discount as discount,
				TL.remarks,TL.created_datetime, concat(U.first_name, " ", U.last_name) as username, CASE WHEN agency_name IS NULL OR agency_name = "" THEN "B2C" ELSE agency_name END AS agent_name from transaction_log TL LEFT JOIN user U ON TL.transaction_owner_id=U.user_id where 1=1 '.$condition.$order.' limit '.$offset.', '.$limit;
				return $this->db->query($query)->result_array();
			}
		}
	}
