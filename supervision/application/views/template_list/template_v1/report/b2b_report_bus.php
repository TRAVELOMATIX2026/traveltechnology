            ...
            <div id="show-search">
                ...
                <form method="GET" autocomplete="off">
                    ...
                    <div class="form-group row gap-0 mb-0">
                        ...
                        <!-- <a href="<?php echo base_url() . 'index.php/report/b2b_bus_report? ' ?>" id="clear-filter" class="btn btn-gradient-info"><i class="fa fa-times"></i> Clear Filter</a> -->
...
<div class="clearfix table-responsive reprt_tble"><!-- PANEL BODY START -->
            ...
            <table class="table table-sm table-bordered example3" id="b2b_report_bus_table">
						<thead>
							<tr>
								<th>Sno</th>
								<th>Agency_name</th>
						...
						<?php
							if (isset($table_data) == true and valid_array($table_data['booking_details']) == true) {
								...
								foreach($booking_details as $parent_k => $parent_v) {
									...
									$action.='<br/>';
									$action .= bus_GST_Invoice($app_reference, $booking_source, $status, 'b2b');							
									$action.='<br/>';
									if($diff > 0){
								...
								?>
									<tr>
										...
										<td><?php echo $fare?></td>
										<td><?php echo $fare-$admin_commission+$admin_tds?></td>
...
function advanced_search_form($from_date, $to_date)
{
	...
	$advanced_search_form .= '<div class="card-header">Advanced Search'; 
	$advanced_search_form .= ' <button class="btn btn-sm btn-info collapsed show_form" id="advance_search_btn_label" data-bs-toggle="collapse" href="#advance_search_form_container" aria-expanded="false" aria-controls="advance_search_form_container">+</button>';
	$advanced_search_form .= '</div>';
	$advanced_search_form .= '<div class="card-body collapse collapse" id="advance_search_form_container"  role="tabpanel" >';
	$advanced_search_form .= '<form role="form" class="row" autocomplete="off" method="get" action="'.base_url().'report/'.$uri_function.'">
							        <div class="form-group">
							            <label class="col-sm-1 form-label"> From </label>
							            <div class="col-sm-2">
							                <input type="text" value="'.$from_date.'" placeholder="From Date" name="from_date" class="form-control" id="from_date">
							            </div>
							            <label class="col-sm-1 form-label"> To </label>
							            <div class="col-sm-2">
...
function bus_voucher_email($app_reference, $booking_source,$status,$recipient_email)
{
	...
	return '<a class="btn btn-sm btn-primary send_email_voucher" data-app-status="'.$status.'"   data-app-reference="'.$app_reference.'" data-booking-source="'.$booking_source.'"data-recipient_email="'.$recipient_email.'"><i class="bi bi-envelope-fill"></i> Email Ticket</a>';
}
function get_cancellation_details_button($app_reference, $booking_source, $status)
{
	if($status == 'BOOKING_CANCELLED'){
		return '<a target="_blank" href="'.base_url().'bus/cancellation_refund_details?app_reference='.$app_reference.'&booking_source='.$booking_source.'&status='.$status.'" class="col-md-12 btn btn-sm btn-info "><i class="bi bi-info-circle-fill"></i> Cancellation Details</a>';
	}
}
