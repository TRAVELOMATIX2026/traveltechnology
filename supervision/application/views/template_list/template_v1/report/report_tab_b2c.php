...
$module_icons = array(
	...
	'activities' => 'bi bi-camera-reels'
);

foreach ($master_module_list as $k => $v) {
	if (in_array ( $k, $active_domain_modules ) && $v != 'package') {
		// echo $v;
		...
		$icon = isset($module_icons[$v]) ? $module_icons[$v] : 'bi bi-file-earmark-text';
	?>
		<li role="presentation" class="<?=$act_tab?>"><a href="<?=base_url()?>index.php/report/b2c_<?=($v).'_report'?><?= $append_query_string ?>"><i class="<?=$icon?>"></i> <?='B2C '.ucfirst($v).' Report'?></a></li>
	<?php
	}
}
