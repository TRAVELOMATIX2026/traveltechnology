            ...
            <div id="show-search">
                ...
                <form method="GET" autocomplete="off" action="<?= base_url() . 'report/b2b_flight_report?' ?>">
                    ...
                    <div class="form-group row gap-0 mb-0">
                        ...
                        <!-- <a href="<?= base_url() . 'index.php/report/b2b_flight_report? ' ?>" id="clear-filter" class="btn btn-gradient-info"><i class="fa fa-times"></i> Clear Filter</a> -->
...
<div class="clearfix table-responsive reprt_tble"><!-- PANEL BODY START -->
            ...
            <table class="table table-sm table-bordered example3" id="b2b_report_airline_table">
                <thead>
                    <tr>
                        <th>Sno</th>
                        <th>Source</th>
                ...
                <tfoot>
                    ...
                    if (valid_array($table_data['booking_details']) == true) {
                        ...
                        foreach ($booking_details as $parent_k => $parent_v) {
                            ...
                            if($parent_v['booking_source'] != CLARITYNDC_FLIGHT_BOOKING_SOURCE){
                            ...
                            <tr>
                                ...
                                <td><span class="<?php echo booking_status_label($status) ?>"><?php echo $status ?></span><?php echo (($status == 'BOOKING_HOLD' && $booking_transaction_details[0]['hold_ticket_valid_date']) ? '<br/><br/><span class="text-danger text-bold">'.$booking_transaction_details[0]['hold_ticket_valid_date'].'</span>':'' );?></td>
                                <td><?= @$pnr ?></td>
                        ...
                        ?>
                        <tr>
                            <td colspan="25" class="text-center no-data-found">
...
function flight_voucher_email($app_reference, $booking_source, $status, $recipient_email) {
    return '<a class="btn btn-sm btn-info send_email_voucher" data-app-status="' . $status . '"   data-app-reference="' . $app_reference . '" data-booking-source="' . $booking_source . '"data-recipient_email="' . $recipient_email . '"><i class="far fa-envelope"></i> Email Voucher</a>';
}
...
function customer_details($app_reference, $booking_source = '', $status = '') {
    return '<a  target="_blank" data-app-reference="' . $app_reference . '" data-booking-status="' . $status . '" data-booking-source="' . $booking_source . '" class="btn btn-sm btn-primary flight_u customer_details"><i class="far fa-file"></i> Pax profile</a>';
}
function error_details($app_reference, $booking_source = '', $status = '') {
    return '<a data-app-reference="' . $app_reference . '" data-booking_source="' . $booking_source . '" data-status="' . $master_booking_status . '" class="error_log btn btn-sm btn-danger "><i class="fa fa-exclamation"></i> <small>ErroLog</small></a>';
}
?>
