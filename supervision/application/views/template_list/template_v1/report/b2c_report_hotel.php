            ...
            <div id="show-search">
                ...
                <form method="GET" autocomplete="off">
                    ...
                    <div class="form-group row gap-0 mb-0">
                        ...
                        <!-- <a href="<?php echo base_url() . 'index.php/report/b2c_hotel_report? ' ?>" id="clear-filter" class="btn btn-gradient-info"><i class="fa fa-times"></i> Clear Filter</a> -->
...
{
	$report_data = '';
	$report_data .= '<table class="table table-sm table-bordered example3" id="b2c_report_hotel_table">
		<thead>
		<tr>
			<th>Sno</th>
			<th>Source</th>
		...
		if (isset($table_data) == true and valid_array($table_data['booking_details']) == true) {
			$segment_3 = $GLOBALS['CI']->uri->segment(3);//debug($segment_3);exit;
		    ...
		    foreach($booking_details as $parent_k => $parent_v) { 
                ...
                {
                	$action .= confirm_hotel_booking($app_reference, $booking_source,$status,'b2c');
		...
		$report_data .= '<tr>
					...
					<td>'.date('d-m-Y H:i:s', strtotime($created_datetime)).'</td>
