...
<div class="clearfix table-responsive reprt_tble"><!-- PANEL BODY START -->
            ...
            <table class="table table-sm table-bordered example3" id="b2b_report_transfer_table">
						<thead>
							<tr>
								<th>Sno</th>
						...
						<?php
							...
							if(valid_array($table_data['booking_details']) == true) {
					        	...
					        	foreach($booking_details as $parent_k => $parent_v) {
									...
									$email_btn = transfer_voucher_email($app_reference, $booking_source,$status,$email);
								...
								?>
									<tr>
										...
										<td><?php echo ($adult_count.'+'.$child_count);?></td>
										<td><?php echo $trip_type;?></td>
										<td><?php echo $itinerary_details[0]['from_location'];?></td>
										<td><?php echo $itinerary_details[0]['to_location'];?></td>
...
function get_cancellation_details_button($app_reference, $booking_source, $master_booking_status)
{
	...
	if($master_booking_status == 'BOOKING_CANCELLED'){
		return '<a target="_blank" href="'.base_url().'index.php/transfer/ticket_cancellation_details?app_reference='.$app_reference.'&booking_source='.$booking_source.'&status='.$master_booking_status.'" class="col-md-12 btn btn-sm btn-info "><i class="bi bi-info-circle-fill"></i> Cancellation Details</a>';
	}
...
$(document).ready(function() {
        ...
        var _opp_url = app_base_url+'index.php/transfer/get_pending_booking_status/';
