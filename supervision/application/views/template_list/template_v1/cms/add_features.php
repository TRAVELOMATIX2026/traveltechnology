<?php
   ...
   // debug($get_data);exit;
 	if(isset($get_data) && valid_array($get_data)){
 		$action = base_url().'index.php/cms/add_features?&origin='.$get_data['origin'];
 	}
 	else{
 		$action = base_url().'index.php/cms/add_features';
 	}
 	// echo $action;exit;
?>
<!-- HTML BEGIN -->
...
<ul class="nav nav-tabs" role="tablist" id="myTab">
	...
	<li role="presentation" class="<?=$tab1?>">
		<a id="fromListHead" href="#fromList" aria-controls="home" role="tab"	data-bs-toggle="tab">Manage Features</a>
	</li>
      ...
      <div class="card-body">
         <form class="row" role="form" enctype="multipart/form-data" method="POST" action="<?= $action; ?>" autocomplete="off" name="home_page_heading">
            <fieldset form="promo_codes_form_edit">
               <legend class="form_legend">Add Feature</legend>
               ...
               <div class="form-group">
                  <label form="promo_codes_form_edit" for="header_title" class="col-sm-3 form-label">Description</label>
                     <div class="col-sm-6">
                        <textarea  id="description" class="form-control" name="description" required><?php echo @$description?></textarea>
                     </div>
               </div>
              	<div class="form-group">
