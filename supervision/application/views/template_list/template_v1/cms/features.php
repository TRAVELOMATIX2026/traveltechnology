...
<div class="bodyContent">
	<div class="row">
		<div class="float-start" style="margin:5px 0">
			<a href="<?=base_url().'index.php/cms/add_features'?>">
				<button  class="btn btn-primary btn-sm float-end amarg">Add Feature</button>
			</a>
		</div>
	</div>
			...
			<table class="table table-sm">
				...
				?>
					<tr>
						...
						<a href="<?php echo base_url(); ?>cms/delete_features/<?php echo $v['origin']; ?>/<?php echo $v['origin']; ?>"  data-original-title="Delete"  onclick="return confirm('Do you want delete this record');" class="btn btn-danger btn-xs has-tooltip" data-original-title="Delete"> 
...
{
	return '<a role="button" href="'.base_url().'index.php/cms/add_features?'.$_SERVER['QUERY_STRING'].'&	origin='.$origin.'" class="btn btn-sm btn-primary"><i class="fa fa-edit"></i>
...
$(document).ready(function() {
	$('.toggle-user-status').on('change', function(e) {
		...
		if (parseInt(_user_status) == 1) {
			_opp_url = _opp_url+'activate_features/';
		} else {
			_opp_url = _opp_url+'deactivate_features/';
		}
