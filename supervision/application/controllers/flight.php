				...
				if($booked_user_details[0]['user_type'] == B2B_USER){
					...
					switch($booking_source){
						...
						case CLARITYNDC_FLIGHT_BOOKING_SOURCE:
							$currency_conversion_rate = $currency_obj->getConversionRate(false, get_application_default_currency(), 'CAD');//Currency conversion rate of the domain currency
							// debug($currency_conversion_rate);
							$payment_amount = ($amount[0]*$currency_conversion_rate);
							$payment_amount = ceil($payment_amount * 100) / 100;
							debug($payment_amount);exit;
							$ticket_response = $this->flight_lib->issue_hold_ticket($app_reference,$sequence_number,$pnr,$booking_id);
