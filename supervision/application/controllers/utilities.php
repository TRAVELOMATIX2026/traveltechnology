...
class Utilities extends MY_Controller {
	...
	function toggle_asm_in_status($bs_id, $mc_id, $status = false) {
		$list_data = $this->module_model->get_course_list ( array (
				array (
						'BS.origin',
						'=',
	...
	function currency_converter($value = 0, $id = 0) {
		if (intval ( $id ) > 0 && intval ( $value ) > - 1) {
			$cData = $this->custom_db->single_table_records ( 'currency_converter', '*',['id' => $id]);
			if($cData['status'] == SUCCESS_STATUS){
				$toCur = $cData['data'][0]['country'];
				$curRequest['curFrom'] = 'INR';
				$curRequest['curTo'] = $toCur;
				$curRequest['amount'] = 1;
				$response = $this->api_interface->xeMultiCurrency($curRequest);
				$response = json_decode($response, true);
				if($response['code'] != 9){

					if (!empty($response)) {
						if ($response['to'][0]['quotecurrency'] == $toCur ){
							$updateData['value'] = $response['to'][0]['mid'];
							$updateData['date_time'] = date('Y-m-d h:m:s');
							$this->custom_db->update_record('currency_converter', $updateData, array('id' => $id));
						}
					}
				}
	...
	function country_access($value = 0, $id = 0) {
		if (intval ( $id ) > 0 && intval ( $value ) > - 1) {
			$data ['value'] = $value;
			$this->custom_db->update_record ( 'access_country_master', $data, array ( 'id' => $id ) );
		} else {
			$currency_data = $this->custom_db->single_table_records ( 'access_country_master' );
			$data ['converter'] = $currency_data ['data'];
			$this->template->view ( 'utilities/country_access', $data );
		}
	}
	
	function country_access_status_toggle($id = 0, $status = ACTIVE) {
		if (intval ( $id ) > 0) {
			$data ['access'] = $status;
			$this->custom_db->update_record ( 'access_country_master', $data, array (
					'origin' => $id 
			) );
		}
	}
	...
	function manage_promo_code($offset = 0) 
	{
		...
		if(isset($get_data['eid']) == true && intval($get_data['eid']) > 0 && valid_array($post_data ) == false) {
			...
			if ($this->form_validation->run ()) {
				...
				$promo_code_list['alt_text'] = trim($post_data['alt_text']);
		...
		//$page_data['promocode_module_options']['new_register'] = 'New Register';
		// debug($page_data['promocode_module_options']);exit; 
	...
	function not_access(){
		...
		//// Countrys Master Management ////
	public function update_airport_international_status($origin,$status){
		$condition['origin'] = $origin;
		$data['IsInternational'] = $status;
		$this->db->update('flight_airport_list', $data, $condition);
		redirect(base_url().'utilities/country_list_master');
	}

	public function update_airport_traveller_status($origin,$status){
		$condition['origin'] = $origin;
		$data['isTravellerAirport'] = $status;
		$this->db->update('flight_airport_list', $data, $condition);
		redirect(base_url().'utilities/country_list_master');
	}

	public function update_airport_status($origin,$status){
		$condition['origin'] = $origin;
		$data['status'] = $status;
		$this->db->update('flight_airport_list', $data, $condition);
		redirect(base_url().'utilities/country_list_master');
	...
	public function country_list_master($offset=0){
		$page_data['flight_airport_lists'] = $this->custom_db->single_table_records('flight_airport_list', '*', array(),$offset,RECORDS_RANGE_3,array());
		$total_records = $this->custom_db->get_custom_query('select count(*) as total from flight_airport_list');
		$this->load->library('pagination');
		$config['base_url'] = base_url() . 'index.php/utilities/country_list_master/';
		$page_data['total_rows'] = $config['total_rows'] = $total_records[0]['total'];
		$config['per_page'] = RECORDS_RANGE_3;
		$this->pagination->initialize($config);
		/** TABLE PAGINATION */
		$page_data['total_records'] = $config['total_rows'];
		$this->template->view('utilities/country_list_master', $page_data);
	}
	public function add_airport_master(){
		$page_data['flight_airport_lists'] = $this->custom_db->single_table_records('flight_airport_list', '*', array('origin'=>$origin),0,50,array());
		$this->template->view('utilities/add_airport_master', $page_data);
	}

	public function edit_airport_master($origin){
		$page_data['flight_airport_list'] = $this->custom_db->single_table_records('flight_airport_list', '*', array('origin'=>$origin),0,50,array());
		$this->template->view('utilities/edit_airport_master', $page_data);
	}

	public function update_airport_master($origin){
		$condition['origin'] = $origin;
		$data['airport_code'] = $this->input->post('airport_code');
		$data['airport_name'] = $this->input->post('airport_name');
		$data['airport_city'] = $this->input->post('airport_city');
		...
		if(isset($_POST['top_destination'])){
			$data['top_destination'] = $this->input->post('top_destination');
		}else{
			$data['top_destination'] = 0;
		}

		if(isset($_POST['IsInternational'])){
			$data['IsInternational'] = $this->input->post('IsInternational');
		}else{
			$data['IsInternational'] = 0;
		}

		if(isset($_POST['isTravellerAirport'])){
			$data['isTravellerAirport'] = $this->input->post('isTravellerAirport');
		}else{
			$data['isTravellerAirport'] = 0;
		}
	...
	public function save_airport_master(){
		$data['airport_code'] = $this->input->post('airport_code');
		$data['airport_name'] = $this->input->post('airport_name');
		$data['airport_city'] = $this->input->post('airport_city');
		$data['country'] = $this->input->post('country');
		...
		$this->db->insert('flight_airport_list', $data);
		redirect(base_url().'utilities/country_list_master');
	}
}
