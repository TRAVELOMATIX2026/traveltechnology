...
/**
	...
	public function update_refund_details()
	{
		...
		if ($this->form_validation->run()) {
			...
			if($booking_details['status'] == SUCCESS_STATUS){
				...
				if($refund_status == 'PROCESSED' && floatval($refund_amount) > 0 && $is_agent == true){
					...
					$this->domain_management_model->save_transaction_details('transfer', $app_reference, $fare, $domain_markup, $level_one_markup, $remarks, $convinence, $discount, $booking_currency, $currency_conversion_rate, $booked_user_id);
				...
				#exit;
				$this->custom_db->update_record('transfer_cancellation_details', $update_refund_details, $update_refund_condition);
		...
		redirect('transfer/cancellation_refund_details?'.http_build_query($redirect_url_params));
	}
	/**
	 * Elavarasi
	...
	public function update_supplier_cancellation_status_details()
	{
		...
		if(isset($get_data['app_reference']) == true && isset($get_data['booking_source']) == true && isset($get_data['status']) == true && $get_data['status'] == 'BOOKING_CANCELLED'){
			...
			if($booking_details['status'] == SUCCESS_STATUS){
				...
				$response = $this->transfer_lib->get_cancellation_refund_details($ChangeRequestId, $app_reference);			
			
				
				if($response['status'] == SUCCESS_STATUS){
					$cancellation_details = $response['data'];

					$this->transfer_model->update_cancellation_refund_details($app_reference, $cancellation_details);
	...
	*/
	function get_pending_booking_status($app_reference,$booking_source,$status){
		...
		if($status=='BOOKING_HOLD'){
			...
			if($booking_details['status']==1){
				...
				load_transfer_lib($booking_source);
				$hold_booking_status = $this->transfer_lib->get_sightseeing_booking_status($app_reference);
	...
	public function ticket_cancellation_details()
	{
		...
		if(isset($get_data['app_reference']) == true && isset($get_data['booking_source']) == true && isset($get_data['status']) == true){
			...
			if($booking_details['status'] == SUCCESS_STATUS){
				$this->load->library('booking_data_formatter');
				$booking_details = $this->booking_data_formatter->format_transfer_booking_data($booking_details, $this->config->item('current_module'));
				$page_data = array();
				...
				$this->template->view('transfer/ticket_cancellation_details', $page_data);
			} else {
				redirect(base_url());
			}
		} else {
