...
class Menu extends MY_Controller {
	...
	function index()
	{
		if (web_page_access_privilege('p1')) {
			...
			$hotel_earning = $flight_earning = $bus_earning = $sightseeing_earning = $transfers_earning= $car_earning =  array();
			...
			if (!empty($active_car)) {
				...
				//debug($tmp_car_booking);exit;
				$month_index_car = index_month_number($tmp_car_booking);
				//debug($month_index_car);exit;
			}
			if (!empty($active_transfers)) {
				...
				$tmp_transfers_booking = $this->transfer_model->get_monthly_booking_summary();
			...
			foreach ($time_line_interval as $k => $v) {
				if (!empty($active_hotel)) {
					if (isset($month_index_hotel[$k])) {
						...
						$hotel_earning[$k] = round($month_index_hotel[$k]['monthly_payment']);
					} else {
				...
				if (!empty($active_airline)) {
					if (isset($month_index_flight[$k])) {
						...
						$flight_earning[$k] = round($month_index_flight[$k]['monthly_payment']);
					} else {
				...
				if (!empty($active_bus)) {
					if (isset($month_index_bus[$k])) {
						...
						$bus_earning[$k] = round($month_index_bus[$k]['monthly_payment']);
					} else {
				...
				if (!empty($active_sightseeing)) {
					...
					if (isset($month_index_sightseeing[$k])) {
						...
						$sightseeing_earning[$k] = round($month_index_sightseeing[$k]['monthly_payment']);
					} else {
				...
				if (!empty($active_transfers)) {
					...
					if (isset($month_index_transfers[$k])) {
						...
						$transfers_earning[$k] = round($month_index_transfers[$k]['monthly_payment']);
					} else {
			...
			if (!empty($active_transfers)) {
				$page_data['transfers_booking_count'] = $this->transfer_model->booking($condition, true);
			}

			$condition = array();
			$latest_transaction = $this->transaction_model->logs($condition, false, 0, 5);
			//debug($latest_transaction);
