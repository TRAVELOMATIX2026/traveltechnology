 ...
 * @author    Elavarasi<elavarasi.k@provabmail.com>
 * @version    V1
 */

class Transferv1 extends MY_Controller 
{
...
/**
	...
	function cancel_booking($app_reference, $booking_source)
	{
		if(empty($app_reference) == false) {
			...
			if ($master_booking_details['status'] == SUCCESS_STATUS) {
				$this->load->library('booking_data_formatter');
				$master_booking_details = $this->booking_data_formatter->format_transferv1_booking_data($master_booking_details, 'b2c');
	...
	public function update_refund_details()
	{
		...
		if ($this->form_validation->run()) {
			...
			if($booking_details['status'] == SUCCESS_STATUS){
				...
				if($refund_status == 'PROCESSED' && floatval($refund_amount) > 0 && $is_agent == true){
					...
					$convinence = 0;
					$discount = 0;
					$remarks = 'Transfers Refund was Successfully done';
					$this->domain_management_model->save_transaction_details('transferv1', $app_reference, $fare, $domain_markup, $level_one_markup, $remarks, $convinence, $discount, $booking_currency, $currency_conversion_rate, $booked_user_id);
				...
				#debug($update_refund_details);
				#exit;
				$this->custom_db->update_record('transferv1_cancellation_details', $update_refund_details, $update_refund_condition);
		...
		redirect('transferv1/cancellation_refund_details?'.http_build_query($redirect_url_params));
	}
	/**
	 * Elavarasi
	...
	public function update_supplier_cancellation_status_details()
	{
		...
		if(isset($get_data['app_reference']) == true && isset($get_data['booking_source']) == true && isset($get_data['status']) == true && $get_data['status'] == 'BOOKING_CANCELLED'){
			...
			if($booking_details['status'] == SUCCESS_STATUS){
				...
				$response = $this->transferv1_lib->get_cancellation_refund_details($ChangeRequestId, $app_reference);			
			
				
				if($response['status'] == SUCCESS_STATUS){
					$cancellation_details = $response['data'];

					$this->transferv1_model->update_cancellation_refund_details($app_reference, $cancellation_details);
	...
	/**
	*Get Hotel HOLD Booking status (Transfers)
	*/
	function get_pending_booking_status($app_reference,$booking_source,$status){
		...
		if($status=='BOOKING_HOLD'){
			...
			if($booking_details['status']==1){
				...
				load_transferv1_lib($booking_source);
				$hold_booking_status = $this->transferv1_lib->get_sightseeing_booking_status($app_reference);
	...
	public function ticket_cancellation_details()
	{
		...
		if(isset($get_data['app_reference']) == true && isset($get_data['booking_source']) == true && isset($get_data['status']) == true){
			...
			if($booking_details['status'] == SUCCESS_STATUS){
				$this->load->library('booking_data_formatter');
				$booking_details = $this->booking_data_formatter->format_transferv1_booking_data($booking_details, $this->config->item('current_module'));
				$page_data = array();
				...
				$this->template->view('transferv1/ticket_cancellation_details', $page_data);
			} else {
				redirect(base_url());
			}
		} else {
