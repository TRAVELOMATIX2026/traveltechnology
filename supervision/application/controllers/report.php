...
class Report extends MY_Controller
{
    ...
    function b2b_hotel_report($offset = 0)
    {
        ...
        $page_data['search_params'] = $get_data;
        $page_data['status_options'] = get_enum_list('booking_status_options');
        $page_data['api_source_options'] = $this->hotel_model->get_hotel_source_options();
        $agent_info = $this->custom_db->single_table_records('user', '*', array('user_type' => B2B_USER, 'domain_list_fk' => get_domain_auth_id()));
        $page_data['agent_details'] = magical_converter(array('k' => 'user_id', 'v' => 'agency_name'), $agent_info);
    ...
    /*B2B Transfer Report*/
    function b2b_transfers_report_old($offset = 0)
    {
        $current_user_id = $GLOBALS['CI']->entity_user_id;
        $get_data = $this->input->get();
        $condition = array();
    ...
    /*B2c Transfer Report*/
    function b2c_transfers_report_old($offset = 0)
    {
        $condition = array();
        $get_data = $this->input->get();
        $filter_data = $this->format_basic_search_filters();
    ...
    function b2c_car_report($offset = 0)
    {
        ...
        $config['base_url'] = base_url() . 'index.php/report/b2c_car_report/';
        $config['first_url'] = $config['base_url'] . '?' . http_build_query($_GET);
        $page_data['total_rows'] = $config['total_rows'] = $total_records;
        $config['per_page'] = RECORDS_RANGE_2;
        $this->pagination->initialize($config);
    ...
    function b2c_flight_report($offset = 0)
    {
        ...
        $page_data['search_params'] = $get_data;
        $page_data['status_options'] = get_enum_list('booking_status_options');
        $page_data['api_source_options'] = $this->flight_model->get_flight_source_options();
    ...
    {
        $get_data = $this->input->get();
        $response =  $response =  array('filter_condition' => [], 'from_date' => '', 'to_date' => '');
        if (valid_array($get_data) == true) {
            ...
            if (empty($get_data['booking_source']) == false && strtolower($get_data['booking_source']) != 'all') {
                $filter_condition[] = array('BD.booking_source', '=', $this->db->escape($get_data['booking_source']));
            }
            /*if (empty($get_data['phone']) == false) {
            ...
            if (empty($get_data['domain']) == false) {
                $filter_condition[] = array('BD.domain', ' like ', $this->db->escape($get_data['domain']));
            }
            if (empty($get_data['app_reference']) == false) {
            ...
            $response =  array('filter_condition' => $filter_condition, 'from_date' => $from_date, 'to_date' => $to_date);
        }
        return $response;
    }
    # Get Pending Refund
    ...
    public function export_confirmed_booking_airline_report($op = '')
    {
        ...
        foreach ($flight_booking_data as $k => $v) {
            ...
            $export_data[$k]['api_source_name'] = $v['api_source_name'];
    ...
    public function export_confirmed_booking_airline_report_b2b($op = '')
    {
        $this->load->model('flight_model');
        $get_data = $this->input->get();
        $condition = array();
    ...
    public function export_confirmed_booking_hotel_report($op = '')
    {
        ...
        foreach ($hotel_booking_data as $k => $v) {
            ...
            $export_data[$k]['API'] = $v['api_source_name'];
    ...
    public function export_confirmed_booking_hotel_report_b2b($op = '')
    {
        $this->load->model('hotel_model');
        $get_data = $this->input->get();
        $condition = array();
        ...
        foreach ($hotel_booking_data as $k => $v) {
            $export_data[$k]['Reference No'] = $v['app_reference'];
            $export_data[$k]['API'] = $v['app_source_name'];
            $export_data[$k]['Confirmation_Reference'] = $v['confirmation_reference'];
            $export_data[$k]['lead_pax_name'] = $v['lead_pax_name'];
            $export_data[$k]['lead_pax_email'] = $v['lead_pax_email'];
            $export_data[$k]['lead_pax_phone_number'] = $v['lead_pax_phone_number'];
    ...
    public function export_confirmed_booking_bus_report_b2b($op = '')
    {
        $this->load->model('bus_model');
        $get_data = $this->input->get();
        $condition = array();
    ...
    public function export_confirmed_booking_car_report($op = '')
    {
        ...
        //Adding Confirmed Status Filter
        $condition[] = array('BD.status', '=', $this->db->escape('BOOKING_CONFIRMED'));
        $car_booking_data = $this->car_model->b2c_car_report($condition, false, 0, 2000); //Maximum 500 Data Can be exported at time
        $car_booking_data = $this->booking_data_formatter->format_car_booking_datas($car_booking_data, $this->current_module);
        $car_booking_data = $car_booking_data['data']['booking_details'];
        $export_data = array();
        // debug($car_booking_data);exit;
        $i = 1;
        foreach ($car_booking_data as $k => $v) {
            if ($op == 'csv') {
            ...
            $export_data[$k]['lead_pax_email'] = $v['lead_pax_email'];
            $export_data[$k]['lead_pax_phone_number'] = $v['lead_pax_phone_number'];
            $export_data[$k]['booking_reference'] = $v['booking_reference'];
            $export_data[$k]['car_name'] = $v['car_name'];
            $export_data[$k]['car_supplier_name'] = $v['car_supplier_name'];
            $export_data[$k]['supplier_identifier'] = $v['supplier_identifier'];
            $export_data[$k]['car_pickup_location'] = $v['car_pickup_lcation'];
            $export_data[$k]['car_drop_location'] = $v['car_drop_location'];
            $export_data[$k]['car_from_date'] = $v['car_from_date'] . "" . $v['pickup_time'];
            $export_data[$k]['car_to_date'] = $v['car_to_date'] . "" . $v['drop_time'];
            $export_data[$k]['booked_date'] = date('d-m-Y', strtotime($v['created_datetime']));
            $export_data[$k]['total_fare'] = $v['total_fare'];
            $export_data[$k]['admin_markup'] = $v['admin_markup'];
            $export_data[$k]['convinence_amount'] = $v['convinence_amount'];
            $export_data[$k]['discount'] = $v['discount'];
            $export_data[$k]['grand_total'] = $v['grand_total'];
        ...
        if ($op == 'excel') { // excel export
            ...
            $excel_sheet_properties = array(
                'title' => 'Confirmed_Booking_CarReport_' . date('d-M-Y'),
                'creator' => 'Accentria Solutions',
                'description' => 'Confirmed_Booking_CarReport',
                'sheet_title' => 'Confirmed_Booking_CarReport'
            );
            $this->load->library('provab_excel'); // we need this provab_excel library to export excel.
            $this->provab_excel->excel_export($headings, $fields, $export_data, $excel_sheet_properties);
        } else if ($op == 'csv') { // excel export
            // echo "dd";exit;
            $headings = array("Sl. No.", "Appreference", "Lead Pax Name", "Lead Pax Email", "Lead Pax Phone", "Confirm Number", "Car Name", "Supplier Name", "Supplier Identifier", "From", "To", "Pickup DateTime", "Drop DateTime", "BookedOn", "Admin NetFare", "Admin Markup", "Convn Fee", "Discount", "Grand Total", "Status");
            $this->load->library('provab_csv'); // we need this provab_excel library to export excel.
            // echo "dd";exit;
            $this->provab_csv->csv_export($headings, 'Car Confirmed Booking Report', $export_data);
        } else if ($op == 'pdf') {
            $this->load->library('provab_pdf');
            $this->load->library('provab_mailer');
            $create_pdf = new Provab_Pdf();
            $pdf_data['export_data'] = $car_booking_data;
            //debug($pdf_data['export_data']);exit;
            $mail_template = $this->template->isolated_view('report/b2c_report_car_pdf', $pdf_data);
            $this->provab_pdf->create_pdf($mail_template);
        }
    }
    public function export_cancelled_booking_car_report($op = '')
    {
        ...
        if ($op == 'excel') { // excel export
            ...
            $excel_sheet_properties = array(
                'title' => 'Cancelled_Booking_CarReport_' . date('d-M-Y'),
                'creator' => 'Accentria Solutions',
                'description' => 'Cancelled_Booking_CarReport',
                'sheet_title' => 'Cancelled_Booking_CarReport'
            );
            $this->load->library('provab_excel'); // we need this provab_excel library to export excel.
            $this->provab_excel->excel_export($headings, $fields, $export_data, $excel_sheet_properties);
        } else if ($op == 'csv') { // excel export
            ...
            $this->load->library('provab_csv'); // we need this provab_excel library to export excel.
            // echo "dd";exit;
            $this->provab_csv->csv_export($headings, 'Car Cancelled Booking Report', $export_data);
        } else if ($op == 'pdf') {
            $this->load->library('provab_pdf');
            $this->load->library('provab_mailer');
            $create_pdf = new Provab_Pdf();
    ...
    public function export_all_booking_car_report($op = '')
    {
        // debug($op);exit;
        // error_reporting(E_ALL);
        $this->load->model('car_model');
        ...
        // $condition[] = array('BD.status', '=', $this->db->escape('BOOKING_CONFIRMED'));
        $condition[] = array();
        $car_booking_data = $this->car_model->b2c_car_report($condition, false, 0, 2000);
        // debug($car_booking_data);exit;
        //Maximum 500 Data Can be exported at time
        $car_booking_data = $this->booking_data_formatter->format_car_booking_datas($car_booking_data, $this->current_module);
        // debug($car_booking_data);exit;
        ...
        if ($op == 'excel') { // excel export
            ...
            $excel_sheet_properties = array(
                'title' => 'All_Booking_Car_' . date('d-M-Y'),
                'creator' => 'Accentria Solutions',
                'description' => 'All_Booking_CarReport',
                'sheet_title' => 'All_Booking_CarReport'
            );
            $this->load->library('provab_excel'); // we need this provab_excel library to export excel.
            $this->provab_excel->excel_export($headings, $fields, $export_data, $excel_sheet_properties);
        } else if ($op == 'csv') { // excel export
            ...
            $this->load->library('provab_csv'); // we need this provab_excel library to export excel.
            // echo "dd";exit;
            $this->provab_csv->csv_export($headings, 'Car All Booking Report', $export_data);
        } else if ($op == 'pdf') {
            $this->load->library('provab_pdf');
            $this->load->library('provab_mailer');
            $create_pdf = new Provab_Pdf();
            $pdf_data['export_data'] = $car_booking_data;
            // debug($pdf_data['export_data']);exit;
            $mail_template = $this->template->isolated_view('report/b2c_report_car_all_pdf', $pdf_data);
            $this->provab_pdf->create_pdf($mail_template);
        }
    }
    public function export_confirmed_booking_car_report_b2b($op = '', $all = '')
    {
        // error_reporting(E_ALL);
        // debug("cc");exit;
        $this->load->model('car_model');
        ...
        $car_booking_data = $this->car_model->b2b_car_report($condition, false, 0, 2000); //Maximum 500 Data Can be exported at time
        $car_booking_data = $this->booking_data_formatter->format_car_booking_datas($car_booking_data, $this->current_module);
        $car_booking_data = $car_booking_data['data']['booking_details'];
        $export_data = array();
        // debug($car_booking_data);exit;
        ...
        foreach ($car_booking_data as $k => $v) {
            ...
            $i++;
            $export_data[$k]['ref_no'] = $v['app_reference'];
            $export_data[$k]['lead_pax_name'] = $v['lead_pax_name'];
            $export_data[$k]['lead_pax_email'] = $v['lead_pax_email'];
            $export_data[$k]['lead_pax_phone_number'] = $v['lead_pax_phone_number'];
            $export_data[$k]['confirmation_reference'] = $v['booking_reference'];
            $export_data[$k]['car_name'] = $v['itinerary_details'][0]['car_name'];
            $export_data[$k]['supplier_identifier'] = $v['supplier_identifier'];
            $export_data[$k]['car_pickup_lcation'] = $v['car_pickup_lcation'];
            $export_data[$k]['car_drop_location'] = $v['car_drop_location'];
            $export_data[$k]['car_from_date'] = $v['car_from_date'];
            $export_data[$k]['car_to_date'] = $v['car_to_date'];
            $export_data[$k]['pickup_time'] = $v['pickup_time'];
            $export_data[$k]['drop_time'] = $v['drop_time'];
            $export_data[$k]['agent_markup'] = $v['agent_markup'];
            $export_data[$k]['Admin_markup'] = $v['admin_markup'];
            $export_data[$k]['total_fare'] = $v['total_fare'];
            $export_data[$k]['grand_total'] = round($v['grand_total']);
            if ($all == "all") {
                $export_data[$k]['status'] = $v['status'];
            }
            $export_data[$k]['booked_on'] = date('d-m-Y', strtotime($v['voucher_date']));
        ...
        if ($op == 'excel') { // excel export
            ...
            if ($all == "all") {
                $headings['s1'] = 'Status';
                $headings['t1'] = 'Booked On';
            } else {
                $headings['s1'] = 'Booked On';
            }
            // field names in data set 
            $fields = array(
                'a' => '', // empty for sl. no.
                ...
                'f' => 'confirmation_reference',
                'g' => 'car_name',
                'h' => 'supplier_identifier',
                'i' => 'car_pickup_lcation',
                'j' => 'car_drop_location',
                'k' => 'car_from_date',
                'l' => 'car_to_date',
                'm' => 'pickup_time',
            ...
            if ($all == "all") {
                $excel_sheet_properties = array(
                    'title' => 'All_Booking_CarReport_' . date('d-M-Y'),
                    'creator' => 'Accentria Solutions',
                    'description' => 'All_Booking_CarReport',
                    'sheet_title' => 'All_Booking_CarReport'
                );
            ...
            // echo "dd";exit;
            if ($all == "all") {
                $headings = array("Sl. No.", "App Reference No", "Lead Pax Name", "Lead Pax Email", "Lead Pax Phone", "Confirmation_Reference", "Car Name", "Supplier Identifier", "From", "To", "Pickup From Date", "Pickup To Date", "Pickup Time", "Drop Time", "Agent Markup", "Admin Markup", "Fare", "Grand Total", "Status", "Booked On");
            } else {
                $headings = array("Sl. No.", "App Reference No", "Lead Pax Name", "Lead Pax Email", "Lead Pax Phone", "Confirmation_Reference", "Car Name", "Supplier Identifier", "From", "To", "Pickup From Date", "Pickup To Date", "Pickup Time", "Drop Time", "Agent Markup", "Admin Markup", "Fare", "Grand Total", "Booked On");
            }
            $this->load->library('provab_csv'); // we need this provab_excel library to export excel.
            // echo "dd";exit;
            if ($all == "all") {
            ...
            $pdf_data['record_type'] = $all;
            // debug($pdf_data['export_data']);exit;
            $mail_template = $this->template->isolated_view('report/b2b_report_car_pdf', $pdf_data);
            $this->provab_pdf->create_pdf($mail_template);
        }
    }
    public function export_cancelled_booking_car_report_b2b($op = '', $all = '')
    {
        // error_reporting(E_ALL);
        // debug("cc");exit;
        $this->load->model('car_model');
    ...
    function b2b_transfers_report($offset=0){
        ...
        // $total_records = $this->transferv1_model->b2b_transferv1_report($condition, true);
        $total_records = $this->transfer_model->b2b_transfer_report($condition, true);
        // echo $this->db->last_query();die;

        //echo '<pre>'; print_r($page_data); die;

        // $table_data = $this->transferv1_model->b2b_transferv1_report($condition, false, $offset, RECORDS_RANGE_2);
        $table_data = $this->transfer_model->b2b_transfer_report($condition, false, $offset, RECORDS_RANGE_2);
...

    {
        ...
        // $total_records = $this->transferv1_model->b2c_transferv1_report($condition, true);   
        $total_records = $this->transfer_model->b2c_transfer_report($condition, true);  
        ...
        // $table_data = $this->transferv1_model->b2c_transferv1_report($condition, false, $offset, RECORDS_RANGE_2);
        $table_data = $this->transfer_model->b2c_transfer_report($condition, false, $offset, RECORDS_RANGE_2);
        ...
        $this->template->view('report/b2c_transfer_report', $page_data);
