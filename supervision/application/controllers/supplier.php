...
class Supplier extends MY_Controller {
	...
	public function add_with_price() {
		...
		$data ["tours_continent"] = $this->Supplierpackage_Model->get_tours_continent();
	...
	public function add_package_new() {
		...
		$tours_country = implode(',',$this->input->post ( 'tours_country' ));
		$tours_city_new = implode(',',$this->input->post ( 'tours_city_new' ));
		$description = $this->input->post ( 'Description' );
		// $location = $this->input->post ( 'location' );
		$location = implode(',',$this->input->post ( 'tours_city_new' ));
		...
		$duration_status = $this->input->post ( 'duration_status' );
		...
		$newpackage = array (
				...
				'package_country' => $tours_country,
				'package_city' => $tours_city_new,
	...
	public function edit_with_price($package_id) {
		...
		$data ['price'] = $this->Supplierpackage_Model->get_price ( $package_id );
		$data ['countries'] = $this->Supplierpackage_Model->get_country_city_list ();
		$data ['continent_countries'] = $this->Supplierpackage_Model->continent_countries ();
		// print_r($data);exit;
		$data ["tours_continent"] = $this->Supplierpackage_Model->get_tours_continent();
		// debug($data);exit;
	...
	public function update_package($package_id) {
		...
		$data = array (
				'package_name' => $name,
				'package_location' => $tours_city_new,
				'package_description' => $description,
				'image' => $photo,
				'rating' => $rating,
				'price' => $p_price,
		...
		if($duration_status==0){
		$this->Supplierpackage_Model->delete_itinerary_without_itinerary($package_id);
		}
	...
	public function images($package_id) {
		if ($_FILES) {
			...
			$name_of_traveller_img = 'traveller-' . time () . $_FILES ['traveller'] ['name'];
			// echo $this->template->domain_uploads_packages ();exit;
			move_uploaded_file ( $_FILES ['traveller'] ['tmp_name'], $this->template->domain_uploads_packages ( $name_of_traveller_img ) );
			$traveller_img = $name_of_traveller_img;
			$sup_id = $this->entity_user_id;
			$user_type = $this->entity_user_type;
	...
	public function update_itinerary()
	{
		...
		$deleted_itinerary_ids = $this->input->post('deleted_itinerary_ids');
		...
		for ($i = 0; $i < $cp; $i++) {
			if (!empty($_FILES['imagelable' . $i]['name'])) {
				...
				move_uploaded_file($_FILES['imagelable']['tmp_name'], $this->template->domain_uploads_packages($_FILES['imagelable']['name']));
				$image = $this->template->domain_uploads_packages($_FILES['imagelable']['name']);
			} else {
				$image = $hiddenimage[$i];
			}
			...
			if (preg_match('/^new_\d+$/', $itinerary_id[$i])) {
				$this->Supplierpackage_Model->itinerary($data);
			} else {
				$id = $itinerary_id[$i];
				$this->Supplierpackage_Model->update_itinerary($package_id, $id, $data);
			}
		}
	
		if (!empty($deleted_itinerary_ids)) {
			$ids_to_delete = explode(',', $deleted_itinerary_ids);
			foreach ($ids_to_delete as $itinerary_id) {
				$this->Supplierpackage_Model->delete_itinerary($itinerary_id,$package_id);
			}
		}
		$this->Supplierpackage_Model->update_package_days($package_id, $cp);
    ...
    $tours_continent = $this->Supplierpackage_Model->ajax_tours_continent($tours_continent);          
    foreach($tours_continent as $key => $value)
    {
      $options .=  '<option value="'.$value['origin'].'">'.$value['name'].'</option>';
    } 
    echo $options;      
  }
  public function ajax_tours_country() {
    ...
    $tours_country = $this->Supplierpackage_Model->ajax_tours_country($tours_country);          
        //debug($tours_country); exit; 
    foreach($tours_country as $key => $value)
    {
      $options .=  '<option value="'.$value['destination'].'">'.$value['destination'].'</option>';
    } 
    echo $options;           
  }
}