...
class Ajax extends MY_Controller {
...

	{
		...
		if ($country_id!='') {
			...
			$option_list = $this->custom_db->single_table_records('tbo_city_list', 'origin as k, city_name as v', $condition, 0, 1000000, $order_by);
	...
	public function get_city_lists()

 	{
		...
		$get_resulted_data =  $this->custom_db->single_table_records('api_city_list', '*',array('country' => $country_id,'state_id'=>$state_id), 0, 100000000, array('destination' => 'asc'));
	...
	function get_state_lists($country_id=0, $default_select=0)

	{

		$country_id = $this->input->get('country_id');

		$get_resulted_data =  $this->custom_db->single_table_records('api_state_list', '*',array('country_id' => $country_id), 0, 100000000, array('state_province' => 'asc'));
		...
		if(!empty($get_resulted_data['data'])){ 
			...
			$html = "<option value=''>Select State</option>";
				...
				foreach( $get_resulted_data['data'] as  $get_resulted_data_sub){
					...
					$html= $html."<option value=".$get_resulted_data_sub['origin'].">".$get_resulted_data_sub['state_province']."</option>";
	...
	*/
...

	{
		...
		$table_data = $this->booking_data_formatter->format_car_booking_datas($data_list, 'b2c');
		...
		if (valid_array($booking_details) == true) {
			...
			foreach ($booking_details as $k => $v) {
				...
				$calendar_events[$key]['add_class'] = 'hand-cursor event-hand car-booking';
