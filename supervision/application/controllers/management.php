...
class Management extends MY_Controller
{
						...
						foreach ($page_data['form_data']['airline_origin'] as $__k => $__domain_origin) {
							if (
								...
								$this->domain_management_model->save_markup_data(
									...
									$page_data['form_data']['form_values_origin'],
									$markup_module_type,
									$page_data['form_data']['airline_origin'][$__k],
				...
				case 'add_airline'; //Balu A
					if (isset($page_data['form_data']['airline_code']) == true && empty($page_data['form_data']['airline_code']) == false) {
						...
						$this->domain_management_model->save_markup_data(
							...
							$markup_module_type,
							$airline_list_origin,
							$page_data['form_data']['specific_value'],
							$page_data['form_data']['value_type'],
							get_domain_auth_id()
						);
					}
	...
	{
		if (
			isset($commission_details['module']) == true && empty($commission_details['module']) == false &&
				...
				if (valid_array($_FILES) == true and $_FILES['bank_icon']['error'] == 0 and $_FILES['bank_icon']['size'] > 0) {
					if (function_exists("check_mime_image_type")) {
						if (!check_mime_image_type($_FILES['bank_icon']['tmp_name'])) {
							$msg = "Please select the image files only (gif|jpg|png|jpeg)";
							$this->session->set_flashdata(array('message' => $msg, 'type' => SUCCESS_MESSAGE));
							refresh();
						}
					}
	...
	*/
	...
	public function update_airport_international_status($origin, $status)
	{
		...
		redirect(base_url() . 'management/airport_list_master');
	}
	...
	public function airport_list_master($offset = 0)
	{
		...
		$this->load->library('pagination');
		$config['base_url'] = base_url() . 'index.php/management/airport_list_master/';
		$page_data['total_rows'] = $config['total_rows'] = $total_records[0]['total'];
		$config['per_page'] = RECORDS_RANGE_3;
		$this->pagination->initialize($config);
		/** TABLE PAGINATION */
		$page_data['total_records'] = $config['total_rows'];
		$this->template->view('management/airport_list_master', $page_data);
	}
	public function add_airport_master()
	{
		...
		$this->template->view('management/add_airport_master', $page_data);
	}
	public function edit_airport_master($origin)
	{
		...
		$this->template->view('management/edit_airport_master', $page_data);
	}
